#!/usr/bin/env python3
"""Enqueue bounded, repeated KV smoke retries after the asynchronous release fix."""
...
SNAPSHOT = OUT / "smoke-kv-retry-source-v2"
...
def main() -> None:
    ...
    jobs = []
    for model, tp in (("7b", 2), ("7b", 4), ("14b", 4), ("32b", 2), ("32b", 4)):
        name = f"kv-retry-v2-{model}-tp{tp}-pp1"
        argv = [
            "docker", "run", "--rm", "--name", name, "--gpus", "all",
            "--cap-add", "SYS_ADMIN", "--ipc=host", "--network", "host",
            "--shm-size", "16g", "--ulimit", "nofile=65536:65536",
            "--entrypoint", "/opt/venv/bin/python",
            "-v", f"{SNAPSHOT}/src:/opt/pdblend-src:ro", "-v", "/home/models:/models:ro",
            "-v", "{attempt_dir}:/output:rw", "-e", "PYTHONPATH=/opt/pdblend-src",
            "-e", "PDBLEND_MODELS_DIR=/models", "-e", f"PDBLEND_SOURCE_SHA256={source}",
            "-e", f"PDBLEND_IMAGE_ID={IMAGE}", IMAGE, "-B", "-m", "pdblend.bench.topology_smoke",
            "--model", model, "--tp", str(tp), "--pp", "1", "--out", "/output",
        ]
        queue.enqueue(
            name,
            {"argv": argv, "gpu_count": 8, "exclusive": True, "global_lock": True,
             "container_name": name, "required_receipts": ["completion.json"],
             "timeout_s": 2400, "evidence_class": "kv_retry_v2"},
            priority=10, depends_on=["three-model-smoke-7b-tp1-pp1"], max_attempts=1,
        )
        jobs.append(name)
