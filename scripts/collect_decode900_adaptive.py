#!/usr/bin/env python3
"""Validate all adaptive round6 seeds and emit mean/std/worst statistics."""
...
def stats(key):
    vals=[r[key] for r in rows]; return dict(mean=statistics.mean(vals),std=statistics.stdev(vals),worst=max(vals))
result=dict(profile=str(a.profile.resolve()),profile_sha256=verify_audit_binding(a.profile),rows=rows,stats={k:stats(k) for k in ('joint_slo_rate','ttft_p90','ttft_p99','tpot_p99','mean_power_w','window_mean_power_w','j_per_token','plans','wakes','parks')},gate=dict(all_slo=all(r['joint_slo_rate']>=.9 and r['ttft_p99']<=5 and r['tpot_p99']<=.15 for r in rows),three_seed=len(rows)==3))
