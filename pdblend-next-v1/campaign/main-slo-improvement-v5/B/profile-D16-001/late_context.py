...
def derive_all(root, *, original_evidence, tp2_validator):
    ...
    for index in range(6):
        ...
        if not (spec and spec.get('batch_size') == 16 and spec.get('input_lengths') == [512]*16
                and spec.get('output_lengths') == [256]*16 and spec.get('clock_command_mhz') == (1500 if index < 3 else 2520)
                and spec.get('budget_tokens') == 8192 and spec.get('max_num_seqs') == 32
            ...
            result['valid'] = False
            result['errors'].append(dict(stage='declaration', error='not the six original prescribed B16 work points'))
        if path.exists():
