...
INPUT = 512
OUTPUT = 256
REQUIRED_KV_TOKENS = BATCH * (INPUT + OUTPUT)  # 12288; 768 is block-16 aligned.
INITIAL_PREFILL_TOKENS = BATCH * INPUT  # 8192; not an observed first-step claim.
TOKENS = 8192
SEQS = 32
...
def validate_runtime_capacity(state, *, require_accepting=True, now=None):
    ...
    require(INITIAL_PREFILL_TOKENS <= TOKENS and BATCH <= SEQS and INPUT+OUTPUT <= limits['max_model_len'],
    ...
    require(type(free) is int and type(total) is int and REQUIRED_KV_TOKENS <= free <= total,
            'real free KV cannot hold all16 complete 512+256 allocations')
...
def validate_batch16_observation(raw, events):
    ...
    require(spec['batch_size']==BATCH and spec['input_lengths']==[INPUT]*BATCH
            ...
            'point is not declared real16 concurrent 512->256')
