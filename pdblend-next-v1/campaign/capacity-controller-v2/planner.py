...
class Resident:
    ...
    def idle(self,now,age):
        return (self.accepting and self.transport_healthy and not self.error
...
class Demand:
    ...
    def __post_init__(self):
        if (not all(finite(v) for v in (self.observed_at_s,self.history_span_s,
                self.rate_lower_rps,self.rate_upper_rps,self.oldest_queue_wait_s)) or self.rate_lower_rps>self.rate_upper_rps
...
class CapacityPlanner:
    ...
    def choose(self,snapshot,demand,state,now):
        ...
        deficit=(demand.rate_upper_rps>cap or (demand.queued_requests>0 and
            (not current or demand.oldest_queue_wait_s>=p.queue_pressure_s)))
        high=deficit or (cap>0 and demand.rate_upper_rps>=p.up_utilization*cap)
        ...
        if high:
            ...
            if not deficit and (cooldown or now-state.high_since_s<p.high_hold_s):
            ...
            for slot in snapshot.spares:
                ...
                if not deficit and now-slot.unallocated_since_s<p.min_off_s:continue
                ...
                proposal=Proposal('restore','capacity_deficit' if deficit else 'capacity_headroom',
