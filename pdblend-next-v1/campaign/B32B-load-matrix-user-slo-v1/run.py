...
def package_check():
    ...
    original=read(ROOT/'inputs/controller.source.json');original['controller_source_release']=str(HOST)
    require(cfg==original,'policy differs from verified original except corrected source label')
    require(not cfg.get('prediction_cap_to_max_tokens') and not cfg.get('output_limit_aware_prediction') and cfg['output_prior']==211,'unrequested output-cap policy')
    require(read(ROOT/'inputs/controller.uniform-source.json')['output_prior']==211,'uniform prior source differs')
    carried=read(ROOT/'inputs/carried-alpaca-audit.json')
    require(carried.get('observed_cell_valid') and carried.get('work_complete') and carried.get('clock_release_complete')
        and carried.get('baseline_preservation_verified') and carried.get('completed')==64 and carried.get('original_wrapper_failed') is True,'carried Alpaca raw/cleanup audit missing')
    return spec
...
def dependencies(evidence):
    ...
    anchors_path=ROOT.parent/'B32B-load-screen-user-slo-v1-r2/status.pilots.json'
    anchors=read(anchors_path)
    require(anchors.get('complete') is True and anchors.get('phase')=='finished' and len(anchors.get('cells',[]))==2
        and all(r.get('screen_valid') for r in anchors['cells']),'remaining two short anchors must finish valid before long matrix')
    add(anchors_path)
    for row in anchors['cells']:
        for name in ('summary.json','runtime_config.json','bench.csv','cleanup.json'):
            add(ROOT.parent/'B32B-load-screen-user-slo-v1-r2/cells'/row['cell_id']/name)
...
async def run(part):
    from execution import sweep
    await sweep(part,sys.modules[__name__])
...
async def cli(args):
    ...
    else:await run(args.part)


def main():
    p=argparse.ArgumentParser(description=__doc__);p.add_argument('--phase',choices=('check','prepare','run'),default='check')
    p.add_argument('--part',choices=('matrix',),default='matrix');args=p.parse_args()
