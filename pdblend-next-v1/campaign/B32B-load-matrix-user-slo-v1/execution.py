...
class Cell:
    def __init__(self,b,session,row,freeze,status,status_name,hardware):
    ...
    async def execute(self):
        ...
        try:
            ...
            until=time.monotonic()+self.row['trace_duration_s']+330
            ...
            try:cleanup=await self.cleanup();self.receipt['outer_cleanup']=cleanup
...
async def sweep(part,b):
    ...
    status=dict(part=part,phase='preflight',complete=False,started_s=time.time(),cells=[],baseline_execution=False,
        scope='B32B two TP2 budget8192 exploratory development rates; not verified capacity or formal evidence')
    b.write(name,status);freeze=None
    try:
        freeze=await asyncio.to_thread(b.frozen_check,True)
        if part=='remaining':
            pilots=b.read(b.ROOT/'status.pilots.json')
            b.require(pilots.get('complete') is True and len(pilots.get('cells',[]))==3
                and all(c.get('screen_valid') for c in pilots['cells']),'three valid pilot measurements/cleanup required')
