...
def parse_args(argv=None):
    ...
    p.add_argument('--action',choices=('check','prepare','run'),default='check')
    p.add_argument('--phase',choices=('probe','main','scale','run'),default='main')
    p.add_argument('--part',choices=('main','scale'))
    p.add_argument('--max-cells',type=int,default=1)
    args=p.parse_args(argv)
    if args.phase=='run':
        require(args.part is not None and args.action in ('check','run'),'bridge requires explicit main/scale part')
        args.action='run';args.phase=args.part
    else:require(args.part is None,'part is only valid with bridge --phase run')
    require(args.max_cells>0,'positive max-cells required')
    return args
