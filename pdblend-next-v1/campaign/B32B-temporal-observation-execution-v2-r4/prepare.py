...
def prepare(out,proof):
    ...
    previous=Path(gate['groups']['distserve']['binding']);binding=c.read(previous);c.binding_scope(binding)
    ...
    obs['output_dir']=str(out/'results/capture-live');c.write(out/'observation-spec.json',obs)
    instance=copy.deepcopy(deployment['instances'][1]);cfg=c.read(instance['config'])
    iid='temporalobsb1';port=34501;kv=34732
    ...
    instance.update(id=iid,port=port,kv_port=kv,url=f'http://127.0.0.1:{port}',role='mixed',
        config=str(out/'engine.json'),container_name='pdb-v2-temporalobsb1',image=None)
    ...
    files=dict(gate['files']);files.update(binding['files'])
...
def main():
    ...
    if not a.prepare:print(json.dumps(dict(cpu_only=True,hardware_actions=False,prepared=False,requires_real_three_main_terminal=True)));return
