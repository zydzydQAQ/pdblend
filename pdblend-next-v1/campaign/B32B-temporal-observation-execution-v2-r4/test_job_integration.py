...
@pytest.mark.parametrize('revision',['v1','v2','v2-r2','v2-r3','v2-r4'])
def test_actual_entry_claim_guard_covers_all_revisions(tmp_path,monkeypatch,revision):
    own=tmp_path/'B32B-temporal-observation-execution-v2-r4';own.mkdir()
    monkeypatch.setattr(c,'ROOT',own)
    current.no_prior_claims()
    claim=tmp_path/('B32B-temporal-observation-execution-'+revision)/'execution-once.json'
    claim.parent.mkdir(exist_ok=True);claim.write_text('{}')
    with pytest.raises(RuntimeError,match='prior attempt claim'):current.no_prior_claims()
