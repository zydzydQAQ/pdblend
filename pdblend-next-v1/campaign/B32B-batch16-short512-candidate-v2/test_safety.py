...
def test_exact_natural_spec_stays_six_short16_points_no_temporal():
 a=old.arguments();assert a.input_patterns==[[512]] and a.output_pattern==[256] and a.batches==[16]
 assert a.frequencies==[1500,2520] and a.budgets==[8192] and a.repeats==3 and a.target_gpus==[0,1]
 assert len(a.batches)*len(a.frequencies)*a.repeats==6 and a.arrival_offsets==[0.]
...
def test_complete_source_union_keeps_original_transfer_files_and_release_sources():
 expected=json.loads((ROOT/'expected-identity.json').read_text())
 actual=json.loads((ROOT/'complete-identity-reference-source.json').read_text())
 aux=json.loads((ROOT/'auxiliary-vllm-sources.json').read_text());assert len(aux)==2
 actual['live_vllm_and_serving_source_sha256'].update(aux)
 old.validate_identity(actual,expected)
 assert len(expected['live_vllm_and_serving_source_sha256'])==73

def test_sampler_error_cancels_pending_work_for_outer_cleanup(gate,monkeypatch):
 sampler=SimpleNamespace(error=None);seen=[]
 async def work(hardware):
  seen.append('started');sampler.error='NVML timestamp rejected'
  try:await asyncio.Event().wait()
  finally:seen.append('cancelled')
 monkeypatch.setattr(gate,'work',work)
 with pytest.raises(RuntimeError,match='outer instant sampler failed'):
  asyncio.run(gate.monitored_work(None,sampler))
 assert seen==['started','cancelled']


def test_byte_gate_checks_every_prepared_package_and_dependency(tmp_path):
 import hashlib
 h=lambda p:hashlib.sha256(Path(p).read_bytes()).hexdigest()
 package_file=tmp_path/'source.json';dependency=tmp_path/'model.bin';package_file.write_text('source');dependency.write_bytes(b'weight')
 package=tmp_path/'package-manifest.json';package.write_text(json.dumps({'files':{'source.json':h(package_file)}}))
 freeze=tmp_path/'freeze.json';freeze.write_text(json.dumps({'package_manifest_sha256':h(package),'dependencies':{'files':{str(dependency):h(dependency)}}}))
 fake=SimpleNamespace(ROOT=tmp_path,read=lambda p:json.loads(Path(p).read_text()),sha=h)
 assert b.verify_frozen_bytes(fake)['dependencies']['files'][str(dependency)]==h(dependency)
 dependency.write_bytes(b'changed')
 with pytest.raises(RuntimeError):b.verify_frozen_bytes(fake)


def test_actual_outer_work_refreshes_capacity_before_any_control(gate,monkeypatch):
 calls=[]
 async def identity(label):calls.append('identity');return {'instances':[]}
 async def runtime(port,label):
  calls.append(('runtime',port));return dict(id='nextv3b0' if port==33500 else 'nextv3b1',timestamp=0)
 async def create(*a,**k):pytest.fail('capacity failure cannot launch child')
 monkeypatch.setattr(gate,'identity',identity);monkeypatch.setattr(gate,'runtime',runtime)
 monkeypatch.setattr(b.asyncio,'create_subprocess_exec',create)
 with pytest.raises(RuntimeError,match='stale'):asyncio.run(gate.work(None))
 assert calls==['identity',('runtime',33500),('runtime',33501)]
 assert not gate.verified and gate.child is None


def test_direct_child_entry_requires_actual_outer_parent(tmp_path,monkeypatch):
 monkeypatch.setattr(child,'ROOT',tmp_path);monkeypatch.setattr(child.os,'getppid',lambda:100)
 with pytest.raises(RuntimeError,match='owned'):child.verify_owned_parent(101)
 (tmp_path/'status.json').write_text(json.dumps(dict(verified_for_controls=False,phase='preflight')))
 with pytest.raises(RuntimeError,match='gate'):child.verify_owned_parent(100)
 (tmp_path/'status.json').write_text(json.dumps(dict(verified_for_controls=True,phase='natural_batch_observations')))
 child.verify_owned_parent(100)
