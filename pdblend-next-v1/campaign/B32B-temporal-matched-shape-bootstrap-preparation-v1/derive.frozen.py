...
def qualification_contract(q,binding,gate,inventory):
    ...
    require(old.get('passed') is False and old.get('mechanism_gate',{}).get('temporal') is False
        and checks.get('checks',{}).get('temporal_exact') is False,'do not rewrite original failed exact gate')
...
def bind(a):
    require(time.time()+400<DEADLINE,'original deadline cannot fit another cell/cleanup')
    ...
    original,checks,gate_files=qualification_contract(q,b,a.gate.resolve(),inventory)
    files=dict(b['files'])
    ...
    receipt=dict(complete=True,binding=dict(path=str(bp.resolve()),sha256=sha(bp)),system='ecoserve',main_cells=30,
        ...
        original_gate_passed=False,original_temporal_exact=False,measurement_or_controls_started=False,
