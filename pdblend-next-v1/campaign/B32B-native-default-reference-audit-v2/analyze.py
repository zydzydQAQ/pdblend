...
N=C/'B32B-native-default-reference-attempt-002';T=C/'B32B-temporal-observation-attempt-003'
...
files += list((N/'results/prefill-capture-frozen').glob('*')) + [N/'results/worker-config.json']
...
report=dict(schema=1,checked_s=time.time(),actual_terminal_observed_s=terminal['observed_s'],performance_evidence=False,
    ...
    all256_equal_original003=all(x['exact'] for x in parity.values()),prefill_evidence=status['owner_evidence']['prefill_evidence'],per_request_full64_parity=parity,native_pair_vs_own_solos=[diff(n[ids[j]],n[ids[j+2]]) for j in range(2)],
