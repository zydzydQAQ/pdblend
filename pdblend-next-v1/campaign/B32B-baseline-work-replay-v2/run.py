...
def package_check():
    ...
    return validate(sys.modules[__name__])
...
async def cli(args):
    ...
    if args.action=='prepare':await prepare()
        ...
        from replay_queue import sweep
        await sweep(sys.modules[__name__])

def main():
    p=argparse.ArgumentParser(description=__doc__);p.add_argument('--action',choices=('check','prepare','run'),default='check')
    p.add_argument('--phase',choices=('anchors',),default='anchors');p.add_argument('--max-cells',type=int,default=1);args=p.parse_args()
