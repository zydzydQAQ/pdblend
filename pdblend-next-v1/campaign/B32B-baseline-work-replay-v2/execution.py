...
def expected_configuration(b,row,out):
    config=b.read(b.CONFIG)
    config['journal']=str(out/'control.jsonl')
    config.update({k:row[k] for k in ('slo_ttft_s','slo_tpot_s')})
    config['slo_attainment_target']=.9
    return config
...
class Cell:
    def __init__(self,b,session,row,freeze,status,status_name,hardware,*,limits):
        ...
        self.limits=limits;self.b.require(limits["arrival_span_s"]==row["trace_duration_s"],"wrong replay limits")
        ...
        for name in self.b.IDS:
            ...
            for line in parts:
                ...
                self.b.require(event.get('mode')=='continuous' and event.get('role')=='mixed'
                    and type(event.get('tokens')) is int and 0<event['tokens']<=8192,'wrong owner mode/budget execution')
    ...
    async def execute(self):
        ...
        try:
            ...
            b.require(time.time()<=self.limits['latest_arrival_epoch_s'],'startup time exhausted before controls')
            start=self.receipt['operation_start_s']=time.time();self.mutated=True
            setup=await asyncio.gather(*(set_one(b,self.session,i,8192) for i in range(2)),return_exceptions=True)
            ...
            b.require(not sampler.error and time.time()<=self.limits['latest_arrival_epoch_s'],'sampling failed or startup time exhausted before child')
            ...
            until=self.limits['cell_execution_deadline_s']
            waiter=asyncio.create_task(self.child.wait())
            try:
                while not waiter.done():
                    await asyncio.wait({waiter},timeout=.25);self.poll_events()
                    b.require(not sampler.error,'outer instantaneous power failed')
                    b.require(time.time()<until,'host exceeded bounded original-work replay deadline')
            ...
            expected_config=expected_configuration(b,self.row,self.out)
            ...
            from replay_evidence import validate_replay
            validate_replay(b,self.row,summary,b.read(self.operation/'actual_epoch_gate.json'),self.limits,self.out)
            ...
            after=await asyncio.gather(*(wait_idle(b,self.session,i,tokens=8192,accepting=None) for i in range(2)))
            ...
            try:
                b.require(start is not None and len(sampler.samples)>=2
                    and sampler.samples[0][0]<=start<end<=sampler.samples[-1][0]
                    and not sampler.error,'incomplete whole-operation power coverage')
                self.receipt['full_operation_energy_j']=trapezoid_energy(clip_power_window(sampler.samples,start,end,pad_s=0))
