...
def cache_fixture_arguments(config):
    """An explicit real cache allocation, solely for bounded mechanism tests."""
    blocks=config.get('num_gpu_blocks_override')
    diagnostic=config.get('kv_capacity_fixture',False)
    if blocks is None and diagnostic is False:return {}
    if diagnostic is not True or type(blocks) is not int or blocks<=0:
        raise ValueError('positive num_gpu_blocks_override requires explicit kv_capacity_fixture=true')
    return dict(num_gpu_blocks_override=blocks)


class EngineService:
    def __init__(self, config):
        cache_fixture_arguments(config)
        self.config = config
    ...
    def initialize(self):
        ...
        args.update(cache_fixture_arguments(c))
    ...
    def update_snapshot(self):
        ...
        self.snapshot = dict(id=self.config["id"], timestamp=time.time(),
            ...
            **self.state, running=len(allocations), waiting=len(sched.waiting),
            ...
            block_size=block_size,num_gpu_blocks=self.engine.cache_config.num_gpu_blocks,
            num_gpu_blocks_override=self.config.get('num_gpu_blocks_override'),
            kv_capacity_fixture=self.config.get('kv_capacity_fixture',False),
    ...
    async def provenance(self,request):
        ...
        return web.json_response(dict(instance_id=self.config['id'],pid=os.getpid(),
            ...
            num_gpu_blocks_override=self.config.get('num_gpu_blocks_override'),
            kv_capacity_fixture=self.config.get('kv_capacity_fixture',False),
            block_size=self.snapshot.get('block_size'),num_gpu_blocks=self.snapshot.get('num_gpu_blocks'),
            total_kv_tokens=self.snapshot.get('total_kv_tokens'),
