...
def verify(reference):
    ...
    assert qualification['schema'] == 'migration-B-fixed14B-qualification-v1'
    assert qualification['datasets'] == ['sharegpt']
    ...
    assert set(binding['configs']) == {'sharegpt'}
    ...
    for dataset, template in spec['config_templates'].items():
        ...
        expected.update(instances=copy.deepcopy(bootstrap['instances']), port=34950,
    ...
    for case in cases:
        ...
        for row, clock in zip(case['requests'], case['loaded_clocks']):
            ...
            final_clock = q.clock_window(clocks, instance['gpus'], frequency, row['token_received_s'][0], row['token_received_s'][-1])
            assert {k:v for k,v in final_clock.items() if k != 'samples'} == {k:v for k,v in clock.items() if k != 'samples'}
            assert final_clock['samples'] >= clock['samples']
    ...
    common = b.load(spec['common_executor']['path'], 'migrationB_saved_native_barrier')
    ...
    hooks = b.load(p.HOOKS, 'migrationB_fixed_saved_observers')
    ...
    return dict(passed=True, independently_recomputed=True, node=p.NODE, model='14b',
                binding=qualification['binding'], qualification=reference, host_manifest=p.ref(p.HOST / 'manifest.json'),
                datasets=['sharegpt'], native_shape_cases=82, native_requests=len(requests_seen),
                ...
                files={str(Path(__file__)): p.sha(__file__), str(p.HERE / 'qualify_fixed.py'): p.sha(p.HERE / 'qualify_fixed.py')})
