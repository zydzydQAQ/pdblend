...
def test_stop_between_power_and_frequency_keeps_complete_tail(fake):
 a,tmp=fake
 backend=tmp/'host/src/ecopadg/measure/backends.py'
 marker=tmp/'tail-in-progress'
 source=backend.read_text().replace(' def current_freq(self,gpu):return 1500',f" def current_freq(self,gpu):\n  if gpu==0:\n   from pathlib import Path\n   Path({str(marker)!r}).write_text('started')\n   time.sleep(.12)\n  return 1500")
 backend.write_text(source)
 hostref=m._CONTEXT[1];v=json.loads(Path(hostref['path']).read_text());v['files']['src/ecopadg/measure/backends.py']=ref(backend)['sha256'];newref=write(Path(hostref['path']),v);m._CONTEXT=(m._CONTEXT[0],newref,m._CONTEXT[2])
 a.start()
 until=time.monotonic()+3
 while not marker.exists():
  assert time.monotonic()<until;time.sleep(.002)
 # The original sampler has appended power and util but is inside freq read.
 a.stop()
 assert a.error is None and len(a.samples)==len(a.frequency_samples)>=1
