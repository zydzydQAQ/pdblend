...
from vllm.pdblend_transfer_stats import SyncSendCounters
...
def set_p2p_nccl_context(num_channels: str):
    ...
    try:
        ...
        # Respect the deployment's CUDA IPC compatibility setting. Forcing
        # cuMem here breaks single-node L20 containers configured with it off.
...
class P2pNcclEngine:
    ...
    def __init__(self,
        ...
        self.sync_send_counters = SyncSendCounters() if self.send_type == "PUT" else None
        ...
        self.buffer_size = 0
        self.strict_ids = config.get_from_extra_config("request_id_safety", False)
        self.seen_tensors = OrderedDict()
        self.cancelled_requests = OrderedDict()
        self.inflight_receives = 0
    ...
    def _create_connect(self, remote_address: typing.Optional[str] = None):
        ...
        if remote_address not in self.socks:
            ...
            sock.setsockopt(zmq.RCVTIMEO, int(1000*self.config.get_from_extra_config('recv_timeout_s',30)))
    ...
    def send_tensor(
        ...
        if remote_address is None:
            ...
            if self.send_type == "PUT":
                return self._send_put_counted(tensor_id, tensor, remote_address)
    ...
    def recv_tensor(
        ...
        if self.send_type == "PUT" or self.send_type == "PUT_ASYNC":
            ...
            with self.recv_store_cv:
                timeout = float(self.config.get_from_extra_config("recv_timeout_s", 30.0))
                while tensor_id not in self.recv_store:
                    if not self._listener_thread.is_alive():
                        raise RuntimeError('KV listener unavailable')
                    if tensor_id.split('#')[0] in self.cancelled_requests:
                        raise RuntimeError('KV receive cancelled')
                    remaining = timeout - (time.time() - start_time)
                    if remaining <= 0:
                        raise TimeoutError("KV receive timed out: " + tensor_id)
                    self.recv_store_cv.wait(timeout=min(remaining,.1))
                tensor = self.recv_store.pop(tensor_id)
                request_id = tensor_id.split("#")[0]
                owned = self.recv_request_id_to_tensor_ids.get(request_id, set())
                owned.discard(tensor_id)
                if not owned:
                ...
                if isinstance(tensor, tuple):
                    ...
                    with self.pool_lock:
                        try:
                            ...
                            # load_tensor copies from pinned memory but does
                            # not release its allocation. This request owns it.
                            self.pool.free(addr)
                else:
                    with self.recv_store_cv:
        ...
        while True:
            ...
            if self.router_socket in socks:
                ...
                if data["cmd"] == "NEW":
                    ...
                    with self.recv_store_cv:
                        if self.strict_ids and (tensor_id in self.seen_tensors or
                                tensor_id.split("#")[0] in self.cancelled_requests):
                            self.router_socket.send_multipart([remote_address, b"2"])
                            continue
                        if self.strict_ids:
                            size=math.prod(data['shape'])*torch.empty((),dtype=getattr(torch,data['dtype'])).element_size()
                            if size+self.buffer_size>self.buffer_size_threshold:
                                # PDBlend reserves GPU staging capacity before
                                # prefill. Do not silently spill beyond it.
                                self.router_socket.send_multipart([remote_address,b'3'])
                                continue
                        if self.strict_ids:
                            self.seen_tensors[tensor_id] = None
                            if len(self.seen_tensors) > 100000:
                                self.seen_tensors.popitem(last=False)
                        self.inflight_receives += 1
                    try:
                        acknowledged=False
                    ...
                    with self.recv_store_cv:
                        ...
                        if tensor_id.split("#")[0] in self.cancelled_requests:
                            if isinstance(tensor, tuple):
                                with self.pool_lock:
                                    self.pool.free(tensor[0])
                            elif tensor is not None:
                                self.buffer_size -= tensor.element_size() * tensor.numel()
                            if self.strict_ids and acknowledged:
                                self.router_socket.send_multipart([remote_address,b'2'])
                            continue
                        self.recv_store[tensor_id] = tensor
                        ...
                        self.recv_store_cv.notify()
                    if self.strict_ids and acknowledged:
                        self.router_socket.send_multipart([remote_address,b'0' if tensor is not None else b'1'])
    ...
    def request_state(self, cancel_nonce=None):
        with self.recv_store_cv:
            if cancel_nonce is not None:
                self.cancelled_requests[cancel_nonce] = None
                if len(self.cancelled_requests) > 100000:
                    self.cancelled_requests.popitem(last=False)
                for tensor_id in self.recv_request_id_to_tensor_ids.pop(cancel_nonce, set()):
            ...
            send_state = (self.sync_send_counters.snapshot() if self.sync_send_counters is not None
                          else dict(send_counters_observed=False, inflight_sends=None,
                                    send_started=None, send_completed=None, send_failed=None,
                                    send_healthy=None, send_counter_scope="unsupported send mode"))
            return dict(buffered_tensors=len(self.recv_store), **send_state,
                        inflight_receives=self.inflight_receives,
                        buffered_gpu_bytes=self.buffer_size,
                        buffer_capacity_bytes=int(self.buffer_size_threshold),
                        allocations={rid:sum(self.recv_store[t].element_size()*self.recv_store[t].numel()
                            for t in ids if t in self.recv_store and isinstance(self.recv_store[t],torch.Tensor))
                            for rid,ids in self.recv_request_id_to_tensor_ids.items()},
                        listener_alive=self._listener_thread.is_alive())
    ...
    def _send_put_counted(self, tensor_id, tensor, remote_address):
        # Count the real synchronous call, including connection setup, CUDA
        # completion and the strict receiver ACK. Never clear counters merely
        # because a cancellation request arrived or a send queue looks empty.
        self.sync_send_counters.begin()
        completed = False
        error = None
        try:
            completed = self._send_sync(tensor_id, tensor, remote_address)
            return completed
        except BaseException as exc:
            error = "%s: %s" % (type(exc).__name__, exc)
            raise
        finally:
            self.sync_send_counters.finish(completed, error)
    ...
    def _send_sync(
        ...
        if self.strict_ids and sock.recv()!=b'0':
            return False
    ...
    def _send(self, comm, tensor: torch.Tensor, dst: int, stream=None):
        ...
        # The model and head-slicing kernels run on the caller's stream.
        # NCCL's dedicated stream must observe those writes before reading KV.
