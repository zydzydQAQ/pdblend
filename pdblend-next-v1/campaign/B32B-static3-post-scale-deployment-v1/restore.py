"""B static-three deployment after actual model main150 and scale90.
...
def validate_previous(previous):
    require(previous==contracts.read(contracts.PREVIOUS),'exact actual B scale baseline binding required')
    require([(i['tp'],i['gpus']) for i in previous['instances']]==[(2,[0,1]),(2,[2,3]),(2,[4,5]),(2,[6,7])],'four current resident TP2s required')
...
async def failure_cleanup(spec,cleanup,result,m):
    """Capture cancellation for outer archival; no further controls after it."""
    async def stop_owned(x):
        try:await m.command(['docker','stop','--time','10',x['container_id']],cleanup,result['commands'],cap=20)
    ...
    try:
        ...
        await third.stop_intents(spec,cleanup,result,m)
    except BaseException as e:
        result['errors'].append('failure cleanup interrupted: '+repr(e))
        result['cleanup_interrupted']=True
        return e
    return None
...
async def launch(spec_path):
    ...
    require(spec['hostname']==socket.gethostname()==HOST and spec['operation']=='restore-two-create-third-after-model-scale','wrong restore host/operation')
    ...
    result=dict(started_s=time.time(),complete=False,measurement_valid=False,created=[],restarted=[],start_intents=[],creation_intents=[],stopped=[],
        commands=[],http_records=[],errors=[],model_main_release=release,predecessor_proof=[proof],
        new_containers_created=False,created_field_semantics='two restarted IDs and one newly created ID, separately recorded',
    ...
    async with aiohttp.ClientSession(trust_env=False) as session:
        ...
        try:
            ...
            stopped=verify_stopped(spec,inventory);third.preflight(spec,inventory);write(out/'containers.before.json',inventory)
        ...
        try:
            ...
            await third.create_and_prepare(spec,session,limit,result,common,sys.modules[__name__])
            verify_prefixes(result['runtime_archive']);verify_files(spec['files'])
            final=json.loads(await command(['docker','inspect',*spec['expected_containers'],spec['third_instance']['container_name']],limit,result['commands']))
            final_by={c['Name'].lstrip('/'):c for c in final}
            retained_final={n:final_by[n] for n in spec['expected_containers']}
            verify_restarted(spec,stopped,retained_final);third.verify_final(spec,result,final_by)
            ...
            require(set((await command(['docker','ps','--format','{{.Names}}'],limit,result['commands'])).split())==set(final_by),'unexpected running engine after recovery')
            write(out/'containers.after.json',final);result['created']=copy.deepcopy(result['restarted'])+[result['third_created']];result['complete']=True
            ...
            if not result['complete']:
                ...
                interrupted=await failure_cleanup(spec,cleanup,result,sys.modules[__name__])
                if interrupted is not None:failure=interrupted
            ...
            result.update(finished_s=time.time(),sampling_error=sampler.error,
                scope='measured two retained PDB restart plus third TP2 creation/peers/native only; no output correctness or performance qualification')
...
def main():
    ...
    if a.command=='prepare':result=prepare(a)
        ...
        with node_lease():
            ...
            result['bootstrap']=third.bootstrap(read(a.spec),a.spec)
