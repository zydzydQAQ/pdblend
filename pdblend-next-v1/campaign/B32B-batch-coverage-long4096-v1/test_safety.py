...
def test_exact_natural_spec_stays_six_long_points_no_temporal():
 a=old.arguments();assert a.input_patterns==[[4096]] and a.output_pattern==[256] and a.batches==[8]
...
def test_long_input_capacity_and_exact_six_points():
 args=old.arguments();assert args.input_patterns==[[4096]] and args.batches==[8] and args.repeats==3 and args.frequencies==[2520,1500]
 assert args.output_pattern==[256] and args.budgets==[8192]
 for free in (None,34815):
  with pytest.raises(RuntimeError):b.check_long_capacity(dict(instances=[dict(runtime=dict(free_kv_tokens=free))]))
 b.check_long_capacity(dict(instances=[dict(runtime=dict(free_kv_tokens=34816))]))
