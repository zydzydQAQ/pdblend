...
BARRIER=CAMPAIGN/'main-first-barrier-v1/barrier.py'
...
def plan(original,receipt,inventory,previous,out,release,release_sha):
    ...
    result.update(schema=2,out=str(Path(out).resolve()),layout='resident',operation='restart-retained-residents',
        expected_containers=expected,expected_provenance=provenance,global_main_release=str(Path(release).resolve()),
        global_main_release_sha256=release_sha,previous_phases=['main','scale'],pdb_phases=['main'],
...
def prepare(a):
    ...
    return dict(spec=str(a.out/'deployment.json'),sha256=sha(a.out/'deployment.json'),hardware_actions=False,
        runnable_only_after_real_global_release_and_fresh_lease=True,correctness_verified=False)
...
def validate_spec(spec):
    ...
    expected=plan(base,read(ORIGINAL/'deployment-receipt.json'),read(ORIGINAL/'containers.after.json'),previous,
        spec['out'],spec['global_main_release'],spec['global_main_release_sha256'])
...
async def launch(spec_path):
    ...
    release=barrier.verify_release(spec['global_main_release'],spec['global_main_release_sha256'])
    ...
    result=dict(started_s=time.time(),complete=False,measurement_valid=False,created=[],restarted=[],start_intents=[],stopped=[],
        commands=[],http_records=[],errors=[],global_main_release=release,predecessor_proof=[proof],
