...
R=ROOT.parents[1]
SOURCE=R/'campaign/B32B-baseline-sequence-v1/attempt-001/status.json'
...
class Observer:
    def __init__(self,out,publisher):
        ...
        self.state=dict(pid=os.getpid(),started_s=time.time(),complete=False,source=str(SOURCE),observed={},publications=[],errors=[],hardware_actions=False,service_configuration_writes=False)
        self.save()
    ...
    def tick(self,source):
        ...
        if source.get('phase') in ('finished','failed','stopped'):
