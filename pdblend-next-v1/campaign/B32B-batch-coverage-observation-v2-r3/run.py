...
def predecessor():
    ...
    prior=json.loads((base/'invocations/000001.json').read_text())
    require(prior.get('complete') is True and prior.get('phase')=='finished' and prior.get('checkpointed_cells')==1,
        'first long cell must finish native cleanup and checkpoint')
    require(prior.get('baseline_preservation_verified') is True and len(prior.get('cells',[]))==1
        and prior['cells'][0].get('cell_id')=='32b-alpaca-m1-s701'
        and prior['cells'][0].get('screen_valid') is True,'first long measurement not valid')
    handle=json.loads((ROOT.parent/'B32B-load-matrix-queue-v1/pid-handle.json').read_text())
    require(not Path('/proc/'+str(handle['pid'])).exists(),'prior queue process must exit and release its lease')
    return dict(status_path=str(base/'invocations/000001.json'),status_sha256=sha(base/'invocations/000001.json'),
        queue_handle=handle,checkpoint_sha256=sha(base/'checkpoints/0001-32b-alpaca-m1-s701.json'))
...
class Gate:
    def __init__(self):
        ...
        self.state = dict(complete=False, passed=False, phase='preflight', started_s=time.time(),
            scope='12 natural batch4/8 TP2 observations; no profile publication or KV certification',
    ...
    async def identity(self,label):
        ...
        if label=='before':
            self.state['frozen_identity_gate']=await asyncio.to_thread(self.b.frozen_check,True)
    ...
    async def work(self,hardware):
        ...
        require(code==0 and report.get('complete') is True and len(report.get('points',[]))==12,'natural batch observations failed or incomplete')
    ...
    async def run(self):
        ...
        async with aiohttp.ClientSession(trust_env=False) as self.session:
            try:
                ...
                await asyncio.wait_for(self.work(hardware),600)
...
if __name__=='__main__':
    ...
    check()
    if args.check:print(json.dumps(dict(package_valid=True,planned_points=12,gpu_executed=False)))
    else:
