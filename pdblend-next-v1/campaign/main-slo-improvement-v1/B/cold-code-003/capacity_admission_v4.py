"""Causal queue/SLO risk changes ranking; ordinary low-load energy policy stays exact."""
from dataclasses import replace
import time

def evaluate(planner,snapshot,pending,*,now=None,joint=True,enabled=False):
    now=time.time() if now is None else now
    original=planner.plan(snapshot,pending,now=now,joint=joint)
    if not enabled or not pending:return original,False
    oldest=min(pending,key=lambda r:(r.arrival_s,r.request_id))
    ...
    backlog=len(pending)>fresh
    remaining=pending[0].ttft_remaining(now)
...
def plan(planner,snapshot,pending,*,now=None,joint=True,enabled=False):
    return evaluate(planner,snapshot,pending,now=now,joint=joint,enabled=enabled)[0]
