...
class TransitionMeter:
    ...
    async def start(self):
        ...
        class Observed:
            ...
            def power_reading(self, gpu):
                reading = backend.power_reading(gpu)
                value = backend._nvml.nvmlDeviceGetMemoryInfo(backend._handle(gpu))
                memory.append(dict(at_s=time.time(), gpu=gpu, used_bytes=int(value.used)))
                return reading
        self.sampler = PowerSampler(range(8), interval=.02, backend=Observed(), sample_clocks=True)
        self.sampler.start()
    ...
    async def finish(self):
        ...
        save_raw(self.out, [], self.sampler.samples, self.sampler.utilization_samples,
                 power_source=self.sampler.power_source, power_metadata=self.sampler.power_metadata)
        durable(self.out / 'memory.json', self.memory)
        evidence = power_evidence(self.sampler.samples, self.sampler.power_source, self.sampler.power_metadata)
        window_error = None
        try:
            energy = trapezoid_energy(clip_power_window(self.sampler.samples, self.started_s, finished_s))
        except ValueError as exc:
            energy = None
            window_error = repr(exc)
        result = dict(measurement_valid=not self.sampler.error and not window_error and evidence['power_source_verified'],
                      ...
                      sampling_error=self.sampler.error,
                      window_error=window_error,
                      peak_memory_per_gpu_bytes={str(g): max([v['used_bytes'] for v in self.memory if v['gpu']==g] or [0])
                                                 for g in range(8)},
...
Mixed-only replicas intentionally have peers={}. This does not modify or drain
the existing serving engines, and does not claim cross-instance PD capability.
"""
    def __init__(self, controller, binding, inventory):
        ...
        require(self.oracles.get('schema') == 'capacity-correctness-oracles-v1'
                and self.oracles.get('model_source_identity') == binding['identity']
    ...
    def allocate(self, gpus):
        ...
        config = dict(copy.deepcopy(self.template), id=iid, tp=self.tp, port=port, kv_port=kv_port,
                      role='mixed', peers={}, retained_weights=None,
    ...
    async def start(self, instance, before, limit):
        ...
        args += ['-v', '/root/workspace:/root/workspace', '-v', '/root/workspace/models:/models:ro',
                 self.binding['image'], 'python3', '-m', self.binding['engine_module'], '--config', str(path)]
        ...
        require(row['Config']['Cmd'] == ['python3', '-m', self.binding['engine_module'], '--config', str(path)]
    ...
    async def verify(self, instance, limit):
        ...
        require(not engine_residual(raw, time.time()), 'new replica native resources not clean')
    ...
    async def verify_cancel(self, instance, limit):
        ...
        try:
            ...
            require(len(ranks) == instance['tp'] and {r.get('rank') for r in ranks} == set(range(instance['tp'])),
                    'native cancel must acknowledge each distinct TP rank')
    ...
    async def stop(self, instance, limit):
        ...
        if self.controller.backend.clocks:
            await self.controller.backend.clocks.park(instance['gpus'], dict(self.controller.backend.clocks.epochs))
