...
from capacity_executor import Inventory, PhysicalCapacityExecutor, check_lease, fixed, require, sha
...
class CapacityService:
    ...
    def demand(self, now):
        ...
        matches = [domain for domain in domains if all(
            r.input_tokens <= domain['max_input_tokens'] and r.ttft_s >= domain['min_ttft_s']
            and r.tpot_s >= domain['min_tpot_s'] and type(r.output_limit) is int
        ...
        domain_sha = matches[0]['sha256'] if len(matches) == 1 else '0'*64
