...
class TransitionMeter:
    ...
    async def finish(self):
        ...
        result = dict(measurement_valid=not self.sampler.error and evidence['power_source_verified'],
                      ...
                      peak_memory_per_gpu_bytes={str(g): max(v['used_bytes'] for v in self.memory if v['gpu']==g)
                                                 for g in range(8)},
...
"""
    ...
    async def start(self, instance, before, limit):
        ...
        args += ['-v', '/root/workspace:/root/workspace', '-v', '/root/workspace/models:/models:ro',
                 self.binding['image'], 'python3', self.binding['engine_entry'], '--config', str(path)]
        ...
        require(row['Config']['Cmd'] == ['python3', self.binding['engine_entry'], '--config', str(path)]
