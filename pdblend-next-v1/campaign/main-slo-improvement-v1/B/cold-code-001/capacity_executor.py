...
def check_lease(path='/root/workspace/pdblend/new-results/campaigns/node-experiment.lock'):
    ...
    for fd in Path('/proc/self/fd').iterdir():
        try:
            ...
            pass
    require(open_inode, 'capacity executor requires an inherited/open original node lease')
    ...
    require(any('FLOCK' in row and 'WRITE' in row and inode in row for row in locks),
            'original node lease is not held exclusively')
...
class Inventory:
    ...
    def event(self, kind, **payload):
        value = dict(kind=kind, at_s=time.time(), **payload)
    ...
    def intent(self, instance, action, transaction):
        ...
        self.value['known_instances'][instance['id']] = dict(old, **instance,
    ...
    async def _execute(self, operation, gpus, remove_id, budget_s, *, proposal=None,
        ...
        async with self.lock:
            ...
            if proposal is not None:
                require(await self.validate_proposal(proposal), 'expired/stale physical proposal')
