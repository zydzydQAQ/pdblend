...
async def current_clock_first_plan(controller, plan, request):
    ...
    async with clocks.lock, controller.state.lock:
        ...
        candidates=controller.planner.candidates(snapshot,request,now)
        ...
        event.update(allowed=True,chosen_frequency_mhz=chosen.frequencies[0].frequency_mhz,
        ...
        return replace(chosen,reason=('idle first admission preserves original measured target with all-TP idle-only throttle and deferred wakeup verification; '
