...
class ClockOwner:
    ...
    def require_covered_write(self,gpus,frequency,reason,bootstrap=None):
        ...
        if not proof.get('allowed'):
            event=dict(at_s=time.time(),gpus=list(gpus),rejected_target_mhz=frequency,
                retained_commands={g:self.applied.get(g) for g in gpus},reason=reason,
                proof=proof,measurement_confirmation=False)
            self.coverage_limits=(self.coverage_limits+[event])[-256:]
            raise RuntimeError('frequency coverage-limited: physical recovery unconfirmed; '+str(event))
                ...
                if pending and time.monotonic()>=deadline:
                    ...
                    for gpu in gpus:
                        if self.applied.get(gpu)!=2520:
                            self.require_covered_write(gpus,2520,'active settling failure fallback')
                            await loop.run_in_executor(self.pool,partial(self.hardware.set_clock,gpu,2520))
                            self.last_write_finished_s=time.time()
        ...
        async with self.lock:
            ...
            for gpu in tuple(self.deferred):
                ...
                for member in item['gpus']:
                    ...
                    self.require_covered_write(item['gpus'],2520,'deferred wakeup failure fallback')
                    await loop.run_in_executor(self.pool,partial(self.hardware.set_clock,member,2520))
                    self.last_write_finished_s=time.time()
