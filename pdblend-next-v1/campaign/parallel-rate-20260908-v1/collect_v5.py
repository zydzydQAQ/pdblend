...
def collect(out,series):
    ...
    code_sources={str(path):sha(path) for path in (Path(__file__),ROOT/'boundary_first_loss.py',ROOT/'inspect_exploration.py',AUDIT/'report.py',AUDIT/'protocol.py',AUDIT/'raw_metrics.py')}
    ...
    boundaries=boundary_evidence(originals,points)
    write_adaptive_plan(out,originals,points)
    (out/'boundaries.json').write_text(json.dumps(boundaries,ensure_ascii=False,indent=2)+'\n')
    figures(out,originals,points,series)
