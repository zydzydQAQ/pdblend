...
def next_boundary(observations):
    ...
    observed_loss=[Decimal(str(x['rate_rps'])) for x in observations if x.get('measurement_valid') and x.get('work_complete') and x['slo_attainment']<.90]
    end=min(observed_loss,default=None)
    ...
    answer=dict(implementation_id=next(iter(versions),None),jobs=[],
        ...
        first_loss_repeats_agree=(end in fail_rates) if end is not None else None,
        repeated_threshold_crossing_preserved=bool(end in uncertain) if end is not None else False)
    if end is not None:
