...
def load_audit():
    ...
    module.raw_metrics=raw_metrics_v2
    return module
...
def collect(out,series):
    ...
    code_sources={str(path):sha(path) for path in (Path(__file__),ROOT/'boundary_first_loss.py',ROOT/'inspect_exploration.py',ROOT/'report_versions_v5.py',ROOT/'inspect_baseline_v3.py',ROOT/'source_identity.py',ROOT/'source_identity_v2.py',ROOT/'audit_dynamic.py',ROOT/'raw_metrics_v2.py',ROOT/'report_scope.py',ROOT/'boundary_first_loss_v2.py',AUDIT/'report.py',AUDIT/'protocol.py',AUDIT/'raw_metrics.py')}
