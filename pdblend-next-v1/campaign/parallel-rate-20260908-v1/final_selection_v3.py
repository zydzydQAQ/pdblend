...
import final_selection_base_v3 as prior
from final_selection_base_v3 import checked, need, sha, read
...
def declared_a_cells(selection):
    ...
    for reference in a_declarations(selection):
        ...
        for row in declaration['cells']:
            need(row['model']=='14b' and row['cell_id'].startswith('parallel-rate-p7-'),
...
def preserve_identity(point,binding,config,checkpoint,selection):
    ...
    if point['model']=='14b':
        import source_identity_v3
        release=checked(spec['formal_release'])
        expected=source_identity_v3.identity(checked(release['binding']),point['dataset'])
