"""Package finished fixed-SLO work and existing historical observations."""
import argparse
...
def check_scope(result,historical):
    need(result.get('final_report_complete') is True,'selected rate comparison still has required gaps')
    need(historical.get('existing_history_archive_complete') is True
         and historical.get('new_historical_measurements_required') is False,
         'existing historical archive is incomplete or requires out-of-scope measurements')
    reference=historical['fixed_slo_scope']
    need(reference['sha256']==SCOPE_SHA and sha(reference['path'])==SCOPE_SHA,
         'fixed-SLO scope correction changed')
    scope=read(reference['path'])
    need(scope['historical_scale11_required_for_this_task'] is False
         and scope['historical_scale11_unmeasured'] is True
         and scope['historical_observed_archive_rows']==259,'historical scope differs')
    need(sum(x['verified_main'] for x in historical['models'])==450
         and sum(x['verified_scale'] for x in historical['models'])==259,
         'existing historical 450+259 raw coverage differs')
    outside=historical['historical_unmeasured_not_required_by_current_scope']
    need(len(outside)==11 and set(outside)=={x['cell_id'] for x in scope['excluded_historical_cells']}
         and historical['historical_scale_declared']==270
         and historical['historical_scale_observed']==259
         and historical['historical_scale_unmeasured']==11
         and historical['all_original_720_records_exactly_unchanged'] is True,
         'eleven unmeasured historical points were dropped or called completed')
    need(historical.get('historical_scientific_overlay_applied') is True
         ...
         'historical scientific correction has not been applied')
    overlay_ref=result['baseline_scientific_overlay']
    need(sha(overlay_ref['path'])==overlay_ref['sha256'],'final baseline overlay changed')
    overlay=read(overlay_ref['path'])
    old_ids={x['cell_id'] for x in historical['points']}
    expected={x['cell_id'] for x in overlay['engineering_quarantines']} & old_ids
    actual={x['cell_id'] for x in historical['scientific_comparison_overlay']['quarantined']}
    need(expected<=actual,'historical corrected view omits a final engineering quarantine')
    return reference
...
def package(final_report,historical_complete,history,out):
    ...
    scope_ref=check_scope(result,historical)
    ...
    with zipfile.ZipFile(archive,'x',compression=zipfile.ZIP_DEFLATED,compresslevel=6) as target:
        ...
        overview=('全部历史与本轮表格、figure归档\n\n'
            ...
            +'历史 450 主表 +259 已测 SLO-scale（另11个未测、不在本轮范围）：current/'+str(historical_complete.relative_to(ROOT))+'/README.md\n\n'
            ...
            +'历史原表保留原始数值；已诊断的工程失效点另有科学资格修正表，最终比较和图排除其胜负，旧快照可能尚未包含此修正。'
    ...
    (out/'manifest.json').write_text(json.dumps(dict(created_s=time.time(),
        code_sha256=sha(__file__),fixed_slo_scope=scope_ref,historical_unmeasured_outside_current_scope=11,final_report=dict(path=str(final_report),sha256=sha(final_report/'manifest.json')),
