...
import final_selected_baseline_v1 as inspect_baseline
import source_identity_v2 as source_identity
import audit_dynamic
import raw_metrics_v3
import report_scope
import final_selection_v2 as selection_rules
...
def collect(out,selection_path,selection_sha):
    ...
    series="selected-Cp4-Ap6-Bp4"
    if out.exists():raise FileExistsError(out)
    audit=load_audit();p=audit.p
    code_sources={str(path):sha(path) for path in (Path(__file__),ROOT/'boundary_first_loss.py',ROOT/'inspect_exploration.py',ROOT/'report_versions_v6.py',ROOT/'final_selected_baseline_v1.py',ROOT/'source_identity.py',ROOT/'source_identity_v2.py',ROOT/'audit_dynamic.py',ROOT/'raw_metrics_v3.py',ROOT/'raw_metrics_v2.py',ROOT/'report_scope.py',ROOT/'boundary_first_loss_v2.py',AUDIT/'report.py',AUDIT/'protocol.py',AUDIT/'raw_metrics.py')}
    code_sources.update({str(path):sha(path) for path in (ROOT/"collect_v13.py",ROOT/"final_selection_v1.py",ROOT/"final_selection_v2.py",ROOT/"final_completion_v1.py")})
    ...
    baseline_points,baseline_sources=inspect_baseline.collect(audit,ROOT,points,selection['baseline_declarations'],selection.get('baseline_continuations',[]))
    ...
    completion=completion_rules.assess(points,pairs,grid,baseline_points,boundaries)
    ...
    lines+=['','原450点、旧版本与失败尝试保留。零GPU锁冲突失败单列于results.json的attempts，不计为性能观测。',
        '本矩阵采用预声明的整模型版本：C/B实际p4关闭容量扩缩，A实际p6开启经资格验证的扩缩。按版本保留实际源码、画像、策略及每次重复；未完成队列仍明确显示缺口。',
