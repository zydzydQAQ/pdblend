...
screen=root/'explore-p4-alpaca-r18.75';s=json.loads((screen/'status.json').read_text())
assert s['pid']==886683 and s['phase']=='complete' and s['node_lease_held'] is False
assert not pathlib.Path('/proc/886683').exists() and not s['failed']
...
if __name__=='__main__':
    ...
    v=json.loads(p.stdout);path=HERE/('explore-alp18.75-terminal-verified-'+str(time.time_ns())+'.json')
