...
RELEASE=HERE/'new-rate-release-p4-strict/release.json'
...
def verify_release(path,digest):
    ...
    result=[]
    for name,count in [('screen-p4',12),('new-rate-pdb-p4',4)]:
...
def expected():
    ...
    result.update(schema=2,out=str(OUT),operation='restart-retained-residents',previous_binding=str(PREVIOUS),pdb_binding=str(PREVIOUS),
        ...
        required_predecessors=[dict(stage=str(HERE/'screen-p4'),count=12),dict(stage=str(HERE/'new-rate-pdb-p4'),count=4)],
