...
FLAGS = {'admission_round_fairness': True, 'pending_admission_capacity_guard': True,
         'measured_frequency_write_guard_v1': True, 'observed_idle_admission_frequency_v2': True, 'unconfirmed_retained_admission_deferral_v1': True}
...
def prepare(model, binding_path, host, qualification, cpu, out, profile=None):
    ...
    for path in (host / 'manifest.json', p.ROOT / 'protocol.py', p.ROOT / 'runner.py',
                 ...
                 p.REPO / 'campaign/parallel-rate-20260908-v1/common/execution-until-complete-v1/run.py',
                 p.REPO / 'campaign/parallel-rate-20260908-v1/common/execution-until-complete-v1/child.py',
                 p.REPO / 'campaign/pdblend-ablation-20260908-v1/execution.py',
                 p.REPO / 'campaign/parallel-rate-20260908-v1/common/execution-until-complete-v1/manifest.json',
                 p.REPO / 'campaign/parallel-rate-20260908-v1/common/execution-until-complete-v1/cpu-validation.json'):
    ...
    work=p.read(p.ROOT/'work-declaration.json')
    ...
    release = dict(schema='main-slo-improvement-release-v1', approved=True, model=model,
        created_s=time.time(), deadline_s=None, campaign_lifecycle='until_declared_complete_v1', implementation_id=host.name,
