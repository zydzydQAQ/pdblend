...
spec=read(HERE/'baseline-boundary-restore-p4v2/deployment.json');decl=read(HERE/'boundary-p4v2/declaration.json')
files=dict(spec['files']);files.update(decl['files'])
for path in (HERE/'baseline-strategy-specs-p4v2').glob('*.json'):
 strategy=read(path);files.update(strategy['files']);files[str(path)]=sha(path)
for name in ['boundary-p4v2/declaration.json','baseline-boundary-restore-p4v2/deployment.json','baseline-boundary-restore-p4v2/adapter-manifest.json',
 'boundary_baseline_queue_p4v2.py','run_boundary_baseline_p4v2.py','prepare_baseline_cooperative_v2.py',Path(__file__).name]:
 p=HERE/name;files[str(p)]=sha(p)
assert all(sha(p)==h for p,h in files.items())
runner=(HERE/'run_boundary_baseline_p4v2.py').read_text();queue=(HERE/'boundary_baseline_queue_p4v2.py').read_text()
...
for p in [HERE/'run_boundary_baseline_p4v2.py',HERE/'boundary_baseline_queue_p4v2.py']:compile(p.read_text(),str(p),'exec')
...
p=HERE/'boundary-baseline-package-p4v2.json'
