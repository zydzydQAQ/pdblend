...
ROOT=C/'after-gamma-handoff-001'
RESTORE=C/'baseline-after-gamma-restore-002'
CODE=C/'boundary-continuation-p4v2-003'
GATE=C/'boundary-baseline-gate-after-gamma-001'
BOOT=C/'boundary-baseline-bootstrap-after-gamma-001'
...
def validate():
 m=read(C/'after-gamma-code-manifest-001.json');assert all(sha(p)==h for p,h in m['files'].items()),'handoff source changed'
...
def main():
 ...
 try:
  ...
  for root in (CODE,RESTORE,GATE,BOOT,C/'baseline-strategy-specs-after-gamma-001'):
   files.update({str(f):sha(f) for f in root.rglob('*') if f.is_file()})
  for path in (C/'after-gamma-code-manifest-001.json',C/'run_boundary_baseline_after_gamma_v1.py'):files[str(path)]=sha(path)
