...
OUT=HERE/'baseline-after-gamma-restore-001'
PREVIOUS=OUT/'empty-node-binding.json'
PAUSE=HERE/'gamma-priority-pause-001/receipt.json'
...
def pause_check():
 ...
 status=m.read(next(p for p in receipt['files'] if p.endswith('/owner-status.json')))
