...
def contract(args):
    ...
    require(Path(binding['deployment_receipt'])==HERE/'baseline-restore-p3/deployment-receipt.json'
        and Path(binding['correctness_evidence'])==HERE/'baseline-gate-p3','old gate/restoration cannot qualify this attempt')
    ...
    require(len(cells)==4 and {(r['rate_rps'],r['repeat']) for r in cells}=={(r,n) for r in (2.,2.5) for n in (1,2)},'declared 2x2 coverage required')
    for c in cells:
        require(sha(c['trace_path'])==c['trace_sha256'],'new trace changed')
        c['cell_id']=c['cell_id'].replace('parallel-rate-p1-new-','parallel-rate-p3-new-')
