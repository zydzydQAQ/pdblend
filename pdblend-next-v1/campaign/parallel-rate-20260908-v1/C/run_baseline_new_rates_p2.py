...
def contract(args):
    ...
    require(Path(binding['deployment_receipt'])==HERE/'baseline-restore-p2/deployment-receipt.json'
        and Path(binding['correctness_evidence'])==HERE/'baseline-gate-p2','old gate/restoration cannot qualify this attempt')
    ...
    for c in cells:
        ...
        c['cell_id']=c['cell_id'].replace('parallel-rate-p1-new-','parallel-rate-p2-new-')
