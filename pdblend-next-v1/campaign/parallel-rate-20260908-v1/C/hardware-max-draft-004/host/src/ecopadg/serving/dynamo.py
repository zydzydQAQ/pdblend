...
class DynamoScheduler:
    ...
    def __init__(self, profiles, assignments, *, costs=(), input_cuts=(255, 1023), output_cuts=(99, 349), clock_settle_s=0.3, frequency_costs=(), max_service_frequency_mhz=2520):
        ...
        self.estimator = JointPlanner(profiles, allow_pd=False, dvfs=True, clock_settle_s=clock_settle_s, frequency_costs=frequency_costs, max_frequency=getattr(self, 'max_service_frequency_mhz', 2520))
    ...
    def forecast(self, now, window=300, bucket=10):
        ...
        for (shape, history) in self.history.items():
            ...
            result[shape] = dict(rate=max(bins.values()) / bucket, input_tokens=max((r.input_tokens for r in samples)), output_tokens=max((r.predicted_output for r in samples)), ttft_s=min((r.ttft_s for r in samples)), tpot_s=min((r.tpot_s for r in samples)))
        return result

    def plan(self, snapshot, pending, *, now):
        request = pending[0]
        shape = self.hierarchy.classify(request.input_tokens, request.predicted_output)
        eligible = [i for i in snapshot.instances if i.role == 'mixed' and dominates(self.assignments.get(i.instance_id, 'SS'), shape)]
        ...
        candidates = [p for p in full if all((a.frequency_mhz == frequencies[a.instance_id] for a in p.frequencies))]
        if not candidates:
            return ControlPlan(snapshot.version, now, now + 1, feasible=False, frequencies=tuple((FrequencyAction(i.instance_id, getattr(self, 'max_service_frequency_mhz', 2520)) for i in eligible)), reason='DynamoLLM emergency: larger pools full, unsafe or unprofiled')
    ...
    def frequency_plan(self, snapshot, now):
        ...
        for instance in snapshot.instances:
            ...
            for frequency in self.profiles.frequencies('mixed', instance.tp):
                ...
                if p is None:
                    continue
                pending = [self.profiles.lookup('mixed', instance.tp, frequency, r.input_tokens, r.input_tokens + 1, 1) for r in requests if not r.emitted]
                if not all(pending):
                    continue
                prefill = sum((q.phase_time_bound('prefill') for q in pending))
                switches = [c for c in self.frequency_costs if c.tp == instance.tp and (c.source_mhz, c.target_mhz) == (instance.frequency_mhz, frequency)]
                settling = max((c.duration_upper_s for c in switches), default=self.estimator.clock_settle_s if frequency > instance.frequency_mhz else 0)
                latency = p.iteration_s * p.bound
                if now - instance.timestamp_s > 1 or any((latency > r.tpot_s or r.next_token_remaining(now) < latency + prefill + settling for r in requests)):
                    continue
                ...
                energy += max((c.energy_upper_j for c in switches), default=0.0)
                candidates.append((energy, frequency))
            chosen = min(candidates)[1] if candidates and instance.dvfs_allowed else getattr(self, 'max_service_frequency_mhz', 2520)
            actions.append(FrequencyAction(instance.instance_id, chosen))
        return ControlPlan(snapshot.version, now, now + 1, frequencies=tuple(actions), reason='DynamoLLM ScaleFreq five-second epoch')
    ...
    def resident_reassignment(self, snapshot, now):
        ...
        idle = [i for i in snapshot.instances if not i.requests and (not i.running) and (not i.waiting) and i.accepting and (now - i.timestamp_s <= 1)]
        ...
        for shape in SHAPES:
            ...
            if leftover:
                demand = dict(leftover) if not demand else dict(rate=demand['rate'] + leftover['rate'], input_tokens=max(demand['input_tokens'], leftover['input_tokens']), output_tokens=max(demand['output_tokens'], leftover['output_tokens']), ttft_s=min(demand['ttft_s'], leftover['ttft_s']), tpot_s=min(demand['tpot_s'], leftover['tpot_s']))
            if not demand:
                continue
            usable = []
            for instance in idle:
                points = [p for p in self.profiles.points if p.role == 'mixed' and p.tp == instance.tp and (p.frequency_mhz == getattr(self, 'max_service_frequency_mhz', 2520)) and (p.input_tokens >= demand['input_tokens']) and (p.context_tokens >= demand['input_tokens'] + demand['output_tokens']) and (p.phase_time_bound('prefill') <= demand['ttft_s']) and (p.iteration_s * p.bound <= demand['tpot_s'])]
                if points:
            ...
            if rate and larger:
                ...
                if previous:
                    carry[larger] = dict(rate=rate + previous['rate'], input_tokens=max(demand['input_tokens'], previous['input_tokens']), output_tokens=max(demand['output_tokens'], previous['output_tokens']), ttft_s=min(demand['ttft_s'], previous['ttft_s']), tpot_s=min(demand['tpot_s'], previous['tpot_s']))
        for instance in idle:
