...
screen=root/'screen-p2';s=json.loads((screen/'status.json').read_text())
assert s['pid']==793448 and s['phase']=='stopped_at_boundary' and s['node_lease_held'] is False
assert not pathlib.Path('/proc/793448').exists() and not s['failed']
...
host='/root/workspace/pdblend-next-v1/campaign/parallel-rate-20260908-v1/hosts/7b-fixed-p2'
...
if __name__=='__main__':
    ...
    v=json.loads(p.stdout);path=HERE/('p2-terminal-verified-'+str(time.time_ns())+'.json')
