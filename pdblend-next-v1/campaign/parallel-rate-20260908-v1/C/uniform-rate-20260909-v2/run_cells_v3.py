...
async def execute(reference, out, state):
    ...
    import aiohttp
    import audit_cell
    independent_cell_audit = p.load(release['cell_auditor'], 'uniform_v2_diagnosed_cell_audit') if release.get('cell_auditor') else audit_cell
    with node_lease():
        ...
        async with aiohttp.ClientSession(trust_env=False) as session:
            for row in release['rows']:
                ...
                if receipt_path.exists():
                    ...
                    if error is None:
                        try:
                            observation = independent_cell_audit.audit(p.ref(cp_path))
                            ...
                            if service_failure:
                                ...
                                rejection = observation['baseline_service_failure'].get('classification') == 'baseline_explicit_controller_admission_queue_full'
                                p.save(diagnosis_path, dict(schema=('uniform-baseline-capacity-rejection-diagnosis-v1' if rejection else 'uniform-baseline-deadline-diagnosis-v1'),
                                ...
                                observation.update(failure_class=('independently_diagnosed_capacity_rejection' if rejection else 'independently_diagnosed_capacity_deadline'),
