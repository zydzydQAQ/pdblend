    ...
    for system in plan['system_order']:
        ...
        for dataset in plan['datasets']:
            while True:
                ...
                await child([sys.executable, '-B', str(Path(__file__).parent / 'prepare_request_v3.py'), '--request',
                ...
                await child([sys.executable, '-B', str(p.HERE / 'run_cells.py'), '--release', ref['path'],
...
def main():
    ...
    if plan.get('previous_pipeline'):
        ...
        if previous.get('error') not in (None, "ValueError('stop requested at cell boundary')"):
            diagnosis = p.checked(plan['resume_diagnosis'])
            p.need(diagnosis['predecessor'] == plan['previous_pipeline'] and diagnosis['kind'] == 'pre_measurement_configuration_gap', 'unknown previous failure')
            failed = Path(diagnosis['failed_attempt'])
            p.need(not (failed / 'measurement').exists() and not (failed / 'release/release.json').exists(), 'a performance attempt already exists')
            p.need(p.ref(failed / 'prepare.log') == diagnosis['prepare_failure_log'], 'CPU failure evidence changed')
            p.need(previous['child']['exitcode'] == 1 and 'prepare_request_v3.py' in previous['child']['argv'][2], 'previous child was not a CPU preparation')
        p.need(plan['initial_observations'] == previous['observations'], 'resume must preserve every prior observation')
        p.need(plan['last_cell_status'] == previous['last_cell_status'], 'resume must follow actual last cell')
    if not args.run:
    ...
    state = dict(schema='uniform-v2-five-system-pipeline-status', plan=plan_ref, pid=os.getpid(),
        ...
        model=plan['model'], datasets=plan['datasets'], declaration=plan['declaration'], scope='five_systems',
