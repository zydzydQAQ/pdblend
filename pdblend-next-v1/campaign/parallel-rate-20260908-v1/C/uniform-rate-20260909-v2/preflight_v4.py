...
import support as p
contract = p.load(U / 'contract_capacity_v1.py', 'C_preflight_capacity_contract')


def main():
    plan_ref = p.ref(U / 'plan-004.json')
    ...
    for dataset in plan['datasets']:
        ...
        for task in decision['baseline_tasks']:
            ...
            kwargs = dict(declaration=plan['declaration'], qualification=h['qualification'],
                ...
                stop_paths=plan['stop_paths'], extra_files=[plan_ref, p.ref(__file__), p.ref(U / 'pipeline_v5.py')],
    ...
    def run(item):
        ...
        with (dest / 'prepare.log').open('xb') as stream:
            proc = subprocess.run([sys.executable, '-B', str(U / 'prepare_request_v4.py'),
