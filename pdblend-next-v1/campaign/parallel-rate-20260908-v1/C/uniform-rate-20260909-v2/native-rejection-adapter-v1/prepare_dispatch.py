...
import sys
sys.path.insert(0, '/root/workspace/pdblend-next-v1/campaign/parallel-rate-20260908-v1/common/uniform-rate-20260909-v2')
import support as p


def prepare(*, declaration, qualification, qualification_validator, node, model, dataset,
            rate, system, out, scheduling_observations=(), predecessors=(), repeats=(1,),
            measurement_executor=None, extra_files=(), stop_paths=(), dynamic_reuse_observations=(), measurement_purpose='normal', cell_auditor=None, cell_auditor_dependencies=()):
    ...
    contract_ref = p.ref(Path(__file__).parent / 'contract.py')
    contract = p.load(contract_ref, 'uniform_prepare_contract')
    rows = [contract.lookup(declaration, model, dataset, rate, system, rep, purpose=measurement_purpose) for rep in repeats]
    ...
    # C's unchanged native queue refusal needs only a structured-ref reader adapter.
    selected_metrics = Path(__file__).parent / 'metrics.py' if node == 'C' and system == 'ecoserve' else p.HERE / 'metrics.py'
    if node == 'C' and system == 'ecoserve':
        p.need(cell_auditor is None or cell_auditor == p.ref(Path(__file__).parent / 'cell_audit.py'), 'unexpected Eco capacity auditor')
        cell_auditor = p.ref(Path(__file__).parent / 'cell_audit.py')
        helper = p.load(Path(__file__).parent / 'native_queue.py', 'uniform_native_release_dependencies')
        cell_auditor_dependencies = tuple(cell_auditor_dependencies) + tuple(
            dict(path=path, sha256=digest) for path, digest in helper.IMPORTED_SOURCES.items())
        cell_auditor_dependencies += tuple(helper.__dict__[name] for name in ('ORIGINAL', 'RAW', 'TIMING', 'ENGINE', 'IDENTITY'))
        cell_auditor_dependencies += tuple(p.ref(Path(__file__).parent / name) for name in ('native_queue.py', 'cell_audit.py'))
        cell_auditor_dependencies += (p.ref(p.HERE / 'audit_cell_capacity_v3.py'),)
    references = dict(declaration=declaration, declaration_contract=contract_ref,
        ...
        measurement_auditor=p.ref(selected_metrics))
    if cell_auditor:
        references['cell_auditor'] = cell_auditor
    ...
    own_sources = [p.HERE / name for name in ('support.py', 'prepare_release.py', 'run_cells.py', 'audit_cell.py',
                                           'prepare_request.py', 'validate_handoff.py')]
    own_sources += [p.HERE / 'prepare_dispatch_v3.py',
        p.ROOT / 'common/token-evidence-v2/audit_native_queue_v3.py',
        p.ROOT / 'common/token-evidence-v2/verify_native_raw_v3.py']
    ...
    own_sources += [Path(__file__), Path(__file__).parent / 'run_cells.py']
    for path in own_sources:
        files[str(path)] = p.sha(path)
    for reference in [*references.values(), *scheduling_observations, *dynamic_reuse_observations, *predecessors, *extra_files, *cell_auditor_dependencies]:
    ...
    release = dict(schema='uniform-rate-cells-release-v2', created_s=time.time(), node=node,
    ...
    run_cells = p.load(Path(__file__).parent / 'run_cells.py', 'uniform_C_v3_cells')
...
def main():
    ...
    parser.add_argument('--measurement-purpose', choices=('normal', 'metric_supplement'), default='normal')
    ...
    print(json.dumps(prepare(declaration=p.ref(args.declaration), qualification=p.ref(args.qualification),
        ...
        repeats=tuple(args.repeat or (1,)), measurement_purpose=args.measurement_purpose,
