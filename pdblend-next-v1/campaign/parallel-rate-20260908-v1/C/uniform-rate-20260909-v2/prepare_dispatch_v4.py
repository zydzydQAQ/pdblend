...
def prepare(*, declaration, qualification, qualification_validator, node, model, dataset,
    ...
    contract_ref = p.ref(Path(__file__).parent / 'contract_capacity_v1.py')
    ...
    selected_metrics = p.HERE / ('metrics_native_refs_v3.py' if node == 'C' and system == 'ecoserve' else 'metrics.py')
    ...
    own_sources += [Path(__file__), Path(__file__).parent / 'run_cells_v3.py']
    ...
    run_cells = p.load(Path(__file__).parent / 'run_cells_v3.py', 'uniform_C_v3_cells')
