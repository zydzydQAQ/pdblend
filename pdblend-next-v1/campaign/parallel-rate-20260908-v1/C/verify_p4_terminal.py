...
screen=root/'screen-p4';s=json.loads((screen/'status.json').read_text())
assert s['pid']==829075 and s['phase']=='stopped_at_boundary' and s['node_lease_held'] is False
assert not pathlib.Path('/proc/829075').exists() and not s['failed']
...
if __name__=='__main__':
    ...
    v=json.loads(p.stdout);path=HERE/('p4-terminal-verified-'+str(time.time_ns())+'.json')
