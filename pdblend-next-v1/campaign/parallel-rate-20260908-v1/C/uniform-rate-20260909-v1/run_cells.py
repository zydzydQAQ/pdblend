...
def validate_rows(release, binding, contract):
    ...
    for row in rows:
        expected = contract.lookup(release['declaration'], row['model'], row['dataset'],
                                   row['rate_rps'], row['system'], row['repeat'])
...
def load_release(reference):
    ...
    p.need(release['schema'] == 'uniform-rate-cells-release-v1', 'unknown release')
        ...
        async with aiohttp.ClientSession(trust_env=False) as session:
            for row in release['rows']:
                ...
                if receipt_path.exists():
                    ...
                    cp = dict(schema='uniform-rate-checkpoint-v1', row=row, repeat=row['repeat'], node=release['node'],
...
def main():
    ...
    state = dict(schema='uniform-rate-cell-status-v1', pid=os.getpid(),
