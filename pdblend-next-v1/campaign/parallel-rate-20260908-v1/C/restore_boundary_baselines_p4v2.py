...
PREVIOUS=HERE/'baseline-boundary-restore-p4v2/previous-binding.json'
RELEASE=HERE/'boundary-p4v2/declaration.json'
OUT=HERE/'baseline-boundary-restore-p4v2'
...
def verify_release(path,digest):
    ...
    result=[]
    for stage in release['pdblend_stages']:
        name,count,expected_phase=stage['name'],stage['count'],stage['phase']
        root=HERE/name;status=read(root/'status.json')
        require(status.get('phase')==expected_phase and not status.get('engineering_gate_failed')
        ...
        for p in cps:
            ...
            require(g.get('passed') is True and not g.get('http503'),'PDB frequency/request failure blocks baseline restoration')
            for file,h in c['artifacts'].items():require(sha(file)==h,'PDB predecessor raw artifact changed')
            require(r['summary'].get('work_complete') is True and r['summary'].get('failed_requests')==0 and r['summary'].get('request_timeouts')==0,'incomplete predecessor blocks baseline expansion')
...
def expected():
    ...
    result.update(schema=2,out=str(OUT),operation='restart-retained-residents',previous_binding=str(PREVIOUS),pdb_binding=str(PREVIOUS),
        ...
        deadline_s=None,campaign_lifecycle='until_declared_complete_v1',executor_release=str(COMMON.parent),
        baseline_source_strategy='correctness-only',baseline_controller_hosts={'mixed': '/root/workspace/pdblend-next-v1/releases/five-system100-C7B-baseline-v1-runtime', 'distserve': '/root/workspace/pdblend-next-v1/releases/five-system100-C7B-baseline-v1-runtime', 'dynamollm': '/root/workspace/pdblend-next-v1/releases/five-system100-C7B-baseline-cooperative-v1-runtime', 'ecoserve': '/root/workspace/pdblend-next-v1/releases/five-system100-C7B-baseline-v1-runtime'},
        expected_containers=expected_containers,expected_provenance=expected_provenance,
        required_predecessors=read(RELEASE)['pdblend_stages'],
...
def bind_parent(spec):
    ...
    parent.HOST=HOST;parent.DEADLINE=float('inf');parent.COMMON=COMMON;parent.package_check=package_check;parent.validate_spec=validate_spec
...
def main():
    ...
    if args.prepare:
        ...
        OUT.mkdir()
        previous=read(CAMPAIGN/'main-rate-rerun-v1/C-attempt-001/binding.json');previous['deadline_s']=None;previous['campaign_lifecycle']='until_declared_complete_v1'
        common_manifest=read(COMMON.parent/'manifest.json');previous['files'].update(common_manifest['files'])
        for name in ('manifest.json','cpu-validation.json'):previous['files'][str(COMMON.parent/name)]=sha(COMMON.parent/name)
        ...
        lifecycle=read(HERE/'baseline-until-complete-v2/manifest.json');dependencies.update(lifecycle['files']);dependencies.update(lifecycle['references'])
        dependencies[str(HERE/'baseline-until-complete-v2/manifest.json')]=sha(HERE/'baseline-until-complete-v2/manifest.json')
