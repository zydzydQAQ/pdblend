"""Read-only original B Dynamo arrival-fidelity and controller-stall screen."""
import csv
import hashlib
import json
...
def main():
    ...
    selected=[row for row in protocol.original_points() if row['model']=='32b' and row['system']=='dynamollm']
    ...
    for old in selected:
        ...
        result=dict(cell_id=old['cell_id'],dataset=old['dataset'],rate_rps=old['rate_rps'],
    ...
    destination=ROOT/'C/B-original-Dynamo30-arrival-audit-v1.json'
    assert not destination.exists()
    destination.write_text(json.dumps(dict(schema='B-original-Dynamo30-arrival-readonly-screen-v1',created_s=time.time(),
