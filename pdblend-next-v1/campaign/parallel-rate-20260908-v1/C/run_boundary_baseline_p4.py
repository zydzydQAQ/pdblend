...
DECLARATION=HERE/'boundary-p4/declaration.json'
...
def contract(args):
    ...
    require(Path(binding['deployment_receipt'])==HERE/'baseline-boundary-restore-p4/deployment-receipt.json'
        and Path(binding['correctness_evidence'])==HERE/'boundary-baseline-gate-p4','old gate/restoration cannot qualify this attempt')
    ...
    for c in cells:
        ...
        c['cell_id']=c['cell_id'].replace('parallel-rate-p4-explore-','parallel-rate-p4-boundary-')
