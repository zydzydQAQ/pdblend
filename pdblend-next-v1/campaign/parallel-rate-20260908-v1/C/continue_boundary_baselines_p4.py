...
HERE=Path(__file__).resolve().parent;OUT=HERE/'boundary-local-handoff-p4'
PACKAGE=HERE/'boundary-baseline-package-p4.json'
EXPECTED='19c7d5dd8c8cf327d22483841c3710585b961ded05cea8312ddb667ea4ba669f'
...
try:
 while True:
  ...
  if not v['alive']:
   ...
   break
  time.sleep(15)
 state['phase']='restore';state['restore']=launch(HERE/'restore_boundary_baselines_p4.py','boundary-retained-restore-p4');save()
 while True:
  v=snapshot(HERE/'baseline-boundary-restore-p4/deployment-receipt.json',state['restore']['pid']);state['restore_observation']=v;save()
  if not v['alive']:break
  time.sleep(15)
 r=v['status'];mirror_terminal_tree(HERE/'baseline-boundary-restore-p4')
 ...
 check();state['phase']='baseline-queue';state['queue']=launch(HERE/'boundary_baseline_queue_p4.py','boundary-baseline-queue-p4');save()
 while True:
  v=snapshot(HERE/'boundary-baseline-queue-p4/status.json',state['queue']['pid']);state['queue_observation']=v;save()
  if not v['alive']:break
  time.sleep(15)
 q=v['status'];mirror_terminal_tree(HERE/'boundary-baseline-queue-p4')
 for name in ['boundary-baseline-bootstrap-p4','boundary-baseline-gate-p4','boundary-baseline-bindings-p4']:
