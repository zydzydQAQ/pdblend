...
HERE=Path('/root/workspace/pdblend-next-v1/campaign/parallel-rate-20260908-v1/C')
CAMPAIGN=HERE.parents[1]
DECLARATION=HERE/'eco-drain37-v1/declaration.json'
PACKAGE=HERE/'eco-drain37-continuation-001/package.json'
...
def contract(args):
    ...
    require(Path(binding['deployment_receipt'])==HERE/'baseline-after-gamma-restore-003/deployment-receipt.json'
        and Path(binding['correctness_evidence'])==HERE/'eco-drain37-v1/gate','old gate/restoration cannot qualify this attempt')
    ...
    require(len(cells)==37 and len({c['cell_id'] for c in cells})==37,'declared unique boundary endpoints required')
    for c in cells:
    ...
    continuation_path=HERE/'eco-drain37-continuation-001/continuation.json'
    ...
    with node_lease():
        ...
        async with aiohttp.ClientSession(trust_env=False) as session:
            ...
            for row in cells:
                ...
                write(output/'checkpoints'/(row['cell_id']+'.json'),dict(row=row,declaration=dict(path=str(DECLARATION),sha256=DECLARATION_SHA),binding=str(bp),binding_sha256=sha(bp),receipt=str(rp),receipt_sha256=sha(rp),
                ...
                checkpoint=output/'checkpoints'/(row['cell_id']+'.json')
                if gate['passed']:
