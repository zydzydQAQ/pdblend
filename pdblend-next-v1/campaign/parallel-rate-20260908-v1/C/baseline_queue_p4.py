...
OUT=HERE/'baseline-queue-p4'
SPEC=HERE/'baseline-restore-p4/deployment.json'
...
class Queue:
    ...
    def boundary(self,reserve):
        ...
        manifest=read(HERE/'baseline-queue-package-p4.json')
    ...
    def run(self):
        try:
            ...
            bootstrap=HERE/'baseline-bootstrap-p4';binder=CAMPAIGN/'AC-baseline-binding-v2/bind.py'
            ...
            gate=HERE/'baseline-gate-p4';entry=CAMPAIGN/'AC-legacy-resident-correctness-v1/validate.py'
            ...
            for repeat in (1,2):
                for system in ('mixed','distserve','dynamollm','ecoserve'):
                    ...
                    out=HERE/'new-rate-baselines-p4'/system/('repeat'+str(repeat))
                    binding=HERE/'baseline-bindings-p4'/name
                    ...
                    self.command(name+'-measure',[sys.executable,'-B',str(HERE/'run_baseline_new_rates_p4_repeats.py'),
...
def main():
    ...
    if not a.run:
        m=read(HERE/'baseline-queue-package-p4.json')
