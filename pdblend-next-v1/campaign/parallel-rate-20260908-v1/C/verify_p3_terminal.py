...
screen=root/'screen-p3';s=json.loads((screen/'status.json').read_text())
assert s['pid']==800614 and s['phase']=='engineering_gate_failed' and s['node_lease_held'] is False
assert not pathlib.Path('/proc/800614').exists() and not s['failed']
...
host='/root/workspace/pdblend-next-v1/campaign/parallel-rate-20260908-v1/hosts/7b-fixed-p3'
...
if __name__=='__main__':
    ...
    v=json.loads(p.stdout);path=HERE/('p3-terminal-verified-'+str(time.time_ns())+'.json')
