...
screen=root/'screen-p1';s=json.loads((screen/'status.json').read_text())
assert s['pid']==790589 and s['phase']=='engineering_gate_failed' and s['node_lease_held'] is False
assert not pathlib.Path('/proc/790589').exists() and not s['failed']
...
if __name__=='__main__':
    ...
    v=json.loads(p.stdout);path=HERE/('p1-terminal-verified-'+str(time.time_ns())+'.json')
