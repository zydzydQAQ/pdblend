...
package=HERE/'boundary-baseline-package-p4v2.json';files=json.loads(package.read_text())['files'];files[str(package)]=sha(package)
...
p=HERE/('boundary-stage-p4v2-'+str(time.time_ns())+'.json');write_new(p,json.dumps(dict(captured_s=time.time(),package=str(package),files=files,inspection=audit,verified=verify),indent=2).encode())
