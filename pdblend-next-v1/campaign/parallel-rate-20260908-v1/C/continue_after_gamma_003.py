...
ROOT=C/'after-gamma-handoff-003'
...
def validate():
 m=read(C/'after-gamma-code-manifest-003.json');assert all(sha(p)==h for p,h in m['files'].items()),'handoff source changed'
...
def main():
 ...
 try:
  ...
  closure=read(C/'restore003-closure-001/receipt.json');assert closure['passed'] and closure['restore_receipt']==ref(RESTORE/'deployment-receipt.json') and closure['restore_spec']==ref(RESTORE/'deployment.json') and closure['node_lease_released'] and closure['all_original_8_idle_same_PID_StartedAt']
  for reference in (closure['identity'],closure['source'],closure['common_source'],closure['log']):assert sha(reference['path'])==reference['sha256']
  assert closure['original_artifacts']=={p:h for p,h in receipt['artifacts'].items() if p!=str(RESTORE/'run.log')}
  assert all(sha(p)==h for p,h in closure['original_artifacts'].items()),'restoration raw evidence changed'
  from close_restore003_stdout_v1 import check_log
  assert check_log((RESTORE/'run.log').read_bytes(),receipt['artifacts'][str(RESTORE/'run.log')])==closure['stdout_append_proof']
  ...
  for path in (C/'after-gamma-code-manifest-003.json',C/'run_boundary_baseline_after_gamma_v2.py'):files[str(path)]=sha(path)
