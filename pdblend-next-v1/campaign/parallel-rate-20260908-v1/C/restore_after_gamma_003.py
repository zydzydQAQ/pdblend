...
OUT=HERE/'baseline-after-gamma-restore-003'
...
from startup_ports_guard_v1 import Guard
...
def prepare():
 ...
 deps=dict(m.read(HERE/'baseline-boundary-restore-p4v2/adapter-manifest.json')['files']);deps[str(Path(__file__).resolve())]=m.sha(__file__);deps[str(PAUSE)]=m.sha(PAUSE);deps.update(previous['files']);deps[str(HERE/'startup_ports_guard_v1.py')]=m.sha(HERE/'startup_ports_guard_v1.py');deps[str(HERE/'baseline-after-gamma-restore-002/deployment-receipt.json')]=m.sha(HERE/'baseline-after-gamma-restore-002/deployment-receipt.json');deps[str(HERE/'baseline-after-gamma-restore-002/stop-known-startup-failure.json')]=m.sha(HERE/'baseline-after-gamma-restore-002/stop-known-startup-failure.json');deps[str(HERE/'gamma-priority-pause-001/containers.before.json')]=m.sha(HERE/'gamma-priority-pause-001/containers.before.json')
...
def main():
 ...
 with node_lease():
  ports={i[k] for i in spec['instances'] for k in ('port','kv_port')};guard=Guard(ports)
  try:
   with guard:r=asyncio.run(run())
  finally:m.write(OUT/'startup-port-reservation.json',guard.state)
 m.require(guard.state['complete'] and guard.state['restored'],'startup port reservations not exactly restored')
