...
OUT=HERE/'boundary-baseline-queue-p4v2'
SPEC=HERE/'baseline-boundary-restore-p4v2/deployment.json'
...
class Queue:
    ...
    def boundary(self,reserve):
        ...
        manifest=read(HERE/'boundary-baseline-package-p4v2.json')
        for p,h in manifest['files'].items():require(sha(p)==h,'baseline queue input changed: '+p)
    def command(self,name,argv,reserve=400,max_work_s=None,performance_out=None,controller_host=None):
        self.boundary(reserve)
        spec=read(SPEC);host=Path(controller_host or spec['host_release'])
    ...
    def run(self):
        try:
            ...
            bootstrap=HERE/'boundary-baseline-bootstrap-p4v2';binder=HERE/'baseline-until-complete-v2/bind.py'
            ...
            gate=HERE/'boundary-baseline-gate-p4v2';entry=HERE/'baseline-until-complete-v2/validate.py'
            ...
            declaration=read(HERE/'boundary-p4v2/declaration.json')
            for system in ('mixed','distserve','dynamollm','ecoserve'):
                ...
                name=system
                out=HERE/'boundary-baselines-p4v2'/system
                binding=HERE/'boundary-baseline-bindings-p4v2'/name
                strategy='dynamollm-resident' if system=='dynamollm' else system
                da=[v for dataset in datasets for v in ['--dataset',dataset]]
                strategy_spec=HERE/'baseline-strategy-specs-p4v2'/(system+'.json')
                require(read(strategy_spec)['host_release']==declaration['baseline_controller_hosts'][system],'strategy source not predeclared')
                self.command(name+'-bind',[sys.executable,'-B',str(binder),'--spec',str(strategy_spec),'--receipt',str(receipt),'--out',str(binding),
                    '--output',str(out/'results'),'--strategy',strategy,'--gate',str(gate),*da],500,120,controller_host=declaration['baseline_controller_hosts'][system])
                self.command(name+'-measure',[sys.executable,'-B',str(HERE/'run_boundary_baseline_p4v2.py'),
                    '--binding',str(binding/'binding.json'),'--out',str(out),'--system',system,'--run'],400,max_work_s=400*len(rows)+90,performance_out=out,controller_host=declaration['baseline_controller_hosts'][system])
...
def main():
    ...
    if not a.run:
        m=read(HERE/'boundary-baseline-package-p4v2.json')
        for f,h in m['files'].items():require(sha(f)==h,'queue source changed')
        print(json.dumps(dict(passed=True,cpu_only=True,baseline_cells=len(read(HERE/'boundary-p4v2/declaration.json')['cells']),all_newly_explored_rates_paired=True)));return
    q=Queue()
