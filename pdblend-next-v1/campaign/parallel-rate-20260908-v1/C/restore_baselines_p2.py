...
PREVIOUS=HERE/'baseline-restore-p2/previous-binding.json'
RELEASE=HERE/'new-rate-release-p2/release.json'
OUT=HERE/'baseline-restore-p2'
...
def verify_release(path,digest):
    ...
    result=[]
    for name,count in [('screen-p2',12),('new-rate-pdb-p2',4)]:
...
def expected():
    ...
    result.update(schema=2,out=str(OUT),operation='restart-retained-residents',previous_binding=str(PREVIOUS),pdb_binding=str(PREVIOUS),
        ...
        required_predecessors=[dict(stage=str(HERE/'screen-p2'),count=12),dict(stage=str(HERE/'new-rate-pdb-p2'),count=4)],
