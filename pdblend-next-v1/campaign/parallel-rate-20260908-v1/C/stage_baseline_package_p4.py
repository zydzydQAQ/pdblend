...
for p in [spec,HERE/'run_baseline_new_rates_p4_repeats.py',HERE/'baseline_queue_p4.py',HERE/'baseline-queue-package-p4.json',HERE/'strict-failure-gates-p4-cpu.json',HERE/'new-rates-p1/declaration.json']:
    files[str(p)]=sha(p)
...
p=HERE/('baseline-stage-p4-'+str(time.time_ns())+'.json');write_new(p,json.dumps(result,indent=2).encode())
