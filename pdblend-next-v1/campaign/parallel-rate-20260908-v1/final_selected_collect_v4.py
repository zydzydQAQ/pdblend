...
import report_versions_v8 as report_versions
import final_selected_baseline_v4 as inspect_baseline
import source_identity_v4 as source_identity
import audit_dynamic
import raw_metrics_v3
import report_scope
import final_selection_v4 as selection_rules
import final_completion_v1 as completion_rules
import final_baseline_overlay_v2 as baseline_overlay
...
def collect(out,selection_path,selection_sha):
    selection=selection_rules.validate(selection_path,selection_sha)
    series="selected-Cp4-Ap8-Bp4"
    if out.exists():raise FileExistsError(out)
    audit=load_audit();p=audit.p
    code_sources={str(path):sha(path) for path in (Path(__file__),ROOT/'boundary_first_loss.py',ROOT/'inspect_exploration.py',ROOT/'report_versions_v8.py',ROOT/'final_selected_baseline_v4.py',ROOT/'source_identity.py',ROOT/'source_identity_v4.py',ROOT/'audit_dynamic.py',ROOT/'raw_metrics_v3.py',ROOT/'raw_metrics_v2.py',ROOT/'report_scope.py',ROOT/'boundary_first_loss_v2.py',AUDIT/'report.py',AUDIT/'protocol.py',AUDIT/'raw_metrics.py')}
    code_sources.update({str(path):sha(path) for path in (ROOT/"collect_v13.py",ROOT/"final_selection_base_v4.py",ROOT/"final_selection_v4.py",ROOT/"final_completion_v1.py",ROOT/"capacity_calibration_compatibility_v1.py",ROOT/"final_baseline_overlay_v2.py")})
    code_sources.update({str(ROOT/name):sha(ROOT/name) for name in ('capacity_calibration_compatibility_v3.py','audit_cooperative_arrivals_v1.py','eco_baseline_replacement_v1.py','eco_baseline_replacement_a_v1.py','eco_baseline_replacement_b_v1.py','baseline_row_metadata_v1.py','audit_eco_observations_v1.py','final_a_qualification_v1.py')})
    code_sources[str(selection_path)]=selection_sha
    for reference in [selection["reuse_declaration"],*selection["baseline_declarations"],*selection.get("scope_declarations",[]),*selection.get("baseline_continuations",[])]:
        code_sources[reference["path"]]=reference["sha256"]
    if selection.get("a_terminal_scope"):
        reference=selection["a_terminal_scope"];code_sources[reference["path"]]=reference["sha256"]
    ...
    overlay=baseline_overlay.validate(p,selection,originals)
    code_sources.update(overlay['sources'])
    for old in originals:
        if old['system']=='pdblend':continue
        baseline_overlay.annotate(old,overlay)
        if old['cell_id'] in overlay['quarantine']:
            code_sources.update(baseline_overlay.audit_quarantined_original(audit,old,overlay['quarantine'][old['cell_id']]))
    ...
    for status_path in stage_paths():
        ...
        if not selection_rules.select_stage(selection,status.get("model"),version,order):continue
        ...
        for cell in order:
            ...
            if cp.exists():
                ...
                checkpoints[cid]=cp
    selection_rules.add_pending_declarations(selection,declarations,origins,sources)
    points=[];pairs=[]
    for cid,cell in declarations.items():
        ...
        point['candidate_series']=selection['models'][point['model']]['series']
        ...
        if cp_path and point['measurement_valid']:
            ...
            selection_rules.preserve_identity(point,binding,config,cp_path,selection)
        ...
        for system,bid in exact_baselines.items():
            ...
            old=baseline[bid]
            if not baseline_overlay.eligible(old):continue
    ...
    selection_rules.annotate_scope(points,origins,selection,p)
    baseline_points,baseline_sources=inspect_baseline.collect(audit,ROOT,points,selection['baseline_declarations'],selection.get('baseline_continuations',[]),overlay,originals)
    sources.update(baseline_sources)
    new_valid=[x for x in baseline_points if baseline_overlay.eligible(x)]
    ...
    for point in points:
        ...
        for old in new_valid:
            same_repeat=old['repeat']==point['repeat']
            original_repeat_reuse=(old['repeat']==1 and point['repeat']>1 and old.get('reuse_original_rate_first_repeat') is True)
            if old['system'] in old_systems or not (same_repeat or original_repeat_reuse) or p.pair_identity(old)!=p.pair_identity(point):continue
            verdict=p.verdict(point,old)
            strict=bool(point['work_complete'] and old['work_complete'] and
                point['slo_attainment']>=.90 and old['slo_attainment']>=.90 and
                point['energy_j']<old['energy_j'] and point['energy_per_good_request_j'] is not None and
            ...
            pairs.append(dict(model=point['model'],dataset=point['dataset'],rate_rps=point['rate_rps'],
                ...
                implementation_id=point['implementation_id'],version_id=point['version_id'],profile_sha256=point['profile_sha256'],
                **{f'baseline_{k}':old[k] for k in ('version_id','controller_source_sha256','profile_sha256','policy_sha256')},
                baseline_repeat=old['repeat'],baseline_new_rate=old['new_rate'],**verdict,strict_service_energy_pass=strict))
    ...
    grid=selection_rules.annotate_grid(logical_grid(originals,points,pairs),selection,originals,points,p)
    boundaries=report_versions.boundary_records(points)
    completion=completion_rules.assess(points,pairs,grid,baseline_overlay.required_baselines(baseline_points),boundaries)
    ...
    result=dict(schema=2,created_s=time.time(),candidate_series=series,baseline_snapshot='snapshot-006',
        ...
        new_baseline_points=baseline_points,new_baseline_verified=len(new_valid),new_baseline_declared=len(baseline_overlay.required_baselines(baseline_points)),
        archived_baseline_observations=[x for x in list(baseline.values())+baseline_points if not x['selected_for_final_comparison']],
        baseline_scientific_overlay=selection['baseline_overlay'],
        ...
        selection=dict(path=str(selection_path),sha256=selection_sha),selected_model_series={m:v['series'] for m,v in selection['models'].items()},
        final_report_complete=completion["complete"],completion=completion,actual_source_hashes_never_relabelled=True,capacity_policy_equal_across_models=False)
    ...
    write_csv(out/'engineering-quarantined-baselines.csv',[x for x in list(baseline.values())+baseline_points if x['cell_id'] in overlay['quarantine']])
    write_csv(out/'archived-replaced-baselines.csv',[x for x in list(baseline.values())+baseline_points if not x['selected_for_final_comparison']])
    ...
    report_versions.figures(out,[x for x in originals if x['system']=='pdblend' or baseline_overlay.eligible(x)]+new_valid,points,series)
    ...
    lines+=['',f'原450点保留；共{len(overlay["quarantine"])}个已确认工程故障的旧baseline观测另表保留原能耗及失败证据，不参与科学胜负或有效曲线。整组替换历史和正常容量负结果分别标记。零GPU锁冲突失败单列于results.json的attempts，不计为性能观测。',
        '本矩阵采用预声明的整模型版本：C/B实际p4关闭容量扩缩，A实际p8开启经资格验证的扩缩。容量和闲置节能数值保留实际P6整组来源，转换成本采用实际P8整组测量，兼容复用证明分别固定。按版本保留实际源码、画像、策略及每次重复；未完成队列仍明确显示缺口。',
        '最新用户要求：首个完整SLO<90%的PDB点即停止上探，取消独立种子边界确认及区间加密；90%只作rate停止条件，主比较允许双方低于90%。新率未配齐五系统时不作完整节能结论。全八卡主能耗与重叠外层窗口不相加。']
    lines+=['', '所选 rate 比较已完成全部要求。' if completion['complete'] else '所选 rate 比较仍有测量或核验缺口，详见 results.json 的 completion。']
        ...
        for dataset in ('alpaca','sharegpt','longbench'):
            for system in ('pdblend','mixed','distserve','dynamollm','ecoserve'):
                ...
                result.append(dict(model=model,dataset=dataset,system=system,complete_service_pass_rates=sorted(cleanpass),complete_service_fail_rates=sorted(cleanfail),incomplete_or_disagreeing_rates=sorted(uncertain),
                    ...
                    certified=False,independent_seeds_required=[],
...
if __name__=='__main__':
    parser=argparse.ArgumentParser();parser.add_argument('--out',type=Path);parser.add_argument('--selection',type=Path,required=True);parser.add_argument('--selection-sha256',required=True);parser.add_argument('--validate-draft',action='store_true')
    args=parser.parse_args()
    if args.validate_draft:
        selection_rules.validate(args.selection,args.selection_sha256,draft=True)
        print(json.dumps(dict(cpu_only=True,draft_valid=True,final_report_generated=False)))
    else:
        if args.out is None:parser.error('--out required for report')
        collect(args.out.resolve(),args.selection.resolve(),args.selection_sha256)
