...
def enrich(point,binding,config,checkpoint):
    point.update(source_identity.identity(binding,point['dataset']))
...
def matrix_by_version(points,pairs):
    ...
    for pair in pairs:
        ...
        grouped[(point['model'],point['dataset'],point['version_id'],pair['baseline_system'],pair['baseline_version_id'])].append(pair)
    result=[]
    for (model,dataset,version,baseline,baseline_version),values in sorted(grouped.items()):
        ...
        result.append(dict(model=model,dataset=dataset,version_id=version,version_label=p['version_label'],
            baseline=baseline,baseline_version_id=baseline_version,
            baseline_controller_source_sha256=values[0]['baseline_controller_source_sha256'],
            baseline_profile_sha256=values[0]['baseline_profile_sha256'],
            baseline_policy_sha256=values[0]['baseline_policy_sha256'],verified_executions=len(values),rates=sorted({v['rate_rps'] for v in values}),
            ...
            configured_gpu_count=p['configured_gpu_count'],
            capacity_integration_enabled=p.get('capacity_integration_enabled',False),
            actual_peak_resident_gpu_counts=sorted({by_cell[v['cell_id']].get('actual_peak_published_resident_gpu_count',p['configured_gpu_count']) for v in values}),
...
def figures(out,originals,points,series):
    ...
    metrics=[('slo_attainment','SLO attainment (%)',100),
        ...
        ('generated_token_throughput_tps','Recorded output tokens / full window (tok/s)',1),
    ...
    with PdfPages(out/'rate-curves.pdf') as pdf:
        for field,label,scale in metrics:
            ...
            for i,m in enumerate(('7b','14b','32b')):
                for j,d in enumerate(('alpaca','sharegpt','longbench')):
                    ...
                    for system,color in styles.items():
                        ...
                        identities=sorted({x['version_id'] for x in rows})
                        for bi,version in enumerate(identities):
                            local=[x for x in rows if x['version_id']==version]
                            for baseline_repeat in sorted({x.get('repeat',1) for x in local}):
                                repeated=[x for x in local if x.get('repeat',1)==baseline_repeat]
                                ax.plot([x['rate_rps'] for x in repeated],[value(x,field,scale) for x in repeated],
                                    label=f"{system} {version[:6]} / r{baseline_repeat}",
                                    color=color,marker=('.', '^', 'v')[bi%3] if baseline_repeat==1 else 's',lw=.9,
                                    ls=('-', ':', '-.')[bi%3] if baseline_repeat==1 else '--',alpha=.65)
                    ...
                    for vi,version in enumerate(sorted({x['version_id'] for x in current})):
                        ...
                        for rep in sorted({x['repeat'] for x in local}):
                            ...
                            layout=(f"start {local[0]['configured_gpu_count']}/peak {max(x.get('actual_peak_published_resident_gpu_count',x['configured_gpu_count']) for x in rows)}"
                                if local[0].get('capacity_integration_enabled') else str(local[0]['configured_gpu_count']))
                            ax.plot([x['rate_rps'] for x in rows],[value(x,field,scale) for x in rows],
                                label=f"PDB {local[0]['version_id'][:6]} / r{rep} / {layout} GPU",
            ...
            fig.suptitle(f'{series}: source/profile/policy and repeats kept separate; x = incomplete work; missing usage may undercount tokens',fontsize=12)
            pdf.savefig(fig)
