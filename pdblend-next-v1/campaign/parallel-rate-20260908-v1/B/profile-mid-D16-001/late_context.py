...
def derive_all(root, *, original_evidence, tp2_validator):
    ...
    for index in range(6):
        ...
        if not (spec and spec.get('batch_size') == 16 and spec.get('input_lengths') == [2048]*16
                and spec.get('output_lengths') == [512]*16 and spec.get('clock_command_mhz') == (1500 if index < 3 else 2520)
                and spec.get('budget_tokens') == 8192 and spec.get('max_num_seqs') == 32
            ...
            result['valid'] = False
            result['errors'].append(dict(stage='declaration', error='not the six prescribed B16 2048->512 work points'))
        if result['valid']:
            late=result['late_context']
            per_id=late['per_request']
            if not (late['complete_prefill_token_sum']==32768 and late['complete_decode_token_sum']==8176
                    and len(per_id)==16 and all(v['total_observed_decode_steps']==511
                        and v['attention_after_max']==2559 and v['declared_bucket_edge']==2560 for v in per_id.values())):
                result['valid']=False
                result['errors'].append(dict(stage='complete_domain',error='full per-ID decode or actual late2560 domain absent'))
        if path.exists():
