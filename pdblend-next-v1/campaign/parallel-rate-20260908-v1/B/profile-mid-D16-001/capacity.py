"""CPU contracts for new, measured batch16 data; no extrapolated ProfilePoint."""
import hashlib
import json
import math
...
INPUT = 2048
OUTPUT = 512
REQUIRED_KV_TOKENS = BATCH * (INPUT + OUTPUT)  # 40960; 2560 is block-16 aligned.
INITIAL_PREFILL_TOKENS = BATCH * INPUT  # 32768; necessarily multiple actual steps.
TOKENS = 8192
SEQS = 32
MINIMUM_PREFILL_STEPS = (INITIAL_PREFILL_TOKENS + TOKENS - 1) // TOKENS
...
def validate_runtime_capacity(state, *, require_accepting=True, now=None):
    ...
    require(type(free) is int and type(total) is int and REQUIRED_KV_TOKENS <= free <= total,
            'real free KV cannot hold all16 complete 2048+512 allocations')
...
def validate_batch16_observation(raw, events):
    ...
    require(spec['batch_size']==BATCH and spec['input_lengths']==[INPUT]*BATCH
            ...
            'point is not declared real16 concurrent 2048->512')
    ...
    require(len(full)>=64,'real full batch16 decode was not sustained for64 steps')
    prefills=[]
    for event in events:
        pf,dc=event.get('prefill'),event.get('decode')
        request_ids=event.get('request_ids')
        require(type(pf) is int and type(dc) is int and pf>=0 and dc>=0
                and isinstance(request_ids,list) and len(request_ids)==len(set(request_ids))==pf+dc
                and set(request_ids)<=ids,'actual phase counts or owner IDs differ')
        require((pf>0 and event['tokens']>dc) or (pf==0 and event['tokens']==dc),
                'prefill/decode token accounting differs')
        if pf>0:prefills.append(event)
    prefill_tokens=sum(e['tokens']-e['decode'] for e in prefills)
    require(prefill_tokens==INITIAL_PREFILL_TOKENS and len(prefills)>=MINIMUM_PREFILL_STEPS,
            'all32768 actual prefill tokens require at least4 budget-limited steps')
    return dict(batch=BATCH,full_batch_decode_steps=len(full),exact_output_equal_within_batch=True,
                actual_prefill_tokens=prefill_tokens,actual_prefill_bearing_steps=len(prefills),
                minimum_prefill_bearing_steps=MINIMUM_PREFILL_STEPS,exact_prefill_step_count_claimed=False,
