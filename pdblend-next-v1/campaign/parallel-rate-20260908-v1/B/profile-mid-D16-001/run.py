...
def predecessor():
    ...
    reference=authority['predecessor_release'];require(sha(reference['path'])==reference['sha256'],'v7 release changed')
    ...
    require(len(expected)==16 and all(r['model']=='32b' and r['stage']=='screen_fixed2' for r in order),
            'exact B v7 sixteen declared points required')
    require(status.get('complete') is True and status.get('phase')=='complete'
            ...
            'running or incomplete B v7 stage cannot release this profile')
    checkpoints=[]
    for cid in expected:
    ...
    short_reference=authority['short_profile_status']
    require(sha(short_reference['path'])==short_reference['sha256'],'short profile terminal changed')
    short=json.loads(Path(short_reference['path']).read_text())
    require(short.get('complete') is True and short.get('passed') is True
            and short.get('cleanup_complete') is True and short.get('clock_release_complete') is True,
            'short profile cleanup incomplete')
    for owner in (short.get('pid'),short.get('child_pid')):
        require(owner is None or not Path('/proc',str(owner)).exists(),'short profile owner remains alive')
    ...
    return dict(authorization_sha256=sha(ROOT/'priority-authorization.json'),
        v7_status_sha256=sha(stage/'status.json'),checkpoints=checkpoints,
...
class Gate:
    def __init__(self):
        ...
        self.state = dict(complete=False, passed=False, phase='preflight', started_s=time.time(),
            scope='6 natural batch16 TP2 2048-to-512 observations; no profile publication or KV certification',
    ...
    async def work(self,hardware):
        ...
        for point in report['points']:
            ...
            references.append(raw['mid2048_batch16_evidence']['output_reference_sha256'])
...
if __name__=='__main__':
    ...
    check()
    if not args.execute:print(json.dumps(dict(package_valid=True,planned_points=6,gpu_executed=False,scheduled=False,required_kv_tokens=40960)))
    else:
