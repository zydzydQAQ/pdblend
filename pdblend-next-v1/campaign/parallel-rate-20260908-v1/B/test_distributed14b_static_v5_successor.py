...
s=importlib.util.spec_from_file_location('v4',P/'distributed14b_static_v5.py');m=importlib.util.module_from_spec(s);s.loader.exec_module(m)
class Successor(unittest.TestCase):
 def setUp(self):
  self.parent={'path':'CPU-only-parent','sha256':'ceb2f27562e249d50a1782c46ad7bf1a802e0cf735c3ece2930ca96d19bfa3e4'}
  ...
  self.s=dict(max_service_frequency_mhz=2100,profile=self.q['profile'],profile_registration=self.q['profile_registration']['registration'],host_manifest=self.q['host_manifest'],cpu_validation={'path':'CPU-proof'},idle_domain_reacquisition=dict(enabled=True,flag='idle_domain_reacquire_v1',parent_controller_manifest=self.parent,timeout_s=1.5,idle_transition_qualification={'CPU':'six-probes'}))
  self.lineage=dict(schema='distributed14b-P11-to-P12-idle-recovery-successor-v1',parent_manifest=self.parent,host_manifest=self.q['host_manifest'],cpu_validation=self.s['cpu_validation'],profile=self.q['profile'],profile_registration=self.s['profile_registration'],idle_domain_reacquire_timeout_s=1.5,idle_transition_qualification={'CPU':'six-probes'})
 def run_check(self):
 ...
 def test_registration(self):
  ...
  with self.assertRaises(ValueError):self.run_check()
 def test_idle_budget_changed(self):
  self.s['idle_domain_reacquisition']['timeout_s']=2
  with self.assertRaises(ValueError):self.run_check()
 def test_idle_probe_changed(self):
  self.s['idle_domain_reacquisition']['idle_transition_qualification']={'CPU':'other'}
  with self.assertRaises(ValueError):self.run_check()
 def test_original_physical_and_full_protocol_AST(self):
  ...
  old,new=nodes(P.parent/'C/distributed14b_static_v3.py'),nodes(P/'distributed14b_static_v5.py')
  for name in old:
