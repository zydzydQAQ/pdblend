"""Predeclared Eco37 window-only group; preserve every outcome and stop on any failure."""
import argparse,asyncio,copy,csv,json,math,os,signal,sys,time
from pathlib import Path
B=Path(__file__).resolve().parent;sys.path.insert(0,str(B));import eco_drained_qualify_v2 as qualification
c=qualification.c;p=c.p;DECL=qualification.DECL;RULES=B/'eco-drain37-v1/execution-rules-v2.json'
...
def contract():
 p.need(p.sha(DECL)==DECL_SHA,'original thirty-five execution-row declaration changed');d,files=qualification.contract();rule=p.read(RULES)
 p.need(rule['schema']=='B-Eco37-arrival-engineering-v1' and rule['declaration']==p.ref(DECL) and rule['host_manifest']==d['host_manifest'],'execution rules bind a different group/source')
 ...
 p.need(len(d['cells'])==35 and len({r['cell_id'] for r in d['cells']})==35,'exact thirty-five distinct points required')
 return d,rule

async def execute(a,state):
 d,rule=contract();previous=p.ref(B/'cooperative-final-observations-001/completion-audit.json');reference=p.ref(qualification.OUT/'ecoserve/binding.json');base=p.checked(reference);qualification.audit(reference)
 ...
 state.update(declared=35,remaining=[r['cell_id'] for r in d['cells']],declaration=p.ref(DECL),execution_rules=p.ref(RULES))
 ...
 with node_lease():
  ...
  async with aiohttp.ClientSession(trust_env=False) as session:
   ...
   for row in d['cells']:
    if a.stop_requested or (B/'STOP-eco-drain37').exists():state.update(phase='stopped_at_boundary');break
    ...
    binding['files'].update({str(DECL):p.sha(DECL),str(RULES):p.sha(RULES),str(Path(__file__).resolve()):p.sha(__file__),row['trace_path']:row['trace_sha256']})
    ...
    try:
     ...
     p.write(cp,dict(row=row,declaration=p.ref(DECL),binding=p.ref(bp),receipt=p.ref(rp),artifacts=artifacts,measurement_valid=receipt['measurement_valid'],work_complete=receipt['summary'].get('work_complete'),arrival_fidelity_gate=arrival,execution_rules=p.ref(RULES),completed_s=time.time()),exclusive=True)
  ...
  state['complete']=len(state['completed'])==35
