...
BINDER = Path('/root/workspace/pdblend-next-v1/campaign/parallel-rate-20260908-v1/B/baseline-binder-until-complete-v1')
...
PINS = {
    ...
    str(BINDER / 'manifest.json'): '48fa11a4c0421557b352d046157a8bfe1b1dc76d7d20867aa7a45feb9aed2471',
    str(BINDER / 'bind.py'): '053346013a1269cd7302108cceac1a856a34c7d0dedc8e01cac26ca0556beabb',
}
...
def restore_contract(bootstrap, original, inventory_ref, binder, power):
    ...
    for key in ('host_release', 'hostname', 'model', 'large_inputs', 'window_s', 'seeds'):
...
def audit_inputs(bootstrap_ref, gate_dir, original_performance_ref, fresh_inventory_ref):
    ...
    require(bootstrap.get('identity_file') == fresh_inventory_ref['path']
            and bootstrap['files'].get(fresh_inventory_ref['path']) == fresh_inventory_ref['sha256'],
            'fresh inventory must be the explicit bootstrap identity')
...
def build_binding(original, bootstrap, qualification, inventory, legacy, refs, output_root):
    ...
    for key in ('fresh_ablation_binding', 'experiment_scope', 'executor_wrapper'):
        result.pop(key, None)
    result.update(deadline_s=None,campaign_lifecycle='until_declared_complete_v1',instances=copy.deepcopy(bootstrap['instances']), configs=configs, output=str(out / 'results'),
