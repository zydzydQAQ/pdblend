...
OUT=B/'historical-scale11-continuation-001'
...
def main():
 ...
 try:
  declaration=B/'historical-ecoserve-scale11-declaration.json';digest=sha(declaration);d=read(declaration)
  for p,h in d['source_files'].items():assert sha(p)==h
  assert read(B/'historical-scale11-cpu-validation.json')['passed']
  while True:
   if (B/'STOP-historical-scale11').exists():raise RuntimeError('historical STOP before GPU stage')
   prior=read(B/'continuation-completion-001/status.json')
  ...
  with (OUT/'runner.log').open('xb') as log:
   child=subprocess.Popen(['python3','-u',str(B/'historical_scale11.py'),'--run'],cwd=B,stdout=log,stderr=subprocess.STDOUT)
  ...
  result=read(B/'historical-ecoserve-scale11-001/status.json')
