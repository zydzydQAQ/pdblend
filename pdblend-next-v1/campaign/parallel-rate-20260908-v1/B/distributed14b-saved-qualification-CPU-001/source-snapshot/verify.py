...
def verify(reference,binding,cp=None):
    ...
    bound(q['profile_registration']['registration'],files)
    ...
    return dict(passed=True,hostname=q['hostname'],measurement_host=q['node'],gpu_identity=gpus,host_manifest=q['host_manifest'],profile_sha256=q['profile']['sha256'],execution_group=q['execution_group'],independently_recomputed=True,native=native,frequency_cases=frequencies,profile_registration=registration)
