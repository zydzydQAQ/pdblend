...
B=Path(__file__).resolve().parent;sys.path.insert(0,str(B));import baseline_reconciliation_v3 as q,remote
...
from pathlib import Path
B=Path('/root/workspace/pdblend-next-v1/campaign/parallel-rate-20260908-v1/B');sys.path.insert(0,str(B));import baseline_control_after_external_v2 as q
x=json.loads(PAYLOAD);p=q.p;refs=p.read(B/'baseline-bindings-after-external-001.json');bindings={k:p.checked(r) for k,r in refs.items()};common=q.fixed.runtime(bindings[x['system']]['host_release']);proofs={k:q.audit_performance(k,b) for k,b in bindings.items()}
async def check():
 import aiohttp
