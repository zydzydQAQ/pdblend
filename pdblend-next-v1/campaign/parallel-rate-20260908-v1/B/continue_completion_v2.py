...
OUT=B/'continuation-completion-002'
...
def main():
 ...
 try:
  ...
  extra=read(B/'completion-continuation-v2-manifest.json')
  for p,h in extra['files'].items():assert sha(p)==h
  assert read(B/'lb025-repeat2-cpu-validation.json')['passed']
  while True:
  ...
  commands=[('lb025_repeat2',[str(B/'append_lb025_repeat2.py'),'--run']),('restore',[str(B/'baseline_control_completion.py'),'restore']),('gate',[str(B/'baseline_control_completion.py'),'gate']),('qualify',[str(B/'baseline_control_completion.py'),'qualify'])]
  for repeat in (1,2):
