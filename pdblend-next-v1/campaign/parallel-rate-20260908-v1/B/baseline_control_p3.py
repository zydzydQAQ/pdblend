...
RESTORE=ROOT/'baseline-return-p3-001'
QUAL=ROOT/'baseline-qualification-p3-001'
...
def modules():
 ...
 execution=fixed.load(ROOT/'baseline-return-p3-source/execution.py','b_new_retained_restore')
 qualifier=fixed.load(ROOT/'qualify_restored_baseline_p3.py','b_new_fresh_eco_qualifier')
 return target,common,execution,qualifier

def finished_pdb():
 new=fixed.load(ROOT/'added_runner_p3.py','b_new_rate_predecessor');new.predecessor()
 out=ROOT/'added-pdb-p3-001';s=p.read(out/'status.json')
 p.need(s.get('complete') is True and len(s['completed'])==6 and not s['failed'] and not s.get('node_lease_held') and not new.alive(s['pid']),'all six new PDB cells must finish before measured baseline return')
...
async def restore():
 ...
 release,previous,_=fixed.release_contract(ROOT/'fixed-release-p3-001/release.json')
 ...
 ready=dict(schema='parallel-rate-B-measured-baseline-return-p3',ready=True,created_s=time.time(),
 ...
 p.write(ROOT/'baseline-return-ready-p3.json',ready,exclusive=True)
 return ready

def bootstrap():
 ready=p.read(ROOT/'baseline-return-ready-p3.json');p.need(ready['ready'] is True,'measured restore absent')
 ...
 for f in (Path(__file__).resolve(),ROOT/'qualify_restored_baseline_p3.py'):b['files'][str(f)]=p.sha(f)
...
def bind_other(system,original_ref,bootstrap_ref,gate_dir,out):
 ...
 q=fixed.load(ROOT/'qualify_restored_baseline_p3.py','b_fresh_identity_contract')
 ...
 b['fresh_baseline_p3']=dict(original=original_ref,bootstrap=bootstrap_ref,gate_dir=str(gate_dir),output_root=str(out),original_policy_bytes_unchanged=True)
 return b

def qualify():
 _,common,_,q=modules();ready=p.read(ROOT/'baseline-return-ready-p3.json');bp=p.ref(QUAL/'fresh-bootstrap.json');gate_dir=QUAL/'original27'
 ...
 p.write(ROOT/'baseline-bindings-p3.json',refs,exclusive=True)
 return refs

def audit_performance(system,binding):
 if system=='ecoserve':
  q=fixed.load(ROOT/'qualify_restored_baseline_p3.py','b_added_eco_qualification')
 ...
 inputs=binding['fresh_baseline_p3'];expected=bind_other(system,inputs['original'],inputs['bootstrap'],Path(inputs['gate_dir']),Path(inputs['output_root']))
