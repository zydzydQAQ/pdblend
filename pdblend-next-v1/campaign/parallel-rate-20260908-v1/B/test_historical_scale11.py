...
s=importlib.util.spec_from_file_location('historical',B/'historical_scale11.py')
...
m.p.read=original_read
out=dict(schema=1,passed=True,cpu_only=True,gpu_executed=False,created_s=time.time(),cases=cases,source={'path':str(B/'historical_scale11.py'),'sha256':hashlib.sha256((B/'historical_scale11.py').read_bytes()).hexdigest()},declaration={'path':str(m.DECL),'sha256':hashlib.sha256(m.DECL.read_bytes()).hexdigest()},inherited_request_failure_gate='execution-completion-p4-002/runner.py engineering_gate and baseline-completion-cpu-validation.json; no mechanism change')
(B/'historical-scale11-cpu-validation.json').write_text(json.dumps(out,indent=2)+'\n');print(json.dumps({'passed':True,'cases':len(cases)}))
