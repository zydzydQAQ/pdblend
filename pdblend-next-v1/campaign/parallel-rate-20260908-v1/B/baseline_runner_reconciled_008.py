...
import baseline_reconciliation_v3 as reconciliation
RECON=ROOT/'baseline-reconciliation-008/declaration.json'
RECON_SHA='5143f1034030a871a0975bcd6f0491d8298fcd5fb241db8337aeedfc1d6a48af'
...
def predecessor():
 controller=fixed.load(ROOT/'baseline_control_after_external_v2.py','b_completion_baseline_controller')
 return controller.finished_pdb()

def contract():
 p.need(p.sha(RECON)==RECON_SHA,'reconciliation declaration changed');reconciled=reconciliation.audit(RECON)
 ...
 p.need({c['cell_id'] for c in reconciled['remaining_cells']}<={c['cell_id'] for c in d['cells']},'reconciled point outside selected original scope')
 d['original_five_system_declaration_count']=len(d['cells']);d['cells']=reconciled['remaining_cells']
...
async def execute(a,state):
 ...
 state.update(declared=len(cells),full_five_system_declaration_count=d['original_five_system_declaration_count'],system_group=a.system_group,repeats=[1,2],reconciliation=p.ref(RECON),remaining=[c['cell_id'] for c in cells])
 ...
 with node_lease():
  ...
  if bases:
   qualifier=fixed.load(ROOT/'baseline_control_after_external_v2.py','baseline_p4_qualification')
  ...
  async with aiohttp.ClientSession(trust_env=False) as session:
   for row in cells:
    ...
    binding['files'].update({str(DECL):p.sha(DECL),str(RECON):RECON_SHA,str(ROOT/'baseline_reconciliation_v3.py'):p.sha(ROOT/'baseline_reconciliation_v3.py'),str(Path(__file__).resolve()):p.sha(__file__),row['trace_path']:row['trace_sha256']})
...
def main():
 q=argparse.ArgumentParser();q.add_argument('--system-group',choices=('baselines',),required=True);q.add_argument('--bindings',type=Path);q.add_argument('--out',required=True,type=Path);q.add_argument('--run',action='store_true');a=q.parse_args();a.stop_requested=False
 if not a.run:
  d,_,_=contract();print(dict(cpu_passed=True,gpu_executed=False,selected_cells=sum((c['system']=='pdblend')==(a.system_group=='pdblend') for c in d['cells'])));return
