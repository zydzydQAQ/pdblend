...
def host_processes():
    ...
    failed = p.checked(p.read(HERE / 'manifest.json')['failed_successor'])
    require(failed['finished_s'] and not failed['complete'] and not failed['children']
            and not failed['observed_checkpoints'], 'v1 must have failed before GPU work')
    pids.append(failed['pid'])
...
def validate_inventory(parent, inventory, original_inventory):
    ...
    for c in inventory:
        ...
        proofs.append(dict(container_id=c['Id'], **hostconfig_equivalence(old['HostConfig'], c['HostConfig'])))
    ...
    return sorted(proofs, key=lambda row: row['container_id'])
...
def cold_snapshot(parent):
    ...
    proofs = validate_inventory(parent, inventory, original)
    ...
    return dict(schema='B32B-resume-cold-preflight-v1', captured_s=time.time(), **processes,
                target_inventory=inventory, historical_target_inventory=original,
                historical_inventory_reference=dict(path=parent['identity_file'], sha256=parent['files'][parent['identity_file']]),
                HostConfig_equivalence_proofs=proofs,
                running_container_ids=[], gpu_compute_pids=[], gpu_inventory=gpu,
