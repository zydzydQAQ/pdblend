...
def restore_contract(bootstrap, original, inventory_ref, binder, power):
    ...
    # Recompute the recorded cold-start equivalence from unmodified inventories.
    preflight = pinned(status['cold_start_preflight'])
    require(preflight['historical_inventory_reference'] == dict(path=original['identity_file'],
            sha256=original['files'][original['identity_file']]), 'wrong historical cold-start inventory')
    require(preflight['historical_target_inventory'] == list(old_inventory.values()),
            'historical raw inspection was altered')
    cold = {c['Id']: c for c in preflight['target_inventory']}
    require(set(cold) == set(old_inventory), 'cold-start target identity set differs')
    proofs = []
    for cid, previous in old_inventory.items():
        require(not cold[cid]['State']['Running'] and cold[cid]['State']['Pid'] == 0,
                'cold-start inventory was not stopped')
        proofs.append(dict(container_id=cid, **hostconfig_equivalence(previous['HostConfig'], cold[cid]['HostConfig'])))
    require(preflight['HostConfig_equivalence_proofs'] == sorted(proofs, key=lambda row: row['container_id']),
            'recorded DNS equivalence does not match the raw inspections')
