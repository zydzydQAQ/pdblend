...
from adapters import control as c, verifier as q, OLD, source_contract
p, g, R = c.p, c.g, c.R
...
def prepare():
    ...
    releases = {}
    previous = p.ref(OLD / 'pdb-performance-001/status.json')
    for system in ('mixed', 'distserve', 'dynamollm', 'ecoserve'):
        ...
        required_refs = [boundary_ref, pdb_release['declaration'], p.ref(contract_path), refs[system],
                         ...
                         p.ref(HERE / 'verify_baseline_v2.py'), p.ref(HERE / 'baseline_control_v2.py'),
        ...
        release = dict(schema='B32B-ascending-cells-release-v1', node='B', system=system, created_s=time.time(),
            ...
            qualification=p.ref(root / 'qualification.json'), qualification_validator=p.ref(HERE / 'verify_baseline_v2.py'),
...
if __name__ == '__main__':
    ...
    source_contract()
    print(json.dumps(prepare() if args.run else dict(passed=True, cpu_only=True)))
