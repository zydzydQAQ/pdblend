...
verifier = g.load(HERE / 'native_saved_verifier.py', 'B32B_resume_verifier_v2')
...
def source_contract():
    ...
    assert manifest['schema'] == 'B32B-cold-resume-source-v2'
...
def qualify_with_dns_equivalence():
    """Original four-system qualification assembly with the scoped Eco verifier."""
    boot_ref = p.ref(control.QUAL / 'bootstrap.json')
    q = g.load(HERE / 'qualify_eco_drained_baseline_v2.py', 'B32B_resume_Eco_dns_qualifier')
    eco = q.qualify(boot_ref, control.QUAL / 'original27', p.ref(control.ORIGINAL),
                    p.ref(control.RESTORE / 'containers.after.json'), control.QUAL / 'ecoserve')
    q.audit_binding(eco)
    refs = {'ecoserve': eco}
    for system in ('mixed', 'distserve', 'dynamollm'):
        binding = control.derive_other(system)
        path = control.QUAL / system / 'binding.json'
        assert not path.exists()
        p.save(path, binding)
        refs[system] = p.ref(path)
    p.save(control.QUAL / 'bindings.json', refs)
    return refs

control.qualify = qualify_with_dns_equivalence
