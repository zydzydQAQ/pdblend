...
A=B.parent/'A/isolated-power-v2'
...
with tempfile.TemporaryDirectory(prefix='isolated-stop-race-') as td:
 ...
 assert len(a.samples)==len(a.utilization_samples)==len(a.frequency_samples)
 assert a._terminal is not None
 out=dict(passed=True,physical_host=socket.gethostname(),pid=os.getpid(),created_s=time.time(),finding='v2 STOP joins original sampler before publishing full consistent tail; same controlled partial-row scenario now passes',source={str(p):hashlib.sha256(p.read_bytes()).hexdigest() for p in [A/'isolated_sampler.py',A/'sampler_worker.py',Path(__file__)]},raw_samples=len(a.samples),raw_utilization=len(a.utilization_samples),error=a.error,child_exited=not a._thread.is_alive(),false_pass=False)
 path=B/'sampler-stop-race-v2-independent-verification.json';path.write_text(json.dumps(out,indent=2)+'\n');print(json.dumps(out))
