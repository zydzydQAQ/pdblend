...
import argparse,ast,asyncio,copy,importlib,json,sys,tempfile,time,faulthandler,hashlib
...
def controller_cases(module):
 ...
 with tempfile.TemporaryDirectory() as d:
  ...
  for strategy in ('pdblend-joint','mixed','ecoserve','distserve','dynamollm-resident','dynamollm'):
   ...
   now=time.time();states=tuple(InstanceState(i['id'],i['role'],i['tp'],tuple(i['gpus']),now,0,2400,100000,0,0) for i in cfg['instances']);snapshot=RuntimeSnapshot(1,now,states);request=RequestBudget('cpu',now,128,64,15,.2,output_limit=64)
   c.state.snapshot=snapshot
   if strategy=='mixed':plans=[c.max_mixed(request,now)]
   elif strategy.startswith('pdblend'):plans=c.planner.candidates(snapshot,request,now)
   elif strategy=='distserve':plans=[c.distserve_scheduler.plan(snapshot,[request],now=now)]
   elif strategy=='ecoserve':plans=[c.eco_scheduler.plan(snapshot,[request],now=now)]
   else:plans=[c.dynamo_scheduler.plan(snapshot,[request],now=now)]
   assert plans and all(p is not None for p in plans)
   assert all(a.frequency_mhz<=2400 for plan in plans for a in plan.frequencies)
   cases.append(dict(case='actual-strategy-admission-or-capacity-fallback-domain',strategy=strategy,passed=True))
...
def bootstrap_cases():
 module=importlib.import_module('ecopadg.serving.physical_frequency');cases=[]
 with tempfile.TemporaryDirectory() as d:
  path=Path(d)/'inventory.json';rows=[dict(gpu=2,process_pids=[],at_s=time.time())]
  inventory=dict(transition_inflight=True,initial_ids=['old'],known_instances={'new':dict(state='start_intent',owner_kind='created_for_cell',transaction='tx',gpus=[2],tp=1)},events=[dict(kind='clock_bootstrap_proof',instance_id='new',transaction='tx',gpus=[2],rows=rows)])
  path.write_text(json.dumps(inventory));proof=dict(schema='capacity-clock-bootstrap-v1',inventory_path=str(path),inventory_sha256=hashlib.sha256(path.read_bytes()).hexdigest(),instance_id='new',transaction='tx',gpus=[2],rows=rows)
  controller=SimpleNamespace(max_service_frequency_mhz=2400,config=dict(capacity_inventory_path=str(path),capacity_lease_authority='CPU-only',capacity_job_path='CPU-only'),backend=SimpleNamespace(instances={'old':dict(gpus=[0])}))
  lease_calls=[]
  with patch.dict(sys.modules,{'capacity_executor':SimpleNamespace(check_lease=lambda **kw:lease_calls.append(kw))}):
   assert module.bootstrap_allowed(controller,[2],2400,proof,time.time()) and len(lease_calls)==1
   cases.append(dict(case='actual-bootstrap-qualified-max-complete-scoped-proof',passed=True))
   assert not module.bootstrap_allowed(controller,[2],2520,proof,time.time()) and len(lease_calls)==1
   cases.append(dict(case='bootstrap-old2520-rejected-without-lease-bypass',passed=True))
   bad=copy.deepcopy(proof);bad['rows'][0]['at_s']-=2
   assert not module.bootstrap_allowed(controller,[2],2400,bad,time.time())
   bad=copy.deepcopy(proof);bad['transaction']='foreign'
   assert not module.bootstrap_allowed(controller,[2],2400,bad,time.time())
   cases.append(dict(case='bootstrap-stale-or-foreign-owner-still-rejected',passed=True))
 return cases

def default_proof(host):
 import importlib.util
 sp=importlib.util.spec_from_file_location('p10_builder_audit',Path(__file__).resolve().parent/'builder.py');m=importlib.util.module_from_spec(sp);sp.loader.exec_module(m)
...
async def main(a):
 ...
 cases=await asyncio.wait_for(backend_cases(b),30);print('controllers',flush=True);cases+=controller_cases(runtime);cases+=bootstrap_cases();cases.append(default_proof(a.host));faulthandler.cancel_dump_traceback_later()
