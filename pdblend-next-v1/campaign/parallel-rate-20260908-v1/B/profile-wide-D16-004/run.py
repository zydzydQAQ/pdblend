...
def predecessor():
    ...
    require(authority['authorized'] is True and authority['automatic_retries'] is False,'explicit new diagnostic work required')
    require(authority.get('global_deadline_cancelled') is True,'latest user removed campaign total deadline; bounded work remains')
    def alive(pid):
    ...
    require(sha(stage/'status.json')==authority['predecessor_status']['sha256'],'p3 terminal status changed')
    require(status['pid']==authority['predecessor_pid'] and not alive(status['pid']) and not alive(authority['predecessor_watcher_pid'])
            and status.get('stopped_at_boundary') is True and status.get('node_lease_held') is False and len(status['completed'])==6 and not status['failed'],'prior serving owner or cleanup incomplete')
    checkpoints=[]
    for cid in status['completed']:
        cpfile=stage/'results/checkpoints'/(cid+'.json');cp=json.loads(cpfile.read_text());receipt=json.loads(Path(cp['receipt']).read_text())
        require(cp['measurement_valid'] is True and sha(cp['receipt'])==cp['receipt_sha256'] and all(sha(k)==v for k,v in cp['artifacts'].items()),'prior observation changed')
        require(receipt['measurement_valid'] and receipt['child_stopped'] and receipt['clock_restore_complete'] and not receipt['outer_cleanup_errors'],'native cleanup incomplete')
        checkpoints.append(dict(path=str(cpfile),sha256=sha(cpfile)))
    prior=authority['prior_profile'];prior_status=json.loads(Path(prior['status_path']).read_text())
    require(sha(prior['status_path'])==prior['status_sha256'] and not alive(prior['pid']), 'prior profile identity or process changed')
    require(prior_status.get('complete') is True and (prior_status.get('measurement_valid') is True or (prior_status.get('error')=="KeyError('deadline_s')" and not prior_status.get('child_pid') and prior_status.get('identity_failure_no_control') is True)) and prior_status.get('child_exit_confirmed') is True and prior_status.get('cleanup_complete') is True, 'prior profile cleanup incomplete')
    reference=authority['original_binding'];require(sha(reference['path'])==reference['sha256'],'actual PDB binding changed')
    return dict(authorization_sha256=sha(ROOT/'priority-authorization.json'),p3_status_sha256=sha(stage/'status.json'),checkpoints=checkpoints,previous_request_failures_preserved=4,diagnostic_profile_only=True,global_deadline_cancelled=True,prior_profile=authority['prior_profile'])
...
class Gate:
    def __init__(self):
        ...
        self.state = dict(complete=False, passed=False, phase='preflight', started_s=time.time(),
            scope='6 natural batch16 TP2 2304-to-512 observations; no profile publication or KV certification',
    ...
    async def identity(self,label):
        ...
        binding['deadline_s']=None;binding['campaign_lifecycle']='until_declared_complete_v1'
        common=load('priority_b_original_identity','/root/workspace/pdblend-next-v1/campaign/parallel-rate-20260908-v1/common/execution-until-complete-v1/run.py')
    ...
    async def work(self,hardware):
        ...
        for point in report['points']:
            ...
            references.append(raw['mid2304_batch16_evidence']['output_reference_sha256'])
...
if __name__=='__main__':
    ...
    check()
    if not args.execute:print(json.dumps(dict(package_valid=True,planned_points=6,gpu_executed=False,scheduled=False,required_kv_tokens=45056)))
    else:
