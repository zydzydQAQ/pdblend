...
INPUT = 2304
OUTPUT = 512
REQUIRED_KV_TOKENS = BATCH * (INPUT + OUTPUT)  # 45056; 2816 is block-16 aligned.
INITIAL_PREFILL_TOKENS = BATCH * INPUT  # 36864; necessarily multiple actual steps.
TOKENS = 8192
SEQS = 32
...
def validate_runtime_capacity(state, *, require_accepting=True, now=None):
    ...
    require(type(free) is int and type(total) is int and REQUIRED_KV_TOKENS <= free <= total,
            'real free KV cannot hold all16 complete 2304+512 allocations')
...
def validate_batch16_observation(raw, events):
    ...
    require(spec['batch_size']==BATCH and spec['input_lengths']==[INPUT]*BATCH
            ...
            'point is not declared real16 concurrent 2304->512')
    ...
    require(prefill_tokens==INITIAL_PREFILL_TOKENS and len(prefills)>=MINIMUM_PREFILL_STEPS,
            'all36864 actual prefill tokens require at least5 budget-limited steps')
