...
def verify_successor_qualification(successor, qualification):
    ...
    need(lineage['schema'] == 'distributed14b-P10-to-P11-idle-domain-successor-v1', 'unknown P11 controller lineage')
    need(idle['parent_controller_manifest'] == lineage['parent_manifest'] and lineage['parent_manifest']['sha256'] == '5dcae91099369a188f4aecb92f7a8c06b138e6ae0ae181b786f7b7011d034b84', 'P11 original P10 parent differs')
...
def load_release(path):
    ...
    if release['system'] == 'pdblend':
        ...
        for p in base['configs'].values():
            cfg = read(p); need(cfg.get('idle_domain_reacquire_v1') is True, 'actual P11 configuration must enable the frozen idle-domain fix'); need(cfg.get('capacity_integration_v1') is False and 'capacity_binding_path' not in cfg and 'capacity_binding_sha256' not in cfg, 'dynamic capacity source leaked')
    return release, base
