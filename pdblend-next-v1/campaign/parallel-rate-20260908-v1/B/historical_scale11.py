...
p=fixed.p
DECL=B/'historical-ecoserve-scale11-declaration.json'
CHAIN=p.REPO/'campaign/main-rate-priority-v2/B/scale_chain.py'
OUT=B/'historical-ecoserve-scale11-001'
...
def prerequisite():
 out=B/'continuation-completion-001';s=p.read(out/'status.json')
 ...
 p.need([x['phase'] for x in s['steps']]==['restore','gate','qualify','baseline_repeat1','baseline_repeat2'] and all(x['exitcode']==0 for x in s['steps']),'baseline predecessor incomplete')
