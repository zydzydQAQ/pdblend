...
import baseline_reconciliation as reconciliation
RECON=ROOT/'baseline-reconciliation-001/declaration.json'
RECON_SHA='503af5c0ae9c139a78ee6ed48f646be5b36bda16db4677374cf475bc57b8304e'
 ...
 with node_lease():
  ...
  async with aiohttp.ClientSession(trust_env=False) as session:
   for row in cells:
    ...
    binding['files'].update({str(DECL):p.sha(DECL),str(RECON):RECON_SHA,str(ROOT/'baseline_reconciliation.py'):p.sha(ROOT/'baseline_reconciliation.py'),str(Path(__file__).resolve()):p.sha(__file__),row['trace_path']:row['trace_sha256']})
