...
def capacity_negative(cp,receipt):
 ...
 p.need(all(r['request_timeout']=='True' and r['error']=='request_hard_timeout' and r['slo_ok']=='0' and r['token_ids_verified']=='0' and math.isclose(float(r['request_deadline_s'])-float(r['planned_arrival_s']),120,abs_tol=1e-7) for r in bad),'failure is not the original hard request deadline')
...
def audit(path):
 ...
 observed={};negatives={r['cell_id']:r for r in d['capacity_negatives']};actualnegative=set()
 for entry in d['prior_stages']:
  ...
  for cid in state['attempted']:
   ...
   observed[cid]=cp
 p.need(actualnegative==set(negatives),'capacity-negative set differs')
 ...
 p.need(d['remaining_cells']==[c for c in expected if c['cell_id'] not in observed],'remaining original repeats changed, omitted or replayed')
