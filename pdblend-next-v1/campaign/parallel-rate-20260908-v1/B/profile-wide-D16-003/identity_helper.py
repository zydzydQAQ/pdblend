...
def validate_binding(binding):
    ...
    require(binding.get('deadline_s') is None and binding.get('global_deadline_cancelled') is True, 'latest user cancelled campaign deadline')
    ...
    try:
        async with aiohttp.ClientSession(trust_env=False) as session:
            for row in rows:
                ...
                validate_binding(binding)
                if args.phase == 'scale':
                    reference = output / 'checkpoints' / (row['reuse_main_cell_id'] + '.json')
                    require(reference.exists() and read(reference)['row']['trace_sha256'] == row['trace_sha256'], 'missing paired 1x result')
...
def main():
    ...
    p.add_argument('--phase', choices=('main', 'scale'), default='main')
    p.add_argument('--max-cells', type=int, default=30)
