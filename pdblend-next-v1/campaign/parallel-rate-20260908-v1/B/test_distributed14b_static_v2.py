...
s=importlib.util.spec_from_file_location('subject',HERE/'distributed14b_static_v2.py');m=importlib.util.module_from_spec(s);s.loader.exec_module(m)
R=HERE.parent
...
class SourceChecks(unittest.TestCase):
    def test_original_raw_math_unchanged(self):
        ...
        old,new=nodes(HERE/'distributed14b_static_v1.py'),nodes(HERE/'distributed14b_static_v2.py')
    ...
    def test_original_executor_invocation(self):
        tree=ast.parse((HERE/'distributed14b_static_v2.py').read_text())
    ...
    def test_hardware_capture_is_readonly(self):
        tree=ast.parse((HERE/'distributed14b_static_v2.py').read_text());f=next(n for n in tree.body if isinstance(n,ast.AsyncFunctionDef) and n.name=='hardware_identity')
