...
HOST=ROOT/'parallel-rate-20260908-v1/hosts/32b-fixed-p4'
...
def register(middle,base,out):
 ...
 for micro,batch,inputlen,outputlen,edge in ((middle,16,2304,512,2816),):
  ...
  need(all(state.get(k) is True for k in ('complete','passed','measurement_valid','evidence_complete','baseline_preservation_verified','clock_release_complete','cleanup_complete','child_exit_confirmed')),'complete actual source/native/full8 measurement cleanup required')
  ...
  for index,item in enumerate(report['points']):
   ...
   shared_edge=min(r['attention_after_max']+1 for r in actual['late_context']['per_request'].values());need(shared_edge>=2633,'shared D16 observed domain does not cover2633')
   rederived.append(dict(shared_edge=shared_edge,point_id=item['point_id'],raw=ref(path/'raw.json'),qualification=ref(path/'late-context.json'),
  ...
  need(not any(state.get(k) for k in ('error','sampling_error','sampling_tail_error','cancel_errors','drain_errors','cleanup_error','clock_cleanup_error')),'outer engineering or cleanup fault retained')
  ...
  observed_edge=min(r['shared_edge'] for r in rederived)
  need(2633<=observed_edge<edge,'conservative shared edge must be strictly within complete per-ID maximum')
  for freq in (1500,2520):
   ...
   groups.append(dict(batch=batch,input_tokens=inputlen,context_tokens=observed_edge,frequency_mhz=freq,rows=rows,
    iteration_s=center,iteration_upper_s=upper,power_upper_w=power,samples=3,actual_max_attention=observed_edge-1,complete_per_id_edge=edge,shared_full_batch_edge=observed_edge,
 ...
 old_points=copy.deepcopy(profile['points']);profile['points'].extend(added)
 need(profile['points'][:len(old_points)]==read(base)['points'],'all historical bounds including290ms must remain unchanged')
 ...
 profile['empirical_registration']=dict(measured_role='decode',actual_owner_role='mixed',new_mixed_points=0,new_interference_points=0,
  ...
  fixed_prompt_repetitions_not_arrival_seeds=True,context_edge_semantics='registered shared full-D16 edge is the minimum across all IDs and all six runs of maximum measured full-batch attention plus one; complete per-ID2816 edge is not shared coverage; no truncation or tail deletion',
 ...
 save('qualification.json',dict(passed=True,all_six_valid=True,profile=ref(out/'profiles.development.json'),reports=reports,groups=groups,
  source_sha256=sources,serving_validation_pending=True,no_gpu_executed_by_registration=True,base_profile=ref(base),old_points_count=len(old_points),all_old_points_retained=True,old290ms_upper_preserved=True,
