...
def verify_successor_qualification(successor, qualification):
    need(successor['max_service_frequency_mhz'] == qualification['max_service_frequency_mhz'], 'declared and qualified hardware maxima differ')
    need(successor['profile'] == qualification['profile'], 'declared and qualified profile references differ')
    need(successor['profile_registration'] == qualification['profile_registration']['registration'], 'declared and qualified raw registration differ')
    need(successor['host_manifest'] == qualification['host_manifest'], 'declared and qualified P11 source differs')
    idle = successor.get('idle_domain_reacquisition')
    need(isinstance(idle, dict) and idle.get('enabled') is True and idle.get('flag') == 'idle_domain_reacquire_v1', 'P11 explicit enabled idle-domain successor required')
    lineage = checked(qualification['controller_successor'])
    need(lineage['schema'] == 'distributed14b-P11-to-P12-idle-recovery-successor-v1', 'unknown P11 controller lineage')
    need(idle['parent_controller_manifest'] == lineage['parent_manifest'] and lineage['parent_manifest']['sha256'] == 'ceb2f27562e249d50a1782c46ad7bf1a802e0cf735c3ece2930ca96d19bfa3e4', 'P11 original P10 parent differs')
    need(lineage['host_manifest'] == qualification['host_manifest'] and lineage['cpu_validation'] == successor['cpu_validation'], 'P11 actual source or common CPU lineage differs')
    need(lineage['profile'] == qualification['profile'] and lineage['profile_registration'] == qualification['profile_registration']['registration'], 'P11 numeric lineage differs')
    need(type(idle.get('timeout_s')) in (int,float) and idle['timeout_s']==lineage['idle_domain_reacquire_timeout_s']==1.5, 'P12 qualified idle budget differs')
    need(idle['idle_transition_qualification']==lineage['idle_transition_qualification'], 'P12 local idle transition evidence differs')
...
def load_release(path):
    ...
    verify_successor_qualification(successor, qualification)
    ...
    if release['system'] == 'pdblend':
        ...
        for p in base['configs'].values():
            cfg = read(p); need(cfg.get('idle_domain_reacquire_v1') is True and type(cfg.get('idle_domain_reacquire_timeout_s')) in (int,float) and cfg['idle_domain_reacquire_timeout_s']==1.5, 'actual P12 configuration must enable the bounded idle-domain fix'); need(cfg.get('capacity_integration_v1') is False and 'capacity_binding_path' not in cfg and 'capacity_binding_sha256' not in cfg, 'dynamic capacity source leaked')
    return release, base
