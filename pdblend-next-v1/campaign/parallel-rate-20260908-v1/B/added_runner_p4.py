...
p=fixed.p
DECL=ROOT/'added-rates-p4-001/declaration.json'
...
def contract():
 d=p.read(DECL);p.need(d['schema']=='parallel-rate-added-five-system-B-p4-v1' and d['deadline_s']==p.DEADLINE,'wrong new-rate declaration')
 ...
 for a,b in zip(d['cells'],old['cells']):
  expected=copy.deepcopy(b);expected['cell_id']=expected['cell_id'].replace('parallel-rate-p1-added','parallel-rate-p4-added')
...
async def execute(a,state):
 ...
 state.update(declared=len(cells),full_five_system_declaration_count=30,system_group=a.system_group,repeat=a.repeat,remaining=[c['cell_id'] for c in cells])
 ...
 with node_lease():
  ...
  if bases:
   qualifier=fixed.load(ROOT/'baseline_control_p4.py','baseline_p4_qualification')
   for system,binding in bases.items():qualifier.audit_performance(system,binding)
  async with aiohttp.ClientSession(trust_env=False) as session:
   for row in cells:
    if a.stop_requested or (ROOT/'STOP-added-p4').exists() or time.time()+400>=p.DEADLINE:
...
def main():
 q=argparse.ArgumentParser();q.add_argument('--system-group',choices=('pdblend','baselines'),required=True);q.add_argument('--repeat',type=int,choices=(1,2),required=True);q.add_argument('--bindings',type=Path);q.add_argument('--out',required=True,type=Path);q.add_argument('--run',action='store_true');a=q.parse_args();a.stop_requested=False
 if not a.run:
