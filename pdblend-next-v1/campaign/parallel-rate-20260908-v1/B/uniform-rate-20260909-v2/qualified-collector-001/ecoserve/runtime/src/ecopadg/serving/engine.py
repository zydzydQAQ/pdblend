...
class EngineService:
    ...
    def update_snapshot(self):
        ...
        if self.config.get('peers') and time.monotonic()-self.transfer_snapshot_s>.1:
            ...
            self.transfer_snapshot=dict(
                ...
                transfer_inflight_sends=(sum(s['inflight_sends'] for s in transfers)
                    if all('inflight_sends' in s for s in transfers) else None),
                transfer_inflight_sends_observed=all('inflight_sends' in s for s in transfers),
    ...
    def commit_state(self, payload):
        ...
        for scheduler in self.engine.scheduler:
            # This is an explicit owner-thread administrative boundary, not a
            # scheduling iteration. Publish the written generation immediately
            # rather than falsely acknowledging a lagging background cache.
            if os.environ.get("PDBLEND_ASYNC_IO") == "1":
                observed = read_runtime(scheduler, force_refresh=True)
                if (observed != payload or getattr(scheduler, "_pdblend_runtime_error", None)):
    ...
    async def drain(self,request):
        ...
        async with self.control_lock:
            ...
            try:
                ...
                # owner/all-rank barrier completes preceding sends without
                # inventing a missing transport send counter.
