...
def template(layout, destination):
    p.need(layout == '8tp1', 'B14 ShareGPT uses the declared eight TP1 layout')
    ...
    cfg, _ = historical.configuration('14b', 'longbench' if hetero else 'sharegpt', 'distserve' if hetero else 'mixed')
    ...
    host = p.REPO / 'releases/five-system100-A14B-baseline-v1-runtime'
    ...
    for path in (host / 'manifest.json', Path(__file__), ROOT / 'common/execution-until-complete-v1/run.py',
                 ROOT / 'B/distributed-14b-v1/pdb-p12-release-001/binding.json',
                 ROOT / 'B/distributed-14b-v1/frequency2100-registered-001/profiles.development.json'):
        source_files[str(path)] = p.sha(path)
    value = dict(schema='uniform-migration-B-legacy-template-v2', node='B', model='14b', layout=layout,
        expected_hostname='iZwz9i5bte3xkpmcoes3t2Z', image='sha256:d11407cd827a43a0dec8ad7d4d7037c97c39bbe93c6f4b4fd951c94e67509a8b',
        host_release=str(host), source_entry=str(DEPLOY / 'engines/A/engine.py'), instances=instances,
        profile=p.ref(ROOT / 'B/distributed-14b-v1/frequency2100-registered-001/profiles.development.json'),
...
def terminal(reference, layout):
    ...
    p.need(saved['node'] == 'B' and saved['model'] == '14b' and saved['complete']
    ...
    contract = p.load(ROOT / 'common/uniform-rate-20260909-v2/contract.py', 'migrationB_baseline_boundary')
    decisions = {}
    for dataset in ('sharegpt',):
        group = contract.resolve_group(saved['declaration'], '14b', dataset, actual_host='B')
    ...
    p.need(binding['hostname'] == 'iZwz9i5bte3xkpmcoes3t2Z' and binding['model'] == '14b', 'foreign predecessor binding')
...
def make_spec(t, terminal_ref, previous, out):
    ...
    portbase, kvbase = (39200, 60000) if hetero else (39000, 59000)
    records = []
    configs = {}
    for n, row in enumerate(t['instances']):
        rid = ('uniformb2l' if hetero else 'uniformb2r') + str(n)
    ...
    spec=dict(schema='uniform-migration-B-legacy-deployment-v2', protocol_id='per-dataset-slo-five-system-fixed-window-v1',
        model='14b', node='B', layout='distserve-longbench' if hetero else 'resident',
        ...
        supported_datasets=['longbench'] if hetero else ['sharegpt'], source_entry=t['source_entry'],
...
async def perform(t, spec_ref, out, state):
    helper=p.load(ROOT/'B/baseline-return-after-external-source-v1/execution.py','migrationB_legacy_executor')
    executor=helper.load_common(t['host_release'])
    from ecopadg.serving.campaign import node_lease
    deploy=p.load(DEPLOY/'deploy.py','migrationB_frozen_legacy_deployer')
    ...
    binder=p.load(BINDER/'bind.py','migrationB_frozen_native_binder')
...
def main():
    ...
    state=dict(schema='uniform-v2-migration-B-baseline-bootstrap-status',node='B',model='14b',layout=t['layout'],
