...
METER = ROOT / 'A/uniform-rate-20260909-v1/meter-runtime'
IDENTITY = HERE / 'node-identity.json'
ADAPTER = ROOT / 'A/uniform-rate-20260909-v1/isolated-power/manifest.json'
...
def validate_identity(identity, hostname, rows):
    assert identity['node'] == 'B'
    assert hostname == identity['actual_hostname'] == 'iZwz9i5bte3xkpmcoes3t2Z', 'wrong physical node'
