...
def check_fixed_resume(plan):
    prior=p.checked(plan['resume_prior_pipeline'])
    p.need(prior['node']=='B' and prior['model']=='14b' and prior['datasets']==['sharegpt']
           and prior['finished_s'] and not p.active_owner(prior) and not prior['node_lease_held']
           and not prior['observations'], 'prior qualification owner is not safely terminal')
    p.need(prior['child']['exitcode']==1 and not p.active_owner(prior['child'])
           and 'idle' in prior['child']['argv'] and str(HERE/'stages.py') in prior['child']['argv'],
           'only the failed CPU idle-spec transition may resume here')
    fixed_reference=plan['reuse_complete_fixed_qualification']
    p.need(fixed_reference['path'] in prior['child']['argv'], 'prior stage used another qualification')
    qualification=p.checked(fixed_reference);qualified_state=p.checked(qualification['status'])
    p.need(qualified_state['passed'] and qualified_state['finished_s'] and not qualified_state.get('error')
           and not qualified_state['node_lease_held'] and not p.active_owner(qualified_state),
           'the retained 82-shape qualification is incomplete or active')
...
async def execute(plan,state,out):
    ...
    try:
        ...
        if plan.get('reuse_complete_fixed_qualification'):
            fixed_reference=check_fixed_resume(plan)
            state['reused_complete_fixed_qualification']=fixed_reference
            state['resume_prior_pipeline']=plan['resume_prior_pipeline']
            state['phase']='independently_replaying_complete_82_shape_qualification'
            p.save(out/'status.json',state)
        else:
            ...
            fixed_reference=p.ref(out/'fixed-qualification-001/qualified.json')
        await child([sys.executable,'-B',HERE/'stages.py','idle','--input',fixed_reference['path'],'--out',out/'idle-spec-001'],out/'idle-spec.log',state,out)
...
def main():
    ...
    p.need(os.environ.get('PYTHONPATH')==plan['runtime_pythonpath'],'explicit qualification dependency path required')
