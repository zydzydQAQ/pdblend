...
async def wait_stage_release(plan,state,out):
    ...
    for name in ('baseline_template','baseline_qualification','baseline_validator','profile','meter_binding'):
    ...
    effective=dict(plan,**{k:stage[k] for k in ('baseline_template','baseline_qualification','baseline_validator','profile','meter_binding')})
...
async def execute(plan,state,out):
    ...
    try:
        ...
        await child([sys.executable,'-B',HERE/'baseline_producer.py','--spec',plan['baseline_template']['path'],'--predecessor-terminal',out/'pdb-boundary.json','--out',out/'baseline-deployment-001','--run'],out/'baseline-deployment.log',state,out)
