...
def predecessor():
    ...
    require(time.time()+720<authority['deadline_s'],'full profile and cleanup reserve required')
    def alive(pid):
    ...
    return dict(authorization_sha256=sha(ROOT/'priority-authorization.json'),p3_status_sha256=sha(stage/'status.json'),checkpoints=checkpoints,previous_request_failures_preserved=4,diagnostic_profile_only=True,deadline_s=authority['deadline_s'])
...
class Gate:
    def __init__(self):
        ...
        self.state = dict(complete=False, passed=False, phase='preflight', started_s=time.time(),
            scope='6 natural batch16 TP2 2304-to-640 observations; no profile publication or KV certification',
...
if __name__=='__main__':
    ...
    check()
    if not args.execute:print(json.dumps(dict(package_valid=True,planned_points=6,gpu_executed=False,scheduled=False,required_kv_tokens=47104)))
    else:
