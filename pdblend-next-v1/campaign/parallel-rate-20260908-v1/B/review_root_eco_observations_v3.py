...
inputs=json.loads((B/'root-eco-observation-review-inputs-v3.json').read_text())
audit=collect.load_audit();p=audit.p
out=B/'root-Eco-observations-independent-review-003';out.mkdir(exist_ok=False)
record=dict(schema='independent-root-Eco-observations-review-v1',physical_host=socket.gethostname(),pid=os.getpid(),started_s=time.time(),inputs=p.ref(B/'root-eco-observation-review-inputs-v3.json'),review_source=p.ref(Path(__file__)),cpu_tests=dict(path=str(R/'test_audit_eco_observations_v1.py'),sha256=p.sha(R/'test_audit_eco_observations_v1.py'),passed=16),points=[],complete=False)
...
try:
 ...
 record['prior22']=inputs['prior22'];prior=p.checked(inputs['prior22']);prior_ids={entry['cell_id'] for entry in prior['points']}
 p.need(len(prior_ids)==22 and all(e['passed'] for e in prior['points']),'prior22 incomplete')
 record['closure']=inputs['closure'];p.checked(inputs['closure'])
 p.need(len(inputs['checkpoints'])==15,'exact remaining15 required')
 ...
 for cp_ref in inputs['checkpoints']:
  ...
  started=time.time();cp=p.checked(cp_ref);row=cp['row'];p.need(row['cell_id'] not in prior_ids,'already audited original22 must not repeat');entry=dict(checkpoint=cp_ref,cell_id=row['cell_id'],model=row['model'],started_s=started)
  try:
 ...
 p.need(len({e['cell_id'] for e in record['points']}|prior_ids)==37,'combined C37 not exact')
 record.update(complete=True,combined_unique_count=37,passed=all(x['passed'] for x in record['points']),finished_s=time.time(),count=len(record['points']))
