"""B ShareGPT P10 factory; only real completed local profile evidence is accepted."""
import argparse,asyncio,copy,importlib.util,json,socket,sys
from pathlib import Path
B=Path(__file__).resolve().parent;R=B.parent
sys.path.insert(0,str(B));import distributed14b_static_v2 as run
...
def prepare(registration,profile,out):
    ...
    out=Path(out).resolve();need(not out.exists(),'new immutable P10 release required');out.mkdir(parents=True)
    host=R/'hosts/14b-capacity-p10';host_manifest=ref(host/'manifest.json');cpu=ref(R/'cpu-validation-p10.json')
    ...
    cfg.update(instances=copy.deepcopy(base['instances']),profiles=str(Path(profile).resolve()),max_service_frequency_mhz=2100,
        capacity_integration_v1=False,controller_source_release=str(host),host_source_release=str(host),journal=str(out/'unused-control.jsonl'))
    ...
    validator_path=R/'common/distributed14b-qualification-v1/verify.py';validator=load(validator_path,'new_B_saved_qualification')
    ...
    source_package=R/'common/distributed14b-pdb-qualification-source-v1/manifest.json'
    files={**base['files'],**registered['sources'],**manifest_files(host/'manifest.json'),**manifest_files(regdir/'manifest.json'),**manifest_files(source_package)}
    for path in (original,config_path,hardware_path,group_path,successor_path,jobs_path,Path(profile),Path(registration),validator_path,Path(__file__),B/'distributed14b_static_v2.py',Path(cpu['path'])):files[str(path)]=sha(path)
    ...
    qualification=dict(schema='distributed14b-qualified-execution-v1',model='14b',node='B',hostname=base['hostname'],dataset='sharegpt',system='pdblend',
        binding=parent_binding,host_manifest=host_manifest,measurement_host_manifest=ref(R/'hosts/14b-capacity-p9/manifest.json'),profile=ref(profile),
    ...
    write(rules_path,dict(schema='distributed14b-static-execution-rules-v2',jobs=ref(jobs_path),source_successor=ref(successor_path),whole_assigned_group=True,actual_arm='fixed2',capacity_integration_v1=False,
        ...
        arrival_dispatch_max_limit_s=1.,arrival_dispatch_p99_limit_s=.1,stop_on_any_request_failure=True,stop_above_first_complete_pdb_slo_below=.9,source=ref(B/'distributed14b_static_v2.py')),True)
