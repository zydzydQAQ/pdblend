...
def modules():
 source=p.read(ROOT/'baseline-after-external-source-manifest-v1.json')
...
def external_predecessor():
 ...
 q=fixed.load(ROOT/'qualify_restored_baseline_completion.py','b_external_same_policy');q.same_policy(last['instances'],actual['instances'])
...
def bootstrap():
 ...
 b['files'].update(p.read(ROOT/'baseline-after-external-source-manifest-v1.json')['files'])
 b['files'][str(ROOT/'baseline-after-external-source-manifest-v1.json')]=p.sha(ROOT/'baseline-after-external-source-manifest-v1.json')
