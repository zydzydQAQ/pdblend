...
BINDER = C / 'B32B-ecoserve-qualified-binding-v2'
...
PINS = {
    ...
    str(BINDER / 'manifest.json'): '0c2c3774d7f5e6a6c8b0381c13691c2faa481eae81ee949480662af9af447b75',
    str(BINDER / 'bind.py'): 'fadbd81d8ff1dc882065dd4d03c32b3c27a60b08024f4c076d8687d1446dd578',
}
...
def restore_contract(bootstrap, original, inventory_ref, binder, power):
    ...
    require(status['started_s'] <= status['measurement_start_s'] < correctness['started_s']
            <= correctness['finished_s'] <= status['measurement_end_s'] <= status['finished_s'] <= DEADLINE,
...
def audit_inputs(bootstrap_ref, gate_dir, original_performance_ref, fresh_inventory_ref):
    ...
    require(actual_oracle['physical_evidence']['full_operation_end_s'] <= restore['started_s']
            ...
            < status['measurement_end_s'] <= status['finished_s'] <= DEADLINE,
...
def build_binding(original, bootstrap, qualification, inventory, legacy, refs, output_root):
    ...
    for key in ('fresh_ablation_binding', 'experiment_scope', 'executor_wrapper'):
        result.pop(key, None)
    result.update(instances=copy.deepcopy(bootstrap['instances']), configs=configs, output=str(out / 'results'),
