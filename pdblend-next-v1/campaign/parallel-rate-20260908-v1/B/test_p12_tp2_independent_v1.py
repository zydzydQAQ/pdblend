"""Independent P12 CPU TP2 transaction counterexamples, real ClockOwner; no GPU calls."""
...
async def main(a):
 ...
 specs += [dict(name='late-idle-995ms',settle=.995),dict(name='past-idle1500ms',settle=1.6,error=True),dict(name='invalid-idle-bool',idle_budget=True,prewrite=True),dict(name='invalid-idle-over2',idle_budget=3,prewrite=True)]
 results=[]
 with tempfile.TemporaryDirectory() as td:
  for case in specs:
   ...
   backend.json=native
   ctl=NS(backend=backend,state=state,action_lock=asyncio.Lock(),planner=NS(telemetry_ttl_s=2.),config=dict(max_service_frequency_mhz=2100,clock_failure_fresh_confirmation_v1=case.get('fresh',True),idle_domain_reacquire_timeout_s=case.get('idle_budget',1.5)),max_service_frequency_mhz=2100)
   ...
   if case.get('prewrite'):
    ...
    if case.get('journal'):
     assert event['confirmed_current_only'] is False and event['confirmed_before_original_settle_deadline'] is False and event['confirmed_within_idle_recovery_bound'] is True
     assert event['original_active_settle_timeout_s']==.3 and event['idle_domain_reacquire_timeout_s']==1.5
