...
import baseline_reconciliation_v2 as reconciliation
RECON=ROOT/'baseline-reconciliation-003/declaration.json'
RECON_SHA='1b937f1e0b7d3a63c41c871201250d2a17660199e47385ba56a3f8a3c490e40d'
 ...
 with node_lease():
  ...
  async with aiohttp.ClientSession(trust_env=False) as session:
   for row in cells:
    ...
    binding['files'].update({str(DECL):p.sha(DECL),str(RECON):RECON_SHA,str(ROOT/'baseline_reconciliation_v2.py'):p.sha(ROOT/'baseline_reconciliation_v2.py'),str(Path(__file__).resolve()):p.sha(__file__),row['trace_path']:row['trace_sha256']})
