...
SOURCE_EXECUTOR = Path('/root/workspace/pdblend-next-v1/campaign/parallel-rate-20260908-v1/common/execution-until-complete-v1/run.py')
...
def target_identity(parent, inventory, *, allow_running_same_targets=False):
    ...
    for i in parent['instances']:
        ...
        require(c['State'].get('Running') is False or allow_running_same_targets is True, 'target unexpectedly already running')
    return by_id
...
async def fresh_binding(common, session, parent, out):
    ...
    b.update(deadline_s=None,campaign_lifecycle='until_declared_complete_v1', output=str(out / 'results'),
...
async def restore_core(common, parent, previous, out):
    ...
    async with aiohttp.ClientSession(trust_env=False) as session:
        try:
            ...
            previous.update(deadline_s=None,campaign_lifecycle='until_declared_complete_v1')
            ...
            target_identity(parent, retained, allow_running_same_targets=({i['container']['id'] for i in previous['instances']} == set(ids)))
