...
def derive_all(root, *, original_evidence, tp2_validator):
    ...
    for index in range(6):
        ...
        if not (spec and spec.get('batch_size') == 16 and spec.get('input_lengths') == [2304]*16
            ...
            result['valid'] = False
            result['errors'].append(dict(stage='declaration', error='not the six prescribed B16 2304->512 work points'))
        if result['valid']:
            late=result['late_context']
            per_id=late['per_request']
            if not (late['complete_prefill_token_sum']==36864 and late['complete_decode_token_sum']==8176
                    and len(per_id)==16 and all(v['total_observed_decode_steps']==511
                        and v['complete_work_attention_upper']==2815 and v['attention_after_max']>=2632 and v['declared_bucket_edge']==2816 for v in per_id.values())):
                result['valid']=False
