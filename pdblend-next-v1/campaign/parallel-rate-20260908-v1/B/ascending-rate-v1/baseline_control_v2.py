...
def terminal():
    ...
    for reference, row in zip(state['observed_checkpoints'], release['rows']):
        ...
        with (Path(cp['receipt']['path']).parents[2] / 'cells' / row['cell_id'] / 'bench.csv').open() as f:
            bench = list(csv.DictReader(f))
        by_idx = {str(x['idx']): x for x in trace['requests']}
        assert len(bench) == len(by_idx) == row['n_requests']
        assert {x['idx'] for x in bench} == set(by_idx)
        good = 0
        for request in bench:
            expected = by_idx[request['idx']]
            assert request['request_id'] == request['idx']
            assert int(request['prompt_len']) == expected['prompt_len'] and int(request['output_len']) == expected['output_len']
            assert request['success'] == request['token_ids_verified'] == '1'
            assert int(request['generated_tokens']) == expected['output_len']
            assert request['request_timeout'].lower() not in ('1', 'true')
            ttft, tpot = float(request['ttft_s']), float(request['tpot_s'])
            assert math.isfinite(ttft) and math.isfinite(tpot)
            within = ttft <= row['slo_ttft_s'] and tpot <= row['slo_tpot_s']
            assert int(request['slo_ok']) == int(within)
            good += int(within)
        actual_q = good / len(bench)
        assert receipt['summary']['good_requests'] == good
        assert abs(receipt['summary']['slo_attainment'] - actual_q) < 1e-12
        observations.append(dict(receipt['summary'], slo_attainment=actual_q,
