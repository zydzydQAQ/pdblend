...
RESTORE=ROOT/'baseline-return-after-external-001'
QUAL=ROOT/'baseline-qualification-after-external-001'
...
def modules():
 source=p.read(ROOT/'baseline-after-external-source-manifest-v2.json')
 ...
 execution=fixed.load(ROOT/'baseline-return-after-external-source-v1/execution.py','b_new_retained_restore')
...
def external_predecessor():
 # This input can only be frozen after the unrelated owner has terminated and
 # restored its captured original containers. It is never synthesized by time.
 reference=ROOT/'external-release-for-B-001.json'
 d=p.read(reference)
 p.need(d['schema']=='B-external-owner-return-v1','explicit external terminal audit required')
 status=p.checked(d['status']);restoration=p.checked(d['restoration'])
 p.need(status['claim_id']=='1788878028113168732' and status['pid']==860907 and status['finished_s'],'wrong/live external owner')
 try:alive=Path('/proc',str(status['pid']),'stat').read_text().rsplit(') ',1)[1].split()[0]!='Z'
 except OSError:alive=False
 p.need(not alive,'external owner is still alive')
 p.need(Path(d['status']['path'])==C/'A14B-sharegpt-slo90-v1/claims/1788878028113168732/status.json' and Path(d['restoration']['path'])==C/'A14B-sharegpt-slo90-v1/execution/deployment/restore-result.json','wrong external receipt paths')
 raw=p.checked(restoration['restoration_receipt'])
 p.need(restoration.get('complete') is True and restoration.get('measurement_valid') is True and not restoration.get('error') and restoration['restoration']==raw,'external restoration wrapper/raw differs')
 p.need(raw.get('complete') is True and raw.get('restored') is True and raw.get('measurement_valid') is True and raw.get('clock_restore_complete') is True and not raw.get('errors'),'external native restoration failed')
 last=p.checked(p.read(ROOT/'baseline-bindings-completion.json')['ecoserve']);actual=p.checked(raw['restored_binding'])
 q=fixed.load(ROOT/'qualify_restored_baseline_completion.py','b_external_same_policy');p.need(len(actual['instances'])==len({i['id'] for i in actual['instances']})==4 and {i['id'] for i in actual['instances']}=={i['id'] for i in last['instances']},'external instance ID set differs')
 aligned={i['id']:i for i in actual['instances']};q.same_policy(last['instances'],[aligned[i['id']] for i in last['instances']])
 p.need({i['container']['id'] for i in actual['instances']}=={i['container']['id'] for i in last['instances']},'external return refers to another physical deployment')
 p.need(d['original_B_final_bindings']==p.ref(ROOT/'baseline-bindings-completion.json'),'wrong original B deployment lineage')
...
async def restore():
 ...
 release,_,_=fixed.release_contract(ROOT/'completion-release-p4-001/release.json')
 external=external_predecessor()
 from ecopadg.serving.campaign import node_lease
 p.need('PDBLEND_NODE_LOCK_FD' not in os.environ,'fresh independent restoration lease required')
 with node_lease():
  import aiohttp
  async with aiohttp.ClientSession(trust_env=False) as session:
   before=ROOT/'baseline-external-return-identity-001';p.need(not before.exists(),'new external restored identity directory required');before.mkdir()
   previous=await execution.fresh_binding(common,session,target,before)
   last=p.checked(p.read(ROOT/'baseline-bindings-completion.json')['ecoserve'])
   p.need(all(a['container']['StartedAt']!=b['container']['StartedAt'] for a,b in zip(last['instances'],previous['instances'])),'external restoration did not produce a new actual process')
   p.write(before/'binding.json',previous,exclusive=True);p.write(before/'external-predecessor.json',external,exclusive=True)
 ...
 p.write(ROOT/'baseline-return-ready-after-external-001.json',ready,exclusive=True)
 return ready

def bootstrap():
 ready=p.read(ROOT/'baseline-return-ready-after-external-001.json');p.need(ready['ready'] is True,'measured restore absent')
 ...
 b['files'].update(p.read(ROOT/'baseline-after-external-source-manifest-v2.json')['files'])
 b['files'][str(ROOT/'baseline-after-external-source-manifest-v2.json')]=p.sha(ROOT/'baseline-after-external-source-manifest-v2.json')
...
def qualify():
 _,common,_,q=modules();ready=p.read(ROOT/'baseline-return-ready-after-external-001.json');bp=p.ref(QUAL/'fresh-bootstrap.json');gate_dir=QUAL/'original27'
 ...
 p.write(ROOT/'baseline-bindings-after-external-001.json',refs,exclusive=True)
 return refs
