...
OUT=A/'p6-load-continuation-003'
SPEC=A/'load-p6-full-inputs-002/spec.json'
SPEC_SHA='2e48285a713c6f6d56cf28517108700f4295637e940f5f09a0dd609837ac1861'
...
def main():
 ...
 try:
  ...
  gate=A/'load-p6-gate-002'
  while True:
   require(not (A/'STOP_LOAD_P6_FULL').exists(),'STOP prevents successor')
   if not (gate/'status.json').exists():
    launch=json.loads((A/'load-p6-gate-launch-002.json').read_text());require(alive(launch['pid']),'gate exited before status publication');time.sleep(2);continue
  ...
  args=[sys.executable,'-u',str(CODE/'capacity_load_calibrate.py'),'--spec',str(SPEC),'--spec-sha256',SPEC_SHA,'--out',str(A/'load-p6-full-002'),'--run']
  ...
  terminal=json.loads((A/'load-p6-full-002/status.json').read_text());require(terminal.get('complete') and terminal.get('cleanup_complete') and not terminal.get('error') and len(terminal['completed'])==27,'full27 terminal incomplete')
