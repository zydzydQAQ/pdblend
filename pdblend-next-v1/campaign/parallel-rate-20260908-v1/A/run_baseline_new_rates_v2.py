    ...
    with node_lease():
        ...
        async with aiohttp.ClientSession(trust_env=False) as session:
            ...
            for row in cells:
                ...
                failed=bool(errors or receipt['summary'].get('failed_requests',0)>0 or receipt['summary'].get('request_timeouts',0)>0 or receipt['summary'].get('work_complete') is not True)
                gate=dict(passed=not failed,http503=errors,work_complete=receipt['summary'].get('work_complete'),failed_requests=receipt['summary'].get('failed_requests',0),request_timeouts=receipt['summary'].get('request_timeouts',0),stop_on_any_request_failure=True,capacity_failure_retained=True)
                write(output/'engineering-gates'/(row['cell_id']+'.json'),gate,True)
                if failed:state.update(phase='engineering_gate_failed',engineering_gate_failed=True);break
                write(args.out/'status.json',state)
