"""A14B retained baseline restoration through unchanged frozen hardware primitives."""
import argparse
import asyncio
import copy
import hashlib
...
PREVIOUS=HERE/'baseline-restore-p4/previous-binding.json'
RELEASE=HERE/'p4-added/fixed-release-001/release.json'
OUT=HERE/'baseline-restore-p4'
HOST='iZwz92bdfqihqp48tekqjyZ'
...
def verify_release(path,digest):
    ...
    result=[]
    for name,count in [('p4/fixed-screen-001',13),('p4-added/fixed-screen-001',4)]:
        ...
        points=[]
        for p in cps:
            c=read(p);r=read(c['receipt']);g=read(root/'results/operations'/p.stem/'engineering-gate.json')
            ...
            require(g.get('passed') is True and not g.get('faults'),'PDB frequency/engineering failure blocks baseline restoration')
...
def expected():
    ...
    require(original['model']=='14b' and original['hostname']==HOST and original['layout']=='resident','original C resident spec required')
    ...
    parent=load(PARENT,'a14b_original_static_validation')
    ...
    require(previous['model']=='14b' and previous['system']=='pdblend' and previous['deadline_s']==DEADLINE,
    ...
    result.update(schema=2,out=str(OUT),operation='restart-retained-residents',previous_binding=str(PREVIOUS),pdb_binding=str(PREVIOUS),
        ...
        required_predecessors=[dict(stage=str(HERE/'p4/fixed-screen-001'),count=13),dict(stage=str(HERE/'p4-added/fixed-screen-001'),count=4)],
...
def validate_spec(spec):require(spec==expected(),'A14B exact restored spec differs')

def bind_parent(spec):
    parent=load(PARENT,'a14b_frozen_hardware_primitives')
...
def main():
    ...
    if args.prepare:
        ...
        OUT.mkdir()
        previous=read(CAMPAIGN/'main-slo-improvement-v6/A/fresh-original-001/restored-binding/binding.json');previous['deadline_s']=DEADLINE
        ...
        dependencies[str(HERE/'run_baseline_new_rates.py')]=sha(HERE/'run_baseline_new_rates.py')
