...
def measurement_contract(spec):
    expected = dict(measurement_adapter=ref(A / 'isolated-power-v2/manifest.json'),
                    measurement_hooks=ref(A / 'dynamic-execution-isolated-power-002/sampler_hooks.py'),
                    measurement_driver=ref(A / 'load-p8-isolated-code-003/capacity_load_calibrate.py'),
                    measurement_hardware_selftest=ref(A / 'isolated-power-hardware-selftest-002/validation.json'))
    for key, reference in expected.items():
        need(spec.get(key) == reference and spec['files'].get(reference['path']) == reference['sha256'],
             'actual900 measurement component is not the frozen qualified version: ' + key)
    adapter = checked(expected['measurement_adapter'])
    need(all(spec['files'].get(path)==digest==sha(path) for path,digest in adapter['files'].items()),
         'actual900 sampler source is not frozen')
    selftest=checked(expected['measurement_hardware_selftest'])
    need(selftest['passed'] and selftest['read_only'] and not selftest['hardware_writes']
         and selftest['new_requests']==0 and selftest['result']['measurement_valid'] is True,
         'actual all8 read-only hardware sampler qualification required')
    return expected
...
def checked_stage_spec(audit, stage_module):
    ...
    driver = Path(spec['measurement_driver']['path']) if 'measurement_driver' in spec else A / 'load-p8-code-001/capacity_load_calibrate.py'
    expected = A / ('load-p8-isolated-code-003/capacity_load_calibrate.py' if audit['output'] == str(A / 'p8-qualification900-dynamic-002') else 'load-p8-code-001/capacity_load_calibrate.py')
    need(driver == expected, 'only the declared original gate or isolated900 driver qualifies')
...
def build_qualification():
    ...
    try:
        ...
        auditor = load(A / 'p8_qualification_audit_v3.py', 'formal_p8_actual_stage_auditor')
        ...
        measurement_contract(dynamic_spec)
        adapter_ref=ref(A / 'isolated-power-v2/manifest.json')
        hooks_ref=ref(A / 'dynamic-execution-isolated-power-002/sampler_hooks.py')
        need(dynamic_spec['measurement_adapter']==adapter_ref and dynamic_spec['measurement_hooks']==hooks_ref, 'actual900 must use the frozen formal measurement adapter')
        failed_power=ref(A / 'p8-qualification900-dynamic-001/status.json')
        failed_power_status=checked(failed_power)
        need(failed_power_status['complete'] is False and failed_power_status['cleanup_complete'] and not failed_power_status['cleanup_errors'], 'old invalid energy observation must remain a terminal negative')
        need(dynamic_spec['prior_failed_power_observation']==failed_power, 'the new observation must preserve its failed predecessor')
        ...
        references = [adapter_ref, hooks_ref, failed_power, ref(__file__), ref(A / 'p8_qualification_audit_v3.py'), fixed_ref, prior_ref,
        ...
        return dict(schema='p8-candidate900-qualified-P6-calibration-v1', passed=True,
            measurement_adapter=adapter_ref, measurement_hooks=hooks_ref, failed_P8_power_observation=failed_power,
            ...
            qualification_auditor=ref(__file__), stage_auditor=ref(A / 'p8_qualification_audit_v3.py'),
