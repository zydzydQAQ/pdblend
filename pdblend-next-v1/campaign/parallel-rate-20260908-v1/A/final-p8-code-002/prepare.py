...
def main():
    ...
    qref = ref(A / 'final-p8-continuation-002/qualification.json')
    q = checked(qref)
    ...
    cap.update(owner_id='ap8final', http_port_base=32300, kv_port_base=62000, max_creations=8,
               runtime_dir=str(A / 'final-p8-002/capacity-runtime'),
    ...
    executor_dir = A / 'dynamic-execution-isolated-power-002'
    files.update(checked(ref(executor_dir / 'manifest.json'))['files'])
    ...
    base.update(isolated_power_adapter=q['measurement_adapter'], measurement_hooks=q['measurement_hooks'], host_release=str(HOST), deadline_s=None, campaign_lifecycle='until_declared_complete_v1',
        ...
        unchanged_pdb_policy=False, formal_eligible=False, output=str(A / 'final-p8-002/results'))
    write(OUT / 'binding.json', base)
    release = dict(schema='parallel-rate-p8-qualified-dynamic-release-v1', approved=True,
        dynamic_qualified=True, measurement_adapter=q['measurement_adapter'], measurement_hooks=q['measurement_hooks'], measurement_executor=ref(executor_dir / 'manifest.json'), production_ready=False, created_s=time.time(), model='14b',
