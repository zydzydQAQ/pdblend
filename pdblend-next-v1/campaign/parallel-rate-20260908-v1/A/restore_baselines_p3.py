...
RELEASE=HERE/'p3-added/fixed-release-001/release.json'
...
def verify_release(path,digest):
    ...
    result=[]
    for name,count in [('p3/fixed-screen-001',13),('p3-added/fixed-screen-001',4)]:
...
def expected():
    ...
    result.update(schema=2,out=str(OUT),operation='restart-retained-residents',previous_binding=str(PREVIOUS),pdb_binding=str(PREVIOUS),
        ...
        required_predecessors=[dict(stage=str(HERE/'p3/fixed-screen-001'),count=13),dict(stage=str(HERE/'p3-added/fixed-screen-001'),count=4)],
