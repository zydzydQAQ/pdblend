...
DECLARATION_SHA = 'd4431f9f1dbf200cda9d3e85eb5b44451e995e26290b6568130077f0aaabd922'
    ...
    for arm, configs in release['configs'].items():
        ...
        for reference in configs.values():
            ...
            p.need(cfg.get('admission_round_fairness') is True and cfg.get('pending_admission_capacity_guard') is True
                   ...
                   and cfg.get('observed_first_admission_frequency_v1') is True,
...
def engineering_assessment(observations, summary, first_admission_gate=False):
    faults = [dict(request_id=r.get('request_id'), http_status=r.get('http_status'), error=r.get('error'))
              for r in observations if str(r.get('http_status')) == '503' or
              'HTTP 503' in (r.get('error') or '') or 'frequency' in (r.get('error') or '').lower()]
    return dict(passed=not faults and (not first_admission_gate or summary.get('work_complete') is True),
                faults=faults)
        ...
        async with aiohttp.ClientSession(trust_env=False) as session:
            ...
            for cell in cells:
                ...
                try:
                    ...
                    bench_path = output / 'cells' / cell['cell_id'] / 'bench.csv'
                    with bench_path.open() as stream:
                        observations = list(csv.DictReader(stream))
                    assessment = engineering_assessment(observations, receipt['summary'], bool(cell.get('first_admission_gate')))
                    faults = assessment['faults']; gate_pass = assessment['passed']
                    p.write(output / 'operations' / cell['cell_id'] / 'engineering-gate.json',
                            dict(passed=gate_pass, faults=faults, first_admission_gate=bool(cell.get('first_admission_gate')),
                                 work_complete=receipt['summary'].get('work_complete'),
                                 all_eight_energy_retained=True, low_slo_does_not_trigger_retry=True), exclusive=True)
                    if not gate_pass:
                        state.update(phase='stopped_engineering_fault', engineering_fault_cell=cell['cell_id'],
                                     engineering_faults=faults, stopped_at_boundary=True)
                        break
                except BaseException as exc:
