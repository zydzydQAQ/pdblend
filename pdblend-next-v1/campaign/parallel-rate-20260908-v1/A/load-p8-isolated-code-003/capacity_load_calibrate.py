...
def local_deadline(spec, local_end, reserve=0):
    total=spec['deadline_s']
    return local_end if total is None else min(local_end,total-reserve)
...
def validate_spec(spec):
    ...
    validate_compatibility(spec, binding)
    require(len(original['instances']) == 2 and config['instances'] == original['instances'],
            'same exact initial two instances required')
    gpus=spec.get('gpus');tp=binding['identity']['tp']
    require(isinstance(gpus,list) and bool(gpus) and len(gpus)==tp
        and all(type(g) is int and 0<=g<8 for g in gpus) and len(set(gpus))==len(gpus),
        'declared extra GPU group must be complete TP, unique integer indices within eight cards')
    retained={g for instance in original['instances'] for g in instance['gpus']}
    require(not set(gpus)&retained,'declared extra GPU group overlaps retained original instances')
    ...
    require(spec['deadline_s'] is None and binding['deadline_s'] is None and spec['campaign_lifecycle']=='until_declared_complete_v1' and binding['campaign_lifecycle']=='until_declared_complete_v1', 'explicit no-total-deadline lifecycle required')
    if spec['mode'] == 'automatic_underload_gate':
        require(config.get('capacity_integration_v1') is True and spec['arm'] == 'dynamic',
                'new gate must use actual autonomous capacity control')
        trace=fixed(spec['trace']);validate_trace(trace,spec['demand_domain_sha256'],duration=60)
        require(trace['n_requests']==752 and spec['trace']==fixed(spec['paired_trace_result'])['trace'],
                'same original752 workload; automatic timing is not the old forced@10 policy')
        require(not spec.get('cycles') and spec.get('manual_restore_at_s') is None,
                'no forced restore or changed arrival is permitted in autonomous gate')
        return original,binding,config

    if spec['mode'] == 'underload_gate':
        require(config.get('capacity_integration_v1') is not True and len(spec['cycles'])==1, 'one manual underload gate required')
        cycle=spec['cycles'][0]
        require(set(cycle)=={'under_load','restore','remove'}, 'exact gate actions required')
        trace=fixed(cycle['under_load']);validate_trace(trace,spec['demand_domain_sha256'],duration=60)
        previous=fixed(spec['paired_failed_result'])
        require(previous['trace']==cycle['under_load'] and trace['n_requests']==752, 'exact original failed752 trace required')
        for key in ('restore','remove'):
            action=fixed(cycle[key]);require(action['operation']==key and action['deadline_s'] is None and action['gpus']==spec['gpus'],'physical action declaration differs')
        return original,binding,config
    if spec['mode'] == 'layout_calibration':
        ...
        traces = []
        if 'matched_idle_duration_s' in spec:
            require(spec['matched_idle_duration_s'] >= 60,'matched real idle requires at least sixty seconds')
        for cycle in spec['cycles']:
            required={'low', 'high2', 'high3', 'under_load', 'restore', 'remove'}
            require(required <= set(cycle) <= required|{'low40'}, 'complete cycle with optional declared second low-rate endpoint required')
            for key in ('low','high2','high3','under_load')+ (('low40',) if 'low40' in cycle else ()):
        ...
        if any('low40' in c for c in spec['cycles']):
            require(all('low40' in c for c in spec['cycles']) and len({c['low40']['sha256'] for c in spec['cycles']})==3,
                'second low endpoint requires three independent paired traces')
    else:
...
def phase_metrics(rows, trace, measurement, native_idle):
    ...
    return dict(n_expected=n, n_rows=len(rows), n_good=good, work_complete=bool(complete and native_idle),
        failed_requests=sum(not bool(r.get('success')) for r in rows),
...
def autonomous_gate_evidence(inventory, dispatched, control, n_expected):
    commits=[e for e in inventory['events'] if e['kind']=='physical_commit']
    require({e['operation'] for e in commits}=={'restore','remove'},
            'actual autonomous growth and measured return required')
    decisions=[e for e in inventory['events'] if e['kind']=='capacity_decision'
        and e.get('proposal') and e['proposal']['action']=='restore']
    require(decisions and any(e['demand']['queued_requests']>0 for e in decisions),
            'autonomous gate needs actually queued work when cold restore is proposed')
    extra=set(inventory['known_instances'])-set(inventory['initial_ids'])
    require(extra and all(inventory['known_instances'][i]['gpus']==[5] for i in extra),
            'only measured GPU5 may be added')
    admissions={e['request_id'] for e in control if e['kind']=='admission'
        and str(e.get('client_request_id','')).isdigit() and 0<=int(e['client_request_id'])<n_expected
        and any(r['decode_id'] in extra for r in e['plan']['routes'])}
    served=[r for r in dispatched if r['instance_id'] in extra and r['request_id'] in admissions]
    require(served,'new GPU must actually serve declared workload, not only correctness probes')
    return dict(passed=True,initial_instances=2,peak_instances=3,actual_autonomous_restore=True,
        actual_measured_return=True,added_instances=sorted(extra),main_native_requests_on_added=len(served),
        main_request_ids=sorted({r['request_id'] for r in served}),manual_restore_used=False)
...
async def measure_phase(controller, service, trace_ref, out, spec, *, action=None):
    ...
    try:
        ...
        await native_idle(controller,local_deadline(spec,epoch+trace['duration_s']+120,120))
        # Preserve the exact client rows even if later physical drain proof fails.
        durable(out/'requests.json',rows)
        if spec['mode'] in ('qualification900', 'automatic_underload_gate'):
            result['dynamic_drain'] = await controller.finish_measurement(
                local_deadline(spec,epoch+trace['duration_s']+120,60))
        ...
        for pending in (work,physical):
            if pending is not None and not pending.done():
                try:
                    await asyncio.wait_for(asyncio.shield(pending),max(.001,local_deadline(spec,time.time()+120,180)-time.time()))
...
async def measure_idle(controller, out, spec, duration_s):
    """Observe actual idle power without inventing a request or an SLO value."""
    from capacity_backend import TransitionMeter
    require(duration_s >= 60, 'idle evidence requires at least sixty seconds')
    require(spec['deadline_s'] is None, 'cancelled global deadline must remain absent; idle keeps finite own duration')
    from ecopadg.serving.completion_policy import engine_residual
    before=sorted((dict(id=i['id'],gpus=i['gpus']) for i in controller.backend.instances.values()),key=lambda r:r['id'])
    meter=await TransitionMeter(out/'raw').start()
    out.mkdir(parents=True,exist_ok=True)
    native_log=(out/'idle-native.jsonl').open('x',buffering=1)
    async def observe():
        require(not controller.active and not controller.request_tasks,'request entered idle observation')
        rows=await asyncio.gather(*(controller.backend.json(i,'/runtime') for i in controller.backend.instances))
        now=time.time()
        require(len(rows)==len(before) and not any(engine_residual(r,now) for r in rows),
            'actual engine work or unobserved state during idle observation')
        native_log.write(json.dumps(dict(at_s=now,raw=rows))+'\n');native_log.flush();os.fsync(native_log.fileno())
    result=dict(schema='capacity-load-measurement-v1',phase=out.name,phase_kind='idle',complete=False,
        n_expected=0,n_rows=0,n_good=0,slo_attainment=None,empirical_sustainable_sample=False,
        resident_before=before,source=dict(original_binding=spec['original_binding'],
        capacity_binding=spec['capacity_binding'],config=spec['config'],
        host_manifest=ref(Path(spec['host_release'])/'manifest.json')))
    try:
        require(not controller.active and not controller.request_tasks,'idle starts with active work')
        await observe()
        start=time.time();end=start+duration_s
        result['actual_idle_start_s']=start
        while time.time()<end:
            require(not controller.active and not controller.request_tasks,'request entered idle observation')
            await observe()
            await asyncio.sleep(min(.5,max(0.,end-time.time())))
        await observe()
        result['actual_idle_end_s']=time.time()
        after=sorted((dict(id=i['id'],gpus=i['gpus']) for i in controller.backend.instances.values()),key=lambda r:r['id'])
        require(before==after,'idle layout changed')
        durable(out/'requests.json',[])
        result.update(complete=True,work_complete=True,native_idle=True,resident_after=after,
            resident_groups=sorted(r['gpus'] for r in before),
            measured_idle_duration_s=result['actual_idle_end_s']-start,
            declared_idle_duration_s=duration_s,zero_work_is_not_capacity_evidence=True)
    except BaseException as exc:
        result['error']=repr(exc);raise
    finally:
        native_log.close()
        measured=await meter.finish();result['raw_measurement']=measured['receipt']
        result.update(energy_j=measured['energy_j'],bounds_are_empirical_not_hard_guarantees=True,
            artifacts={str(p):sha(p) for p in out.iterdir() if p.is_file()})
        durable(out/'result.json',result)
    require(measured['measurement_valid'],'idle whole-node power invalid')
    return result
...
async def execute(spec,out):
    ...
    def boundary(name,reserve):
        ...
        require(spec['deadline_s'] is None,'no total deadline; each workload and cleanup remain locally bounded')
        update(phase=name)
    update()
    ...
    try:
        # The retained original engine binding can contain an earlier queue's
        # deadline; this independent declaration owns the current invocation.
        execution_binding=dict(original,deadline_s=None,campaign_lifecycle='until_declared_complete_v1')
        durable(out/'execution-binding.json',execution_binding)
        common.validate_binding(execution_binding)
        async with aiohttp.ClientSession(trust_env=False) as preflight:
            durable(out/'identity.before.json',await common.identity(preflight,execution_binding))
        await runner.setup()
        await web.TCPSite(runner,'127.0.0.1',config['port']).start()
        service = (getattr(controller,'capacity_service',None) if spec['mode'] in ('qualification900','automatic_underload_gate')
        ...
        if spec['mode'] == 'automatic_underload_gate':
            boundary('automatic_underload_gate',180)
            result=await measure_phase(controller,service,spec['trace'],out/'automatic_underload_gate',spec)
            state['completed'].append(ref(out/'automatic_underload_gate/result.json'));update()
            require(result.get('work_complete') is True and result.get('failed_requests')==0
                and result.get('request_timeouts')==0, 'autonomous gate request failure prevents successor')
            await controller.journal.flush()
            control=[json.loads(line) for line in (out/'control.jsonl').open()]
            evidence=autonomous_gate_evidence(service.inventory.value,dispatched,control,result['n_expected'])
            durable(out/'autonomous-gate-evidence.json',evidence)
        elif spec['mode'] == 'underload_gate':
            cycle=spec['cycles'][0];boundary('underload_gate',420)
            result=await measure_phase(controller,service,cycle['under_load'],out/'underload_gate',spec,action=cycle['restore'])
            state['completed'].append(ref(out/'underload_gate'/'result.json'));update()
            require(result.get('work_complete') is True and result.get('failed_requests')==0 and result.get('request_timeouts')==0,'underload gate incomplete or request failure stops qualification')
            extra=[i for i in controller.backend.instances.values() if i['id'] not in service.inventory.value['initial_ids']]
            require(len(extra)==1,'exactly one owned gate replica required')
            result=await service.executor.calibrate('remove',tuple(spec['gpus']),remove_id=extra[0]['id'],declaration=cycle['remove'])
            durable(out/'remove.json',result)
        elif spec['mode'] == 'layout_calibration':
            for index,cycle in enumerate(spec['cycles'],1):
                if 'matched_idle_duration_s' in spec:
                    name=f'cycle-{index}-idle-layout2';boundary(name,spec['matched_idle_duration_s'])
                    require(len(controller.backend.instances)==2,'idle source layout differs')
                    await measure_idle(controller,out/name,spec,spec['matched_idle_duration_s'])
                    state['completed'].append(ref(out/name/'result.json'));update()
                phases=[(2,'low')]+([(2,'low40')] if 'low40' in cycle else [])+[(2,'high2'),(None,'under_load'),(3,'high3'),(3,'low')]+([(3,'low40')] if 'low40' in cycle else [])
                for layout,key in phases:
                ...
                if 'matched_idle_duration_s' in spec:
                    name=f'cycle-{index}-idle-layout3';boundary(name,spec['matched_idle_duration_s'])
            ...
            result=await measure_phase(controller,service,spec['trace'],out/'qualification900',spec)
            state['completed'].append(ref(out/'qualification900'/'result.json'));update()
            require(result.get('work_complete') is True and result.get('failed_requests')==0
                and result.get('request_timeouts')==0, '900s request failure/incomplete work prevents any successor')
        await native_idle(controller,local_deadline(spec,time.time()+120,60))
        if service is not None:
        ...
        if service is not None:
            try:
                ...
                await native_idle(controller,local_deadline(spec,time.time()+120,30))
                await service.finish_to_initial()
            except BaseException as exc:
        ...
        # Historical control failures stay visible. Retained-engine restoration
        # has its own strict physical-idle/identity/resource gate and always runs.
        from retained_cleanup import restore_retained
        try:
            restoration = await restore_retained(controller,service,common,original,out,
                capacity_cleanup_complete=not any(e.startswith('capacity cleanup:') for e in errors))
            update(retained_restoration=ref(out/'retained-restoration.json'),
                   retained_restoration_complete=restoration['passed'])
        except BaseException as exc:
        ...
        if errors or outer.get('measurement_valid') is not True:
            update(complete=False,phase='needs_attention',full_operation_measurement_valid=outer.get('measurement_valid'))
        require(not errors and outer.get('measurement_valid') is True,'calibration whole-operation measurement or cleanup requires attention')
...
def main():
    ...
    adapter=fixed(spec['measurement_adapter'])
    require(all(sha(p)==h and spec['files'].get(p)==h for p,h in adapter['files'].items()),'exact isolated sampler adapter required')
    require(spec['files'].get(spec['measurement_adapter']['path'])==spec['measurement_adapter']['sha256'],'isolated adapter manifest must be frozen')
    hooks=fixed(spec['measurement_hooks_manifest'])
    require(all(sha(p)==h and spec['files'].get(p)==h for p,h in hooks['files'].items()),'exact measurement hooks required')
    require(spec['files'].get(spec['measurement_hooks']['path'])==spec['measurement_hooks']['sha256'],'measurement hook must be frozen')
    ...
    with node_lease():
        ...
        finish_observers=install(args.out/'isolated-samplers',ref(host/'manifest.json'),spec['measurement_adapter'],spec['measurement_hooks'])
        try:
