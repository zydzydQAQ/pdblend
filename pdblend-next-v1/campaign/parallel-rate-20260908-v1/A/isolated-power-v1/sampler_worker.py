...
def execute(spec):
 ...
 try:
  while True:
   ...
   available=len(sampler.samples)
   if not sampler.error and not stopping.is_set():
    available=min(available,len(sampler.utilization_samples),len(sampler.frequency_samples) if spec['sample_clocks'] else available)
