...
def validate_spec(spec):
    ...
    if spec['mode'] == 'idle_calibration':
        require(config.get('capacity_integration_v1') is not True and len(spec['cycles'])==1
            and spec.get('matched_idle_duration_s')==60, 'one explicitly declared sixty-second idle pair required')
        cycle=spec['cycles'][0]
        require(set(cycle)=={'restore','remove'}, 'idle-only declaration must not contain request traces')
        for key in ('restore','remove'):
            action=fixed(cycle[key]);require(action['operation']==key and action['deadline_s'] is None
                and action['gpus']==spec['gpus'], 'idle physical action declaration differs')
...
async def measure_idle(controller, out, spec, duration_s):
    ...
    async def observe():
        ...
        require(last_sample_s is None or 0 < now-last_sample_s <= 1., 'native idle sample gap exceeds original one-second qualification')
        last_sample_s=now
    ...
    try:
        ...
        while time.time()<end:
            require(not controller.active and not controller.request_tasks,'request entered idle observation')
            await observe()
            await asyncio.sleep(min(.2,max(0.,end-time.time())))
        await observe()
        require(last_sample_s is not None and last_sample_s>=end, 'post-window native sample must bracket the declared sixty seconds')
        result['declared_idle_end_s']=end
        result['post_window_native_sample_s']=last_sample_s
...
async def execute(spec,out):
    ...
    try:
        ...
        elif spec['mode'] == 'idle_calibration':
            # Replaces only the affected declared pair. There is no request
            # load or new throughput/capacity observation in these windows.
            cycle=spec['cycles'][0]
            name='cycle-1-idle-layout2';boundary(name,60)
            require(len(controller.backend.instances)==2, 'idle pair must start on the original two replicas')
            result=await measure_idle(controller,out/name,spec,60)
            state['completed'].append(ref(out/name/'result.json'));update()
            require(result.get('work_complete') is True, 'incomplete idle source prevents physical successor')
            boundary('cycle-1-restore',360)
            result=await service.executor.calibrate('restore',tuple(spec['gpus']),declaration=cycle['restore'])
            durable(out/'cycle-1-restore.json',result)
            name='cycle-1-idle-layout3';boundary(name,60)
            require(len(controller.backend.instances)==3, 'actual cold restore must establish three replicas')
            result=await measure_idle(controller,out/name,spec,60)
            state['completed'].append(ref(out/name/'result.json'));update()
            require(result.get('work_complete') is True, 'incomplete idle target prevents further measurement')
            boundary('cycle-1-remove',360)
            extra=[i for i in controller.backend.instances.values() if i['id'] not in service.inventory.value['initial_ids']]
            require(len(extra)==1, 'exactly one owned idle-pair replica must be removed')
            result=await service.executor.calibrate('remove',tuple(spec['gpus']),remove_id=extra[0]['id'],declaration=cycle['remove'])
            durable(out/'cycle-1-remove.json',result)
