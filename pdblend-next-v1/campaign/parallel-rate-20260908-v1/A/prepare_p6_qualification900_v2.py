...
def main():
 out=A/'p6-qualification900-inputs-002';require(not out.exists(),'new900 declaration required')
 certref=ref(A/'alpaca-capacity-certificate-p6-002/certificate.json');cert=fixed(certref)
 ...
 out.mkdir()
 cap.update(calibration=certref,calibration_only=False,production_ready=False,owner_id='ap6q900',http_port_base=31200,kv_port_base=62000,max_creations=8,runtime_dir=str(A/'p6-qualification900-dynamic-002/runtime'),rate_observation_window_s=60.,arrival_count_margin=2.,policy={})
 for path in [*(A/'alpaca-capacity-certificate-p6-002').glob('*.json'),A/'register_alpaca_capacity_p6_idle.py']:
  cap['files'][str(path)]=sha(path)
 cap['files'][str(HOST/'manifest.json')]=sha(HOST/'manifest.json');cap['files'][str(cpu_path)]=sha(cpu_path);cap['files'][str(A/'prepare_p6_qualification900_v2.py')]=sha(A/'prepare_p6_qualification900_v2.py')
