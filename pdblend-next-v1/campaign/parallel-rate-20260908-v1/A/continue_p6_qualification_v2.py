...
OUT=A/'p6-qualification-continuation-002'
...
def main():
 ...
 sources=[Path(__file__).resolve(),Path(idle_result.__code__.co_filename).resolve(),A/'register_alpaca_capacity_p6.py',A/'prepare_p6_qualification900.py',CODE/'capacity_load_calibrate.py',A/'load-p6-full-inputs-002/spec.json'];pins={str(p):sha(p) for p in sources};durable(OUT/'source-manifest.json',pins)
 ...
 try:
  while True:
   ...
   time.sleep(3)
  audit(A/'load-p6-full-002',27,ref(A/'load-p6-full-inputs-002/spec.json'));run('register',[sys.executable,str(A/'register_alpaca_capacity_p6.py')]);run('prepare900',[sys.executable,str(A/'prepare_p6_qualification900.py')])
