...
import socket
import sys
sys.path.insert(0,'/root/workspace/pdblend-next-v1/campaign/AC-baseline-binding-v2')
from gate_evidence import audit,files,read,require,sha

ROOT=Path('/root/workspace/pdblend-next-v1/campaign/AC-baseline-binding-v2')
...
async def build(a):
    ...
    system='ecoserve' if not a.strategy else 'dynamollm' if a.strategy.startswith('dynamollm') else a.strategy
    expected_hosts={'mixed': '/root/workspace/pdblend-next-v1/releases/five-system100-C7B-baseline-v1-runtime', 'distserve': '/root/workspace/pdblend-next-v1/releases/five-system100-C7B-baseline-v1-runtime', 'dynamollm': '/root/workspace/pdblend-next-v1/releases/five-system100-C7B-baseline-cooperative-v1-runtime', 'ecoserve': '/root/workspace/pdblend-next-v1/releases/five-system100-A14B-baseline-eco-drain-v1-runtime'}
    ...
    binding=dict(schema=1,protocol_id=spec['protocol_id'],model=spec['model'],system=canonical,implementation_variant=a.strategy or 'correctness-only',
        hostname=spec['hostname'],deadline_s=spec['deadline_s'],campaign_lifecycle=spec['campaign_lifecycle'],host_release=spec['host_release'],output=str(output),configs=configs,
