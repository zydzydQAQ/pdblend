...
def validate(spec):
    assert spec['schema'] == 'new-A-fixed14B-qualification-spec-v1'
    ...
    assert [list(v) for v in shapes(profile)] == spec['shapes'] and len(spec['shapes']) == 54
        ...
        async with aiohttp.ClientSession(trust_env=False) as session:
            try:
                ...
                for instance in binding['instances']:
                    ...
                    await owner.set(tuple(instance['gpus']), 2520, verify_rise=False)
                    refs = {}
...
def main():
    ...
    validate(spec)
    if not args.run:
        print(json.dumps(dict(passed=True, cpu_only=True, shape_count=108, GPU_work_started=False)))
        return
    ...
    assert state['passed']
    result = dict(schema='new-A-fixed14B-qualification-v1', status=p.ref(args.out / 'status.json'),
                  ...
                  datasets=['sharegpt', 'longbench'], dynamic_capacity_qualification=False)
