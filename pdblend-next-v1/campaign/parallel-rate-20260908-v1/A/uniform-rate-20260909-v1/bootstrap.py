...
def validate(spec):
    assert spec['schema'] == 'new-A-native14B-bootstrap-v1'
    assert spec['node'] == 'Anew20260909' and spec['model'] == '14b'
    assert all(p.sha(path) == digest for path, digest in spec['files'].items())
    for key in ('node_identity', 'model_manifest', 'engine_template', 'numerical_reference'):
        checked(spec[key])
    environment = checked(spec['environment_preflight'])
    assert environment['passed'] and not environment['errors']
    assert environment['checks']['model_files']['passed']
    assert environment['checks']['model_files']['manifest_sha256'] == spec['model_manifest']['sha256']
    assert environment['checks']['node_identity']['hostname'] == 'iZwz9274emxme9019d2sjgZ'
    assert spec['image'] == 'sha256:0bb51d143b7fcaaea2e794dd6e207cf4165a4f21522a2e932a4bd4a117074bc2'
    assert spec['engine_module'] == 'ecopadg.serving.engine'
            ...
            async with aiohttp.ClientSession(trust_env=False) as session:
                for plan in spec['instances']:
                    ...
                    argv = ['docker', 'run', '-d', '--name', instance['container_name'], '--gpus', 'all',
                            ...
                            '--label', 'pdblend.uniform.owner=Anew20260909-bootstrap']
                ...
                for instance in state['instances']:
                    for case in oracle['cases']:
                        ...
                        replies.append(dict(instance_id=instance['id'], prompt_length=case['prompt_length'],
...
def main():
    ...
    state = dict(schema='new-A-native14B-bootstrap-status-v1', node='Anew20260909', hostname=socket.gethostname(),
