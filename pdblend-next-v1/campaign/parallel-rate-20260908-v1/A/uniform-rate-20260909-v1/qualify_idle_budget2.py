"""Qualify the existing P12 idle-domain recovery on both new-A native engines."""
import argparse
import asyncio
import copy
import json
...
async def execute(spec, out, state):
    ...
    prior = verify_runtime_v3.verify(spec['previous_qualification'])
    previous_binding = b.checked(prior['binding'])
    common = b.load(spec['common_executor']['path'], 'newA_idle_common')
    with node_lease():
        p.actual_identity()
        ...
        for dataset, path in previous_binding['configs'].items():
            ...
            config['idle_domain_reacquire_timeout_s'] = 2.0
        ...
        async with aiohttp.ClientSession(trust_env=False) as session:
            try:
                ...
                for instance in binding['instances']:
                    ...
                    cfg.update(instances=[instance], port=34760, prepare_peers=False,
                    ...
                    stream.session, stream.args, stream.issued = session, SimpleNamespace(port=34760, seed=0), set()
                    ...
                    try:
                        await runner.setup()
                        await web.TCPSite(runner, '127.0.0.1', 34760).start()
...
def main():
    ...
    state = dict(schema='new-A-idle-domain-qualification-status-v1', node='Anew20260909', model='14b',
    ...
    b.save(args.out / 'qualified.json', dict(schema='new-A-fixed14B-idle-qualification-v1',
        previous_qualification=spec['previous_qualification'], previous_validator=p.ref(p.HERE / 'verify_runtime_v3.py'),
