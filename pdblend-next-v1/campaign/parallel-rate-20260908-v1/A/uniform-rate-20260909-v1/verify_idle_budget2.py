...
def verify(reference):
    ...
    assert qualification['schema'] == 'new-A-fixed14B-idle-qualification-v1'
    assert all(p.sha(path) == digest for kind in ('files', 'source_files')
               for path, digest in qualification[kind].items())
    prior = verify_runtime_v3.verify(qualification['previous_qualification'])
    ...
    for dataset, path in binding['configs'].items():
        ...
        expected.update(idle_domain_reacquire_v1=True, idle_domain_reacquire_timeout_s=2.0, journal=str(out / 'unused.jsonl'))
        ...
        for probe in [r for r in state['probes'] if r['instance_id'] == iid]:
            ...
            if probe['cycle']:
                ...
                assert actual and all(e['confirmed'] and e['confirmed_within_idle_recovery_bound']
                    ...
                    and e['idle_domain_reacquire_timeout_s'] == 2.0 and e['original_active_settle_timeout_s'] == .3
