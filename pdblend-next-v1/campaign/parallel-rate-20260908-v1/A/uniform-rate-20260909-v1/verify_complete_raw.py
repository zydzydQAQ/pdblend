...
def verify(reference):
    ...
    for case in cases:
        ...
        for row, clock in zip(case['requests'], case['loaded_clocks']):
            ...
            complete_clock = q.clock_window(clocks, instance['gpus'], frequency, row['token_received_s'][0],
                                            row['token_received_s'][-1])
            # The isolated worker can flush its final in-window row after the online
            # snapshot. Audit every finalized row, retain identical window/domain,
            # and require that no online sample disappeared from final evidence.
            assert {k: v for k, v in complete_clock.items() if k != 'samples'} == {k: v for k, v in clock.items() if k != 'samples'}
            assert complete_clock['samples'] >= clock['samples']
