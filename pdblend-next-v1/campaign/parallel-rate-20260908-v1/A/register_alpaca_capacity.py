...
sys.path.insert(0,str(A/'load-code-until-complete-001'))
...
def main():
 src=A/'load-alpaca-calibration-001';out=A/'alpaca-capacity-certificate-001'
 ...
 durable(out/'qualification.json',dict(passed=True,created_s=time.time(),certificate=reference,full_operation=state['full_operation_measurement'],source_status=ref(src/'status.json'),source_inventory=ref(src/'inventory.json'),requires_actual_900s_p5_validation=True,production_ready=False,source_scope='measured P4 parent strategy; P5 extension must prove parent equivalence and complete actual900s'))
