...
def validate_isolated_inputs():
 ...
 declaration_ref=dict(path=str(A/'p8-qualification900-inputs-003/declaration.json'),sha256='22785f7f0620d31a012948428f7e96c94db75201d00f0583e305a42d130bc113')
 declaration=fixed(declaration_ref);sr=declaration['specs']['dynamic'];sp=fixed(sr)
 require(sr==dict(path=str(A/'p8-qualification900-inputs-003/spec.json'),sha256='fb71371e9d7cfb438c3bedc61af49d62b49d8ed334d9376c4d194fd25463f4b8'),'actual isolated900 input differs')
 driver_ref=dict(path=str(A/'load-p8-isolated-code-003/manifest.json'),sha256='0ec12e26ec2832448096c910d3b41b4847bc834e4ec7be80a0c078b0373bf5d4')
 driver=fixed(driver_ref)
 require(all(sha(p)==h and sp['files'].get(p)==h for p,h in driver['files'].items()),'isolated900 actual driver is not frozen')
 require(sp['measurement_adapter']==dict(path=str(isolated.ADAPTER),sha256=isolated.ADAPTER_SHA)
   and sp['measurement_hooks']==dict(path=str(isolated.HOOK),sha256=isolated.HOOK_SHA)
   and sp['measurement_hooks_manifest']==dict(path=str(isolated.EXECUTOR),sha256=isolated.EXECUTOR_SHA),'isolated measurement package differs')
 for reference in (sp['measurement_adapter'],sp['measurement_hooks'],sp['measurement_hooks_manifest']):
  require(sp['files'].get(reference['path'])==reference['sha256'] and sha(reference['path'])==reference['sha256'],'actual measurement dependency is unbound')
 require(declaration['authorized'] is True and declaration['arrival_duration_s']==900 and declaration['original_request_deadline_s']==120
   and declaration['trace']==sp['trace'] and declaration['profile']==sp['profiles']
   and declaration['source']==ref(Path(sp['host_release'])/'manifest.json'),'isolated900 scientific scope changed')
 isolated.sources()
 return declaration_ref,declaration,sr,sp,driver_ref


def audit_isolated_stage(out,sp):
 import isolated_measurement_audit_v1 as isolated
 out=Path(out);host=ref(Path(sp['host_release'])/'manifest.json')
 terminal_path=out/'isolated-observers-terminal.json';terminal=fixed(ref(terminal_path))
 require(terminal['complete'] is True and terminal['errors']==[]
   and terminal['measurement_adapter']==sp['measurement_adapter'],'not every isolated observer exited cleanly')
 all_audit=isolated.audit_samplers(terminal['isolated_samplers'],host,artifacts=terminal['artifacts'])
 require(all_audit['artifacts']==terminal['artifacts'],'isolated terminal includes omitted or extra artifacts')
 actual_roots={str(p) for p in (out/'isolated-samplers').glob('sampler-*') if p.is_dir()}
 require(actual_roots==set(all_audit['raw_values']),'actual stage observer omitted from terminal')
 status=fixed(ref(out/'status.json'));result=fixed(status['completed'][0]);inventory=fixed(ref(out/'inventory.json'))
 measurements=[status['full_operation_measurement'],result['raw_measurement']]
 measurements.extend(e['receipt'] for e in inventory['events'] if e['kind']=='transition_measurement')
 require(len({r['path'] for r in measurements})==len(measurements),'duplicate phase/transition measurement reference')
 matched=[];used=set()
 for reference in measurements:
  measured=fixed(reference)
  require(measured['measurement_valid'] is True and measured['measurement_adapter']==sp['measurement_adapter']
    and len(measured['isolated_samplers'])==1,'actual measurement lacks exactly one qualified observer')
  local=isolated.audit_samplers(measured['isolated_samplers'],host,artifacts=measured['artifacts'])
  require(set(local['raw_values'])<=actual_roots,'phase observer not in actual terminal')
  match=isolated.match_power_directory(Path(reference['path']).parent,local,artifacts=measured['artifacts'])
  require(match['isolated_directory'] not in used,'same observer attributed to multiple independent measurements')
  used.add(match['isolated_directory']);matched.append(dict(measurement=reference,**match))
 require(used==actual_roots,'isolated observer lacks original raw measurement counterpart')
 return dict(terminal=ref(terminal_path),measurement_adapter=sp['measurement_adapter'],hook_source=sp['measurement_hooks'],
   all_observers_exited=True,all_IPC_streams_independently_reconstructed=True,
   all_original_power_metadata_utilization_clock_rows_exact=True,observers=all_audit['proofs'],measurements=matched,
   verifier=ref(Path(isolated.__file__)))


def audit900():
 declaration_ref,declaration,sr,sp,driver_ref=validate_isolated_inputs()
 out=A/'p8-qualification900-dynamic-002'
 result=audit_stage(out,sr,'900')
 result['isolated_measurement_audit']=audit_isolated_stage(out,sp)
 result['measurement_adapter']=sp['measurement_adapter'];result['measurement_driver']=driver_ref
 result['qualification_declaration']=declaration_ref
 result['files'].update({declaration_ref['path']:declaration_ref['sha256'],driver_ref['path']:driver_ref['sha256']})
 verifier=result['isolated_measurement_audit']['verifier'];result['files'][verifier['path']]=verifier['sha256']
 return result
