 ...
 with node_lease():
  ...
  async with aiohttp.ClientSession(trust_env=False,connector=aiohttp.TCPConnector(limit=0)) as session:
   try:
    ...
    await asyncio.gather(*(reference(i) for i in binding['instances']))
    p.save(a.out/'status.json',s)
    for target in (900,1500,2100):
     ...
     async def instance_cases(i):
      ...
      for frequency,length,batch in [v for v in shapes(profile,i['tp']) if v[0]==target]:
       ...
       s['active_cases'][i['id']]=current;p.save(a.out/'status.json',s)
       rows=await asyncio.gather(*(stream.request(length,64) for _ in range(batch)))
       case=dict(current,requests=rows,native_after=await common.wait_idle(session,i));s['cases'].append(case);p.save(a.out/'status.json',s)
       for row in rows:check(row,length,refs[i['id']][length])
       case['loaded_clocks']=[clock_window(sampler.frequency_samples,i['gpus'],target,r['token_received_s'][0],r['token_received_s'][-1]) for r in rows]
       s['active_cases'].pop(i['id']);p.save(a.out/'status.json',s)
     ...
     s['idle_wakeup'].append(dict(frequency=target,start_s=start,polls=polls,requests=rows));p.save(a.out/'status.json',s)
    if a.retain_weights:
     ...
     s['retained_weights']=dict(instance_id=i['id'],native_before=raw,payload=payload,started_s=time.time());p.save(a.out/'status.json',s)
     async with session.post(i['url']+'/retain_weights',json=payload,timeout=aiohttp.ClientTimeout(total=620)) as response:
      body=await response.json();s['retained_weights'].update(http_status=response.status,response=body,finished_s=time.time());p.save(a.out/'status.json',s)
     ...
     resumed=await common.resume(session,dict(i,role='mixed'));s['retained_weights']['resumed']=resumed;p.save(a.out/'status.json',s)
    s['passed']=True
   finally:
    for i in binding['instances']:
    ...
    s['passed']=s['passed'] and s['measurement_valid'];p.save(a.out/'status.json',s)
