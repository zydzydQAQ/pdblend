...
HERE=Path(__file__).resolve().parent;U=HERE.parents[1];ROOT=U.parents[1]
...
def main():
 ...
 try:
  ...
  for system,reference in bindings.items():
   ...
   child([sys.executable,'-B',definition['meter_binding']['path'],'--binding',reference['path'],'--out',str(destination),'--native-validator',str(HERE/'verify.py')],out,state)
