 ...
 with node_lease():
  ...
  async with aiohttp.ClientSession(trust_env=False,connector=aiohttp.TCPConnector(limit=0)) as session:
   try:
    ...
    # Prior loaded-clock/work evidence is independently replayed before this sole export.
    if a.retain_weights:
...
def main():
 parser=argparse.ArgumentParser();parser.add_argument('--binding',type=Path,required=True);parser.add_argument('--profile',type=Path,required=True);parser.add_argument('--out',type=Path,required=True);parser.add_argument('--retain-weights',action='store_true');parser.add_argument('--previous-frequency',type=Path,required=True);parser.add_argument('--run',action='store_true');a=parser.parse_args()
 p.need(a.run and a.retain_weights and not a.out.exists(),'fresh explicit retention-only recovery')
 prior=p.load(HERE/'verify_prior.py','new_A_prior_complete_frequency').verify(a.previous_frequency,p.ref(a.binding),p.ref(a.profile))
 s=dict(prior_frequency_proof=prior,source_manifest=p.ref(HERE/'source-files.json'),schema='fresh-legacy-retention-only-recovery-v1',passed=False,complete=False,node_lease_held=False,references=[],cases=[],active_cases={},idle_wakeup=[],cleanup_errors=[],cost_values_recalibrated=False)
