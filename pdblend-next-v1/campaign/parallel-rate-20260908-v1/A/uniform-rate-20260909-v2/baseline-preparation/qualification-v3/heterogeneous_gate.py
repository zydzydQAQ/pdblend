...
async def execute(args,binding):
    ...
    async with aiohttp.ClientSession(trust_env=False) as session:
        try:
            ...
            started=time.time();clocks=await asyncio.to_thread(ClockOwner,hardware,tuple(range(8)),max_frequency=2100)
            await asyncio.wait_for(clocks.set(range(8),2100,verify_rise=False),15)
