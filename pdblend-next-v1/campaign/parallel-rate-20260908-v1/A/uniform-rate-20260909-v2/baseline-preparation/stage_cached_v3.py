...
def main():
 ...
 try:
  ...
  for system,reference in bindings.items():
   ...
   child([sys.executable,'-B',definition['cache_helper']['path'],'--qualification',reference['path'],'--validator',str(HERE/'verify.py'),'--out',str(cache_out)],out,state)
   cached_validator=p.ref(cache_out/'verify.py')
   state.setdefault('qualification_caches',{})[system]=dict(cache=p.ref(cache_out/'cache.json'),validator=cached_validator)
