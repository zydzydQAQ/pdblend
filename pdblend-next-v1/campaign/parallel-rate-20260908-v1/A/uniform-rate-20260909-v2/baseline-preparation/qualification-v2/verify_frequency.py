...
def verify(out,binding_ref,profile_ref):
 ...
 p.need(s['passed'] and s['complete'] and s['finished_s'] and not s.get('error') and not s['node_lease_held'] and not s['cleanup_errors'],'terminal physical candidate gate required')
 ...
 retained=s.get('retained_weights')
 if retained:
 ...
 return dict(passed=True,independently_recomputed=True,cases=len(expected),requests=sum(c['batch'] for c in s['cases']),native_idle_wakeup_cycles=3,frequency_domain_mhz=[900,1500,2100],loaded_tolerance_mhz=15,physical=physical,historical_profile_costs_recalibrated=False,files=audit.files(out))
