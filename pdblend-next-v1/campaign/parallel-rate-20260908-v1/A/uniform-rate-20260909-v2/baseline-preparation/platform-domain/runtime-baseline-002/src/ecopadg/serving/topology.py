...
class InstanceSpec:
    ...
    def endpoint(self):
        return dict(id=self.instance_id,tp=self.tp,gpus=list(self.gpus),role=self.role,
                    url=f'http://127.0.0.1:{self.port}',port=self.port,kv_port=self.kv_port)
...
def validate_layout(specs,node_gpus):
    ...
    used=set();ports=set();ids=set()
    for spec in specs:
        if (spec.tp not in (1,2,4,8) or len(spec.gpus)!=spec.tp or len(set(spec.gpus))!=spec.tp
                or not set(spec.gpus)<=set(node_gpus) or used.intersection(spec.gpus)
                or spec.instance_id in ids or spec.role not in ('mixed','prefill','decode')
                or not spec.instance_id or any(c not in 'abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789_-' for c in spec.instance_id)):
        ...
        ports.update(own_ports);used.update(spec.gpus);ids.add(spec.instance_id)
...
def retained_cache_available(root):
    ...
    try:
        ...
        ranks=manifest['ranks'];tp=manifest['tp']
        return bool(manifest.get('complete') and manifest.get('schema')==1 and tp in (1,2,4,8)
            and len(ranks)==tp and {r['rank'] for r in ranks}==set(range(tp))
            and manifest.get('model_config_sha256') and all(
                (root/r['file']).resolve().is_relative_to(root) and (root/r['file']).is_file()
                and (root/r['file']).stat().st_size>0 and len(r['sha256'])==64 for r in ranks))
...
class DockerLifecycle:
    def __init__(self,root,image,engine_template,*,prefix='pdb-v2-',parallelism=2):
        ...
        self.image=image;self.template=dict(engine_template);self.prefix=prefix
        if prefix!='pdb-v2-':
    ...
    async def start(self,spec,peers,retained_weights=None):
        config=dict(self.template,id=spec.instance_id,tp=spec.tp,port=spec.port,kv_port=spec.kv_port,
                    role=spec.role,initial_generation=spec.generation,peers=peers,
                    retained_weights=retained_weights)
        entry=self.template.get('observation_engine_entry')
        ...
        return await self.command('run','-d','--name',self.prefix+spec.instance_id,
            '--gpus','all','--network','host','--ipc','host',
            '-e','CUDA_VISIBLE_DEVICES='+','.join(map(str,spec.gpus)),
            '-e','NCCL_P2P_DISABLE=1','-e','NCCL_SHM_DISABLE=1','-e','NCCL_IB_DISABLE=1',
            '-e','NCCL_CUMEM_ENABLE=0','-e','NCCL_DEBUG=WARN',
            '-e','PYTHONPATH=/root/workspace/pdblend/src','-e','VLLM_HOST_IP=127.0.0.1',
            '-v','/root/workspace:/root/workspace','-v','/root/workspace/models:/models:ro',
            self.image,*launch,'--config',str(path))
    ...
    async def stop(self,spec):
        ...
        try:
            ...
            if 'No such object' in str(exc) or 'No such container' in str(exc):
                return
            raise
        # Save lifecycle evidence before removing only our owned container.
...
class TopologyManager:
    def __init__(self,backend,lifecycle,specs,node_gpus,journal,*,freeze,commit):
        ...
        self.specs={s.instance_id:s for s in specs};self.node_gpus=tuple(node_gpus)
        self.freeze,self.commit=freeze,commit
        self.lock=asyncio.Lock();self.version=0
    ...
    async def ready(self,spec,timeout=180):
        ...
        while True:
            try:
                ...
                if (not state.get('error') and state.get('transport_healthy',True)
                        and state.get('generation')>=spec.generation and state.get('total_kv_tokens',0)>0):
                    return state
    ...
    async def verify(self,spec):
        ...
        if state['role']!='mixed':
            state=await self.request(spec,'/control',dict(generation=generation+1,
                role='mixed',mode='continuous',admit_prefill=True))
        ...
        if state['role']!=spec.role:
            await self.request(spec,'/control',dict(generation=state['generation']+1,
                role=spec.role,mode='continuous',admit_prefill=True))
        return dict(instance_id=spec.instance_id,token_ids=output['token_ids'],
                    validation='runtime output-count check; numerical cache validation is a separate gate')
    ...
    async def reconfigure(self,remove_ids,add_specs,*,savings_lower_j,cost_upper_j,retained_weights=None,capacity_recovery=None):
        ...
        capacity_ok=bool(recovery.get('source_sha256') and all(math.isfinite(recovery.get(k,float('nan')))
            for k in ('current_rps','required_rps','target_rps')) and
            0<=recovery['current_rps']<recovery['required_rps']<=recovery['target_rps'])
        ...
        async with self.lock:
            ...
            transaction=uuid.uuid4().hex;started=time.time();stopped=[];created=[];safe_to_resume=True
            await self.freeze(remove_ids,True)
            await self.journal.emit(dict(kind='topology_begin',transaction=transaction,at_s=started,
                before=[asdict(s) for s in remove],after=[asdict(s) for s in add_specs],
                savings_lower_j=savings_lower_j,cost_upper_j=cost_upper_j,capacity_recovery=capacity_recovery))
            try:
                if self.backend.clocks and add_specs:
                    await self.backend.clocks.set(sorted({g for s in add_specs for g in s.gpus}),self.backend.max_frequency)
                for spec in remove:
                ...
                if retained_weights is None:
                    source=remove[0];raw=await self.request(source,'/runtime')
                    result=await self.request(source,'/retain-weights',dict(transaction=transaction,
                        expected_generation=raw['generation']),timeout=600)
                    retained_weights=result['retained_weights']
                # Physical capacity can be temporarily lost. Its full interval
                # and all eight idle cards remain inside the measured run.
                for spec in remove:
                    stopped.append(spec)
                    await self.lifecycle.stop(spec)
                peers={s.instance_id:dict(host='127.0.0.1',tp=s.tp,kv_port=s.kv_port)
                       for s in untouched+list(add_specs)}
                ...
                for new in add_specs:
                    ...
                    if targets: await self.request(new,'/prepare-peers',dict(peers=targets))
                await self.commit(remove_ids,tuple(add_specs))
                self.specs={s.instance_id:s for s in untouched+list(add_specs)};self.version+=1
                result=dict(transaction=transaction,version=self.version,committed=True,
                    duration_s=time.time()-started,retained_weights=retained_weights,validation=outcomes,
                    live_instances=[asdict(s) for s in self.specs.values()])
                await self.journal.emit(dict(kind='topology_commit',at_s=time.time(),**result))
                return result
            except BaseException as exc:
                # Never leave a cancelled coroutine halfway through rollback.
                await self.journal.emit(dict(kind='topology_failure',transaction=transaction,
                    at_s=time.time(),error=repr(exc),stopped=[s.instance_id for s in stopped]))
                recovery=asyncio.create_task(self.rollback(before,remove,stopped,created,retained_weights,transaction))
                safe_to_resume=False
                try:
                    try: recovered=await asyncio.shield(recovery)
                    except asyncio.CancelledError: recovered=await recovery
                except BaseException as restore_error:
                    await self.journal.emit(dict(kind='topology_recovery_failed',transaction=transaction,
                        at_s=time.time(),error=repr(restore_error),frozen_instances=list(remove_ids)))
                    raise
                safe_to_resume=True
                await self.journal.emit(dict(kind='topology_rollback',transaction=transaction,
                    at_s=time.time(),error=str(exc),recovered=recovered,duration_s=time.time()-started,
                    live_instances=[asdict(s) for s in self.specs.values()]))
                raise
            finally:
                if safe_to_resume: await self.freeze(remove_ids,False)

    async def rollback(self,before,remove,stopped,created,retained_weights,transaction):
        for spec in created:
            try: await self.lifecycle.stop(spec)
            except Exception:
                # A timed-out Docker client may have completed its removal.
                # Retry idempotently; never restore onto an unconfirmed GPU.
                await self.lifecycle.stop(spec)
        for spec in stopped:
            await self.lifecycle.stop(spec)  # resolve a timed-out removal before reusing GPUs
        if not stopped:
            # A pure addition may have published routes before a later journal
            # or refresh failure. Remove only additions that reached the backend.
            published=tuple(s.instance_id for s in created
                            if s.instance_id in getattr(self.backend,'instances',{}))
            if published: await self.commit(published,())
            for spec in remove:
                state=await self.request(spec,'/runtime')
                await self.request(spec,'/control',dict(generation=state['generation']+1,
                    role=spec.role,mode='continuous',admit_prefill=True))
            self.specs=dict(before)
            if published: self.version+=1
            return True
        # Restarted processes receive new IDs and ports, so surviving peers
        # cannot accidentally send into a dead NCCL communicator.
        restored=[replace(s,instance_id=s.instance_id+'r'+transaction[:8],
                    port=s.port+200,kv_port=s.kv_port+200,generation=s.generation+2) for s in stopped]
        ...
        if self.backend.clocks:
            await self.backend.clocks.set(sorted({g for s in restored for g in s.gpus}),self.backend.max_frequency)
        for spec in restored:
            await self.lifecycle.start(spec,peers,retained_weights);await self.verify(spec)
        for old in survivors:
            for new in restored:
                await self.request(old,'/register-peer',dict(id=new.instance_id,peer=peers[new.instance_id]))
        for spec in restored:
            await self.request(spec,'/prepare-peers',dict(peers=[i for i in peers if i!=spec.instance_id]))
        # The route commit may have succeeded before its journal/refresh failed.
        # Reconcile the identities actually published, including pure shrink.
        route_ids=(tuple(s.instance_id for s in stopped+created if s.instance_id in self.backend.instances)
                   if hasattr(self.backend,'instances') else tuple(s.instance_id for s in stopped))
        ...
        for spec in remove:
            if spec not in stopped:
                ...
                await self.request(spec,'/control',dict(generation=state['generation']+1,
                    role=spec.role,mode='continuous',admit_prefill=True))
        self.specs={s.instance_id:s for s in survivors+restored};self.version+=1
        return True
