...
from ecopadg.serving.dynamo_topology import DynamoTopologyPlanner
from ecopadg.serving.baselines import DistServeSearch
results={}
topology=DynamoTopologyPlanner(profiles,(),**kw)
demand=dict(input_tokens=128,output_tokens=64,ttft_s=100.,tpot_s=10.,rate=.1)
results['topology-configurations']=[asdict(c) for c in topology.configurations('SS',demand,2)]
assert results['topology-configurations']
search=DistServeSearch(profiles,[transfer],gpu_count=2,**kw)
results['static-spatial-search']=[asdict(c) for c in search.search(128,64,100.,10.,.1,{1:1000000})]
assert results['static-spatial-search']
