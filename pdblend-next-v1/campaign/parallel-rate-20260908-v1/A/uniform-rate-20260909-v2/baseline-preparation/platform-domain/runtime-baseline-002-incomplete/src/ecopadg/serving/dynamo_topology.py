...
class DynamoTopologyPlanner:
    def __init__(self,profiles,costs,*,error_fraction=.3,gpu_count=8,park_grace_s=.5,max_frequency=2520):
    ...
    def configurations(self,shape,demand,budget):
        ...
        for tp,batch in sorted({(p.tp,p.batch) for p in self.profiles.points if p.role=='mixed'}):
            p=self.profiles.lookup('mixed',tp,self.max_frequency,demand['input_tokens'],
        ...
        for counts in itertools.product(*(range(budget//tp+1) for tp in tps)):
            ...
            for groups in variants:
                ...
                for p,c in variant:
                    ...
                    single=self.profiles.lookup('mixed',p.tp,self.max_frequency,demand['input_tokens'],
