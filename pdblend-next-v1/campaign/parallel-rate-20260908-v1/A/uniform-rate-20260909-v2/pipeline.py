...
async def pdb_handoff(plan,out,state):
 ...
 q=p.ref(path);validator=p.ref(HERE/'formal-dynamic/verify.py')
 handoff=dict(node=plan['node'],model=plan['model'],system='pdblend',qualification=q,qualification_validator=validator,
   prepare_adapter=p.ref(HERE/'formal-dynamic/prepare_adapter.py'),measurement_executor=p.ref(HERE/'formal-dynamic/dynamic_measurement.py'),per_cell_release=True,
   predecessors=[p.ref(qstatus)],extra_files=[p.ref(HERE/'formal-dynamic/source-equivalence.json')])
...
async def execute(plan,out,state):
 ...
 p.need(pdb is not None,'new-A Alpaca must have actual qualified PDB work')
 verification=p.read(out/'pdb-handoff-verification.json')
