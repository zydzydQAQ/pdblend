...
def bind_parent_authority(binding, row, operation, config, inventory_path, lease):
    """The original final-P8 point_binding authority, created by the actual parent owner."""
    from pathlib import Path
    reference = lambda path: dict(path=str(Path(path).resolve()), sha256=sha(path))
    invocation = operation / 'invocation.json'
    write(invocation, dict(schema='uniform-v2-dynamic-parent-invocation-v1',
        row=row, binding=binding, template_config=reference(config),
        capacity_inventory_path=str(inventory_path), executor=reference(__file__)))
    job_path = operation / 'job.json'
    authority = dict(schema='capacity-parent-lease-authority-v1', fd=lease['fd'],
        ...
        capacity_inventory_path=str(inventory_path), expected_job_path=str(job_path), invocation=reference(invocation))
    path = operation / 'authority.json'
    write(path, authority)
    actual = dict(read(config), capacity_job_path=str(job_path), capacity_lease_authority=reference(path))
    destination = operation / 'parent-bound-config.json'
...
async def run_one(session, binding, row, output, hardware):
    ...
    config = bind_parent_authority(binding, row, operation, config, inventory_path, lease)
    job = dict(row=row, config=config, out=str(output / 'cells' / row['cell_id']),
        ...
        engine_ports=[i['port'] for i in binding['instances']], lease=lease,
    ...
    try:
        ...
        with (operation / 'child.log').open('xb') as log:
            child = await asyncio.create_subprocess_exec(sys.executable, '-u',
                str(Path(__file__).with_name('dynamic_child.py')), str(operation / 'job.json'),
        ...
        try:
            authority = read(operation / 'authority.json')
            invocation = read(operation / 'invocation.json')
            require(invocation['binding'] == binding and invocation['row'] == row,
                    'actual parent invocation differs from qualified binding/row')
            desired = dict(read(binding['configs'][row['dataset']]),
                capacity_job_path=str(operation / 'job.json'),
                capacity_lease_authority=dict(path=str(operation / 'authority.json'),
                    sha256=sha(operation / 'authority.json')))
            require(read(config) == desired and authority['capacity_inventory_path'] == str(inventory_path)
                    and all(authority[k] == lease[v] for k,v in
                        [('fd','fd'),('holder_pid','holder_pid'),('holder_start_ticks','holder_start_ticks'),
                         ('lock_device','device'),('lock_inode','inode')]),
                    'actual parent authority/config differs')
            receipt['parent_authority_verified'] = True
            receipt.setdefault('dynamic_artifacts', {}).update({str(operation / name): sha(operation / name)
                for name in ('invocation.json','authority.json','parent-bound-config.json')})
        except BaseException as exc:
            errors.append('parent invocation authority: ' + repr(exc))
