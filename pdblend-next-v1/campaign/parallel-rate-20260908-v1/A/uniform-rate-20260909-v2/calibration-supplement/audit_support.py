...
def audit_stage(out, spec_ref, mode, *, supplement=False, exclude_gap=False):
    out = Path(out)
    ...
    f.need(mode=='layout_calibration', 'mixed calibration terminal audit only')
    ...
    for reference in state['completed']:
        ...
        if result.get('phase_kind') == 'idle':
            if exclude_gap and Path(reference['path']).parent.name=='cycle-3-idle-layout2':
                try:
    ...
    f.need(not exclude_gap or len(excluded)==1,'one preserved idle-gap exclusion required')
    if supplement:
    ...
    return dict(schema='new-A-mixed-calibration-terminal-audit-v2', passed=True, independently_recomputed=True,
                supplement=supplement, excluded_measurements=excluded, all_phases_qualified=False,
