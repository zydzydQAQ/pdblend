...
async def measure_idle(controller, out, spec, duration_s):
    ...
    async def observe():
        ...
        require(previous_native_at is None or 0 < now-previous_native_at <= 1., 'native idle observation gap exceeds unchanged one-second gate')
    ...
    try:
        ...
        while time.time()<end:
            require(not controller.active and not controller.request_tasks,'request entered idle observation')
            await observe()
            await asyncio.sleep(min(spec['native_idle_poll_interval_s'],max(0.,end-time.time())))
        await observe()
