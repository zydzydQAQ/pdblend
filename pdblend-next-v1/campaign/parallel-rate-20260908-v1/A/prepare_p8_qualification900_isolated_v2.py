...
A=Path(__file__).resolve().parent;R=A.parent;CODE=A/'load-p8-isolated-code-003';ADAPTER=A/'isolated-power-v2';OUT=A/'p8-qualification900-inputs-003';RUN=A/'p8-qualification900-dynamic-002'
...
def main():
 ...
 hooks_manifest=ref(A/'dynamic-execution-isolated-power-002/manifest.json');hooks=checked(hooks_manifest)
 selftest=ref(A/'isolated-power-hardware-selftest-002/validation.json');assert checked(selftest)['passed'] is True
 files={**old['files'],**cpu['files'],**adapter['files'],**hooks['files']}
 for p in (A/'isolated-power-hardware-selftest-002').rglob('*'):
  if p.is_file():files[str(p)]=sha(p)
 files[hooks_manifest['path']]=hooks_manifest['sha256']
