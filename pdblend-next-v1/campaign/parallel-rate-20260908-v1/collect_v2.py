...
def stage_paths():
    ...
    for node in ('A','B','C'):
        base=ROOT/node
        for pattern in ('fixed-screen*/status.json','screen-p*/status.json','p*/fixed-screen*/status.json',
...
def collect(out,series):
    ...
    for status_path in stage_paths():
        ...
        version='p2' if ('p2' in str(directory.relative_to(ROOT))) else 'p1'
...
if __name__=='__main__':
    parser=argparse.ArgumentParser();parser.add_argument('--out',type=Path,required=True);parser.add_argument('--series',choices=('p1','p2'),required=True)
