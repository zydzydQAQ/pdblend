...
def test_frozen_common_source_and_parent_feature_defaults():
    ...
    manifests=[json.loads((root/'hosts'/f'{m}-fixed-p3/manifest.json').read_text()) for m in ('7b','14b','32b')]
    ...
    for n in ('idle_admission.py','backend.py'):
        assert (HOST/'src/ecopadg/serving'/n).read_bytes()==(parent/'src/ecopadg/serving'/n).read_bytes()
    spec=importlib.util.spec_from_file_location('common_build',root/'build_p3.py')
    mod=importlib.util.module_from_spec(spec);spec.loader.exec_module(mod)
    assert all((HOST/n).read_text()==s for n,s in mod.sources().items())
