...
def validate(p,selection,originals):
 ...
 p.need(doc['schema']=='final-baseline-scientific-overlay-v1' and doc['authorized'] is True,'unapproved baseline overlay')
 sources={ref['path']:ref['sha256']};quarantine={};groups=[];retired={};fresh={};original_by_id={x['cell_id']:x for x in originals}
 def pin(r):sources[r['path']]=r['sha256'];return p.checked(r)
 ...
 for reference in doc['replacement_groups']:
  d=pin(reference);p.need(d['schema']=='B-whole-group-cooperative-Dynamo-eight-v1','unrecognized whole-group replacement')
  ...
  p.need(expected_retired==set(retired),'replacement did not replace complete original group')
...
def annotate(point,overlay):
 ...
 if cid in overlay['fresh']:
  point.update(baseline_scope_status='required_whole_group_replacement',replacement_declaration=overlay['fresh'][cid]['declaration'],new_rate=not point.get('existing_original_rate',overlay['fresh'][cid]['row'].get('existing_original_rate',False)))
 return point
