...
import report_versions_v2 as report_versions
import inspect_baseline
...
def stage_paths():
    return sorted({p.parent/'status.json' for node in ('A','B','C')
...
def collect(out,series):
    ...
    code_sources={str(path):sha(path) for path in (Path(__file__),ROOT/'boundary_first_loss.py',ROOT/'inspect_exploration.py',ROOT/'report_versions_v2.py',ROOT/'inspect_baseline.py',ROOT/'boundary_first_loss_v2.py',AUDIT/'report.py',AUDIT/'protocol.py',AUDIT/'raw_metrics.py')}
    ...
    for cid,cell in declarations.items():
        ...
        for system,bid in cell.get('baseline_cell_ids',{}).items():
            ...
            pairs.append(dict(model=point['model'],dataset=point['dataset'],rate_rps=point['rate_rps'],
                ...
                implementation_id=point['implementation_id'],version_id=point['version_id'],profile_sha256=point['profile_sha256'],**verdict,strict_service_energy_pass=strict))
    baseline_points,baseline_sources=inspect_baseline.collect(audit,ROOT,points) if series=='p4' else ([],{})
        ...
        for dataset in ('alpaca','sharegpt','longbench'):
            ...
            for system in p.BASELINES:
                ...
                matrix.append(dict(model=model,dataset=dataset,baseline=system,declared_executions=len(subset),
                    ...
                    measured_logical_points=len({x['original_cell_id'] for x in subset if x['measurement_valid']}),
...
def logical_grid(originals,points,pairs):
    ...
    for old in originals:
        ...
        rows.append(dict(model=old['model'],dataset=old['dataset'],rate_rps=old['rate_rps'],seed=old['seed'],
            ...
            repeat_requirement_complete=bool(actual and len(actual)==len(measured)),
...
if __name__=='__main__':
    parser=argparse.ArgumentParser();parser.add_argument('--out',type=Path,required=True);parser.add_argument('--series',choices=('p1','p2','p3','p4'),required=True)
