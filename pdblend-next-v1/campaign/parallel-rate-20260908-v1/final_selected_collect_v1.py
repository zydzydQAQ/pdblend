...
import report_scope
import final_selection_v1 as selection_rules
...
def collect(out,selection_path,selection_sha):
    ...
    code_sources.update({str(path):sha(path) for path in (ROOT/"collect_v13.py",ROOT/"final_selection_v1.py")})
    ...
    selection_rules.annotate_scope(points,origins)
    ...
    result=dict(schema=2,created_s=time.time(),candidate_series=series,baseline_snapshot='snapshot-006',
        ...
        final_report_complete=False,actual_source_hashes_never_relabelled=True,capacity_policy_equal_across_models=False)
