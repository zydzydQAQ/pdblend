...
import report_versions_v4 as report_versions
...
def collect(out,series):
    ...
    code_sources={str(path):sha(path) for path in (Path(__file__),ROOT/'boundary_first_loss.py',ROOT/'inspect_exploration.py',ROOT/'report_versions_v4.py',ROOT/'inspect_baseline_v2.py',ROOT/'source_identity.py',ROOT/'report_scope.py',ROOT/'boundary_first_loss_v2.py',AUDIT/'report.py',AUDIT/'protocol.py',AUDIT/'raw_metrics.py')}
    ...
    baseline_points,baseline_sources=inspect_baseline.collect(audit,ROOT,points) if series in ('p4','p5') else ([],{})
...
if __name__=='__main__':
    parser=argparse.ArgumentParser();parser.add_argument('--out',type=Path,required=True);parser.add_argument('--series',choices=('p1','p2','p3','p4','p5'),required=True)
