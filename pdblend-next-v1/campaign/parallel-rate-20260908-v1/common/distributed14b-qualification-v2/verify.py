...
def policy_identity(config,parent,system):
    ...
    if system=='pdblend':operational|={'capacity_integration_v1','capacity_binding_path','capacity_binding_sha256','idle_domain_reacquire_v1'}
...
def controller_successor(q, config, files):
    need(config.get('idle_domain_reacquire_v1') is True, 'P11 explicit idle-domain reacquire must be enabled')
    lineage=bound(q['controller_successor'],files)
    need(lineage['schema']=='distributed14b-P10-to-P11-idle-domain-successor-v1' and lineage['host_manifest']==q['host_manifest'], 'actual P11 successor differs')
    need(lineage['parent_manifest']['sha256']=='5dcae91099369a188f4aecb92f7a8c06b138e6ae0ae181b786f7b7011d034b84', 'wrong numeric P10 parent')
    ...
    allowed={'src/ecopadg/serving/idle_admission.py','src/ecopadg/serving/idle_domain_reacquire.py'}
    changes={name for name in set(parent['files'])|set(actual['files']) if parent['files'].get(name)!=actual['files'].get(name)}
    need(changes and changes<=allowed and 'src/ecopadg/serving/idle_domain_reacquire.py' in actual['files'], 'P11 changes more than the reviewed idle-domain helper')
    cpu=bound(lineage['cpu_validation'],files)
    need(cpu['passed'] is True and any(x['manifest']==q['host_manifest'] for x in cpu['sources']), 'actual P11 source not in frozen common CPU proof')
    need(lineage['profile']==q['profile'] and lineage['profile_registration']==q['profile_registration']['registration'], 'P11 changed the measured numeric profile lineage')
    return dict(parent_manifest=lineage['parent_manifest'],actual_manifest=q['host_manifest'],cpu_validation=lineage['cpu_validation'],changed_execution_modules=sorted(changes),original_numeric_and_measurement_sources_preserved=True,enabled_opt_in='idle_domain_reacquire_v1')
