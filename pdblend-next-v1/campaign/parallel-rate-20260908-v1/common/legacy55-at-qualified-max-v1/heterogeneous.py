...
sys.path.insert(0,'/root/workspace/pdblend-next-v1/campaign/A14B-legacy-heterogeneous-correctness-v1')
from checks import require,write
from route_checks import Checks
ROOT=Path('/root/workspace/pdblend-next-v1/campaign/A14B-legacy-heterogeneous-correctness-v1')
...
def validate_scope(binding):
    require(binding.get('model')=='14b','A14B original heterogeneous route only')
    instances=binding.get('instances',[])
    require(len(instances)==6 and [(i['tp'],i['gpus']) for i in instances]==[(1,[g]) for g in range(5)]+[(2,[6,7])],'five original TP1 prefills plus one TP2 decoder required')
    for i in instances:
        require(i['native_kind']=='legacy_sync_put' and not i.get('scheduler_cache_observed'),'legacy fields only')
        ...
        require('service_budget_tokens' not in i and 'restore_budget_tokens' not in i,'legacy does not accept dynamic budget controls')
    ...
    async with aiohttp.ClientSession(trust_env=False) as session:
        try:
            ...
            for instance, actual_identity in zip(binding['instances'],identity):
                ...
                cfg=m.read(path);require(cfg['max_num_batched_tokens']==8192 and cfg['max_num_seqs']==32 and cfg['max_model_len']==8192 and cfg['tp']==instance['tp'],'static startup budget/TP/model work limit differs')
                actual=actual_identity['provenance'];expected_model='/models/Qwen2.5-'+binding['model'].upper()+'-Instruct'
            ...
            started=time.time();clocks=await asyncio.to_thread(ClockOwner,hardware,tuple(range(8)),max_service_frequency_mhz=args.maximum)
            await asyncio.wait_for(clocks.set(range(8),args.maximum,verify_rise=False),15)
            checker=Checks(session,binding,args.out/'checks');status['phase']='ordinary_local_tp1_local_tp2_then_real_cross_tp_pd';save()
            ...
            status['system_eligibility']={'distserve-longbench-heterogeneous':status['mechanism_gate']['ordinary'] and status['mechanism_gate']['pd']}
...
def main():
    ...
    else:print(json.dumps(dict(cpu_only=True,hardware_actions=False,binding_checked=True,scope='A14B five TP1 to TP2 route correctness; distinct precision oracles; no performance')))
