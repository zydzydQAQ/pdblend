...
sys.path.insert(0,'/root/workspace/pdblend-next-v1/campaign/AC-legacy-resident-correctness-v1')
from checks import Checks,require,write
ROOT=Path('/root/workspace/pdblend-next-v1/campaign/AC-legacy-resident-correctness-v1')
...
def validate_scope(binding):
    ...
    require(len(binding.get('instances',[]))==8,'eight resident TP1 replicas required')
    require([i['gpus'] for i in binding['instances']]==[[j] for j in range(8)],'fixed original A/C single-GPU residents required')
    for i in binding['instances']:
        require(i['tp']==1 and i['native_kind']=='legacy_sync_put' and not i.get('scheduler_cache_observed'),'legacy owner ACK must not invent v3 cache telemetry')
    ...
    async with aiohttp.ClientSession(trust_env=False) as session:
        try:
            ...
            for instance, actual_identity in zip(binding['instances'],identity):
                ...
                cfg=m.read(path);require(cfg['max_num_batched_tokens']==8192 and cfg['max_num_seqs']==32 and cfg['max_model_len']==8192 and cfg['tp']==1,'static startup budget/TP/model work limit differs')
...
def main():
    ...
    else:print(json.dumps(dict(cpu_only=True,hardware_actions=False,binding_checked=True,scope='eight TP1 A/C resident legacy correctness, no baseline performance')))
