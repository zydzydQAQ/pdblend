...
def policy_identity(config,parent,system):
    ...
    if system=='pdblend':operational|={'capacity_integration_v1','capacity_binding_path','capacity_binding_sha256','idle_domain_reacquire_v1','idle_domain_reacquire_timeout_s'}
...
def controller_successor(q, config, files):
    need(config.get('idle_domain_reacquire_v1') is True and type(config.get('idle_domain_reacquire_timeout_s')) in (int,float) and config['idle_domain_reacquire_timeout_s']==1.5, 'P12 explicit independently bounded idle recovery required')
    lineage=bound(q['controller_successor'],files)
    need(lineage['schema']=='distributed14b-P11-to-P12-idle-recovery-successor-v1' and lineage['host_manifest']==q['host_manifest'], 'actual P12 successor differs')
    need(lineage['parent_manifest']['sha256']=='ceb2f27562e249d50a1782c46ad7bf1a802e0cf735c3ece2930ca96d19bfa3e4', 'wrong P11 parent')
    parent=bound(lineage['parent_manifest'],files);actual=bound(lineage['host_manifest'],files)
    manifest(lineage['parent_manifest']);manifest(lineage['host_manifest'])
    changes={name for name in set(parent['files'])|set(actual['files']) if parent['files'].get(name)!=actual['files'].get(name)}
    need(changes=={'src/ecopadg/serving/idle_domain_reacquire.py'},'P12 changes more than the independently bounded idle helper')
    need(parent['files']['src/ecopadg/serving/backend.py']==actual['files']['src/ecopadg/serving/backend.py'],'original active DVFS bound changed')
    cpu=bound(lineage['cpu_validation'],files)
    need(cpu['passed'] is True and any(x['manifest']==q['host_manifest'] for x in cpu['sources']), 'actual P12 source absent from frozen CPU proof')
    need(lineage['profile']==q['profile'] and lineage['profile_registration']==q['profile_registration']['registration'], 'P12 changed measured numeric lineage')
    need(lineage['idle_domain_reacquire_timeout_s']==config['idle_domain_reacquire_timeout_s'],'declared and actual idle budgets differ')
    observation=lineage['idle_transition_qualification'];bound(observation['measurement'],files)
    need(observation['validator']['sha256']=='70d3c64c34f564b4538c5f9e159d10d640cae6628465ce1e31f2c0fe187d3fbe','wrong pure six-transition audit implementation')
    proof=module(observation['validator'],files).verify(observation['measurement'])
    need(proof['independently_recomputed'] is True and proof['node']==q['node'] and proof['hostname']==q['hostname'] and proof['model_requests']==0,'idle recovery evidence belongs to another host')
    observed_binding=bound(proof['binding'],files);native_binding=bound(q['binding'],files)
    need(observed_binding['instances']==native_binding['instances'],'idle recovery measured on different actual native engines')
    need(len(proof['points'])==6 and proof['maximum_stable_observation_s']<config['idle_domain_reacquire_timeout_s'],'declared idle budget does not cover the complete local measured group')
    need(all(files.get(p)==h and sha(p)==h for p,h in proof['files'].items()),'idle recovery raw proof outside qualified closure')
    return dict(parent_manifest=lineage['parent_manifest'],actual_manifest=q['host_manifest'],cpu_validation=lineage['cpu_validation'],changed_execution_modules=sorted(changes),original_active_settle_timeout_s=.3,original_numeric_and_measurement_sources_preserved=True,idle_domain_reacquire_timeout_s=config['idle_domain_reacquire_timeout_s'],idle_transition_qualification=observation,maximum_empirical_observed_s=proof['maximum_stable_observation_s'],empirical_value_not_a_future_guarantee=True)
...
def verify(reference,binding,cp=None):
    ...
    successor_proof=controller_successor(q,config,files)
    ...
    return dict(passed=True,hostname=q['hostname'],measurement_host=q['node'],gpu_identity=gpus,host_manifest=q['host_manifest'],profile_sha256=q['profile']['sha256'],execution_group=q['execution_group'],independently_recomputed=True,controller_successor=successor_proof,policy=policy,native=native,frequency_cases=frequencies,profile_registration=registration)
