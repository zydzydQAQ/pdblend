...
class SavedEvidence(unittest.TestCase):
    ...
    def policy(self):
        return q.read(R/'A/final-p9-release-001/configs/sharegpt.json')

    def test_explicit_operational_migration_allowed(self):
        parent=self.policy();changed=copy.deepcopy(parent)
        changed.update(max_service_frequency_mhz=2400,profiles='/CPU-only/actual2400.json',capacity_integration_v1=False,instances=self.b['instances'])
        changed.pop('capacity_binding_path');changed.pop('capacity_binding_sha256')
        self.assertEqual(q.policy_identity(changed,parent,'pdblend')['strategy'],'pdblend-joint')

    def test_original_slo_cannot_be_tuned(self):
        parent=self.policy();changed=dict(parent,slo_tpot_s=.2)
        with self.assertRaisesRegex(ValueError,'original serving policy'):q.policy_identity(changed,parent,'pdblend')

    def test_original_pending_guard_cannot_be_disabled(self):
        parent=self.policy();changed=dict(parent,pending_admission_capacity_guard=False)
        with self.assertRaisesRegex(ValueError,'original serving policy'):q.policy_identity(changed,parent,'pdblend')

    def test_original_native_budget_cannot_be_tuned(self):
        parent=self.policy();changed=copy.deepcopy(parent);changed['scheduler_budget_ablation']['max_num_batched_tokens']=8192
        with self.assertRaisesRegex(ValueError,'original serving policy'):q.policy_identity(changed,parent,'pdblend')

    def test_original_clocks_guard_cannot_be_disabled(self):
        parent=self.policy();changed=dict(parent,measured_frequency_write_guard_v1=False)
        with self.assertRaisesRegex(ValueError,'original serving policy'):q.policy_identity(changed,parent,'pdblend')
