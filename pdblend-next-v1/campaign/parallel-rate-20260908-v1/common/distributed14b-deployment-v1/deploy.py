...
def prepare_spec(stage, frozen_release, hostname, idle_proof, previous_binding=None):
    ...
    name_prefix = release.get('container_prefix', 'slo90-distributed14b')
    ...
    spec = dict(schema=1, protocol_id=adapter.PROTOCOL, model='14b', stage=stage, hostname=hostname,
        ...
        redistribution_jobs=release['redistribution_jobs'], node=release['node'], baseline_system=release.get('baseline_system'),
        host_manifest=reference(host/'manifest.json'))
    files[release['redistribution_jobs']['path']]=release['redistribution_jobs']['sha256']
...
def validate_spec(spec, *, check_host=True):
    ...
    require(sha(jobs_ref['path'])==jobs_ref['sha256'], 'redistribution jobs changed')
    jobs=read(jobs_ref['path']);parent=jobs['parent']
    require(parent['sha256']=='913a2d5834dbcc3466cff9d6a45e30cba574069ed50d36c89704ffff89b80438'
            and sha(parent['path'])==parent['sha256'], 'exact authorized redistribution required')
    require(spec['files'].get(jobs_ref['path'])==jobs_ref['sha256'], 'jobs outside deployment freeze')
    require(jobs['node']==spec['node'] and jobs['model']=='14b' and spec['node'] in ('B','C'), 'wrong distributed node/model')
    require(spec['hostname']=={'B':'iZwz9i5bte3xkpmcoes3t2Z','C':'iZwz9gfq11hx1sbob59yrgZ'}[spec['node']], 'wrong physical assigned node')
    expected_geometry=[(1,[6]),(1,[7])] if spec['stage']=='pdblend' else [(1,[j]) for j in range(8)]
    if spec['stage']=='baselines' and spec.get('baseline_system')=='distserve' and jobs['dataset']=='longbench':
        expected_geometry=[(1,[j]) for j in range(5)]+[(2,[6,7])]
    require([(i['tp'],i['gpus']) for i in spec['instances']]==expected_geometry,'declared deployment geometry differs')
    require(len({i['id'] for i in spec['instances']})==len(spec['instances']), 'duplicate engine ID')
    require(spec['host_manifest']==reference(Path(spec['host_release'])/'manifest.json'), 'actual host manifest changed')
    if spec['stage']=='pdblend':
        require(spec['host_manifest']==jobs['common_controller_manifest'], 'PDB must use common P9 source')
    ...
    for instance in spec['instances']:
        ...
        require(instance['image'] == image and instance['tp'] in (1,2) and cfg['id'] == instance['id'] and
                cfg['model'] == '/models/Qwen2.5-14B-Instruct' and cfg['tp'] == instance['tp'] and
