...
def prepare(feasibility,out,prior_domain_negative):
    ...
    references=[v.ref(prior_domain_negative),v.ref(HERE/'manifest.json'),v.ref(feasibility/'evidence-manifest.json'),v.ref(feasibility/'status.json'),v.ref(feasibility/'spec.json'),
    ...
    spec.update(schema='distributed14b-actual2100-profile-input-v1',registration_frequency_mhz=2100,prior_domain_negative=v.ref(prior_domain_negative),max_service_frequency_mhz=2100,authorized=True,points=v.declared_points(profile,binding),
        ...
        transition_pairs=[[a,b] for a in (900,1500,2100) for b in (900,1500,2100) if a!=b],
...
if __name__=='__main__':
    parser=argparse.ArgumentParser();parser.add_argument('--feasibility',type=Path,required=True);parser.add_argument('--out',type=Path,required=True);parser.add_argument('--prior-domain-negative',type=Path,required=True)
    args=parser.parse_args();print(json.dumps(prepare(args.feasibility,args.out,args.prior_domain_negative)))
