...
RUNTIME_SHA = '251490dff01ec10681bcf5ab25cf394f71ddb6df3477c518e259712ac29b3d1c'
...
def audit(checkpoint_reference):
    ...
    need(binding['hostname']=='iZwz9i5bte3xkpmcoes3t2Z' and row['model']=='32b'
         and row['system'] in ('mixed','distserve','dynamollm'), 'unrecognized physical host/model/runtime family')
    ...
    need(sha(runtime)==RUNTIME_SHA and manifest['files']['src/ecopadg/serving/runtime.py']==RUNTIME_SHA,
         'unrecognized forward-before-native source')
    need(binding['files'][str(runtime)]==RUNTIME_SHA,'executed runtime is not pinned by binding')
    ...
    return dict(schema='independent-unforwarded-deadline-zero-output-v1',passed=True,
