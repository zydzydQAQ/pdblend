...
def fixture():
    ...
    raw,_,_=old.fixture();p=raw['point'];p.update(frequency_mhz=2400,output_tokens=256,context_tokens=256,arrival_offsets_s=[0.])
    ...
    clocks=[(-3+j*.02,[2400]*8) for j in range(950)];power=[(t,[100.]*8) for t,_ in clocks]
...
class Tests(unittest.TestCase):
    ...
    def test_phase_no_bracket(self):
        with self.assertRaises(ValueError):v.phase_clock([(1,[2400]*8),(1.1,[2400]*8)],6,2400,.9,1.05)
    def test_phase_gap(self):
        with self.assertRaises(ValueError):v.phase_clock([(0,[2400]*8),(.3,[2400]*8)],6,2400,.01,.29)
    def test_phase_tolerance(self):
        self.assertEqual(v.phase_clock([(0,[2385]*8),(.1,[2415]*8)],6,2400,.01,.09)['min_mhz'],2385)
    ...
    def test_source_order_original_byte_identity(self):
        for name in ('source_order.py','context_export.py'):
...
class AliasTests(unittest.TestCase):
    def test_exact_logger_alias_bytes(self):
        original=v.read(v.HERE.parent/'distributed14b-frequency-profile-v1/source-order-contract.json')
        current=v.read(v.HERE/'source-order-contract.json')
        self.assertEqual({k:x for k,x in original.items() if k!='engine_logger_source'},{k:x for k,x in current.items() if k!='engine_logger_source'})
        self.assertEqual(Path(original['engine_logger_source']).read_bytes(),Path(current['engine_logger_source']).read_bytes())
