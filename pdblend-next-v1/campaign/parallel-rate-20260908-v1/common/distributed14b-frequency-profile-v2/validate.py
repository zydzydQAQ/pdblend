"""Actual two-card 2400-MHz same-shape empirical profile evidence."""
...
def declared_points(profile,binding):
    ...
    for shape in geometry(profile):
        for repeat,instance in enumerate(instances,1):
            result.append(dict(shape,point_id=f"in{shape['input_tokens']}-b{shape['batch']}-ctx{shape['context_tokens']}-f2400-gpu{instance['gpus'][0]}",
                frequency_mhz=2400,output_tokens=1024,instance_id=instance['id'],gpu=instance['gpus'][0],repeat=repeat,
...
def source_check(spec):
    require(spec['schema']=='distributed14b-actual2400-profile-input-v1' and spec['authorized'] is True,'explicit measured-profile declaration required')
    ...
    require(spec['transition_gpus']==[6,7] and spec['transition_pairs']==[[a,b] for low in (900,1500,2100) for a,b in ((low,2400),(2400,low))],
    ...
    require(feasible['passed'] and feasible['complete'] and feasible['measurement_valid'] and not feasible['errors']
            and fspec['binding']==spec['binding'],'actual prior 2400 feasibility did not pass on this deployment')
...
def validate_profile_point(raw,events,power,clock_samples,source_order_verified=False):
    ...
    original=load(PARENT,'actual2400_original_native_validator')
    ...
    if any(not e.get('request_ids') for e in events):
        selected,terminal=load(HERE/'terminal.py','actual2400_terminal_empty').trailing_empty(compatible,events)
    context=load(HERE/'context_export.py','actual2400_logical_context')
    ...
    late=context.attach_power(late,power,clocks,target_gpu=gpu,frequency_mhz=2400)
    ...
    for rid in sorted(ids):
        ...
        prefills.append(dict(request_id=rid,start_s=a,end_s=b,duration_s=b-a,target_gpu_power_w=target/(b-a),
            all8_energy_j=whole,background_decode_max=max(e['decode'] for e in pf),clock=phase_clock(clocks,gpu,2400,a,b)))
    ...
    for run in runs:
        ...
        decode.append(dict(start_s=a,end_s=b,steps=len(run),finish_spacing_values_s=[y['finished_s']-x['finished_s'] for x,y in zip(run,run[1:])],
                           target_gpu_mean_power_w=target/(b-a),all8_energy_j=whole,clock=phase_clock(clocks,gpu,2400,a,b)))
    ...
    return dict(passed=True,point=p,native=native,late_context=late,prefill_phases=prefills,full_decode_runs=decode,
        idle_residency=dict(start_s=idle['start_s'],end_s=idle['end_s'],all8_energy_j=whole,target_gpu_power_w=target/(idle['end_s']-idle['start_s']),
                            commanded_frequency_mhz=2400,lower_idle_Pstate_is_legal=True),
