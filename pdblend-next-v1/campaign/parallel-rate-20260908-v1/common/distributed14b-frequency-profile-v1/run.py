...
async def execute(spec,out,lease):
    ...
    status=dict(schema='distributed14b-actual2400-profile-result-v1',pid=os.getpid(),started_s=time.time(),passed=False,complete=False,errors=[],points=[],profile_reference=spec['profile_reference'],profile_publication_allowed=False,not_formal_performance=True,binding=spec['binding'],spec=v.ref(out/'spec.json'))
    hooks=v.load(Path(spec['measurement_hooks']['path']),'actual2400_isolated_hooks')
    ...
    order=v.load(HERE/'source_order.py','actual2400_source_order')
    class SourceGate:
    ...
    async with aiohttp.ClientSession(trust_env=False) as session:
        try:
            ...
            switcher=v.load(HERE/'frequency_switch.py','actual2400_frequency_switch')
            status['switches']=[]
            ...
            deploy.write(out/'evidence-manifest.json',dict(schema='distributed14b-actual2400-profile-evidence-v1',files=files,passed=status['passed'],spec=v.ref(out/'spec.json'),source=v.ref(HERE/'manifest.json')))
    return status
