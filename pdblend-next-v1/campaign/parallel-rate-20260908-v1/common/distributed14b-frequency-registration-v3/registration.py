"""Recompute target-local 2100 MHz evidence; never infer it from 2520 MHz.
...
DRIVERS={str(HERE.parent/'distributed14b-frequency-profile2100-v2/manifest.json'):'e7a4d7f6130f24dca8e009b51cc435c648ac91f731e810a9d9036f4442aba187'}
...
def aggregate(profile,rows,transitions,*,source_reference,hostname,node,measurement_reference):
    """Only actual repeated phase envelopes become new 2100 ProfilePoints."""
    from ecopadg.serving.profiles import ProfilePoint,ProfileStore
    result=copy.deepcopy(profile)
    removed=[p for p in profile['points'] if p['frequency_mhz']>=2100]
    result['points']=[p for p in result['points'] if p['frequency_mhz']<=1500]
    result['interference_points']=[p for p in result.get('interference_points',[]) if p['frequency_mhz']<=1500]
    groups={}
    for row in rows:
    ...
    for (length,batch,context),members in sorted(groups.items()):
        ...
        need(all(r['point']['frequency_mhz']==2100 and r['point']['output_tokens']==1024 for r in members), 'actual frequency and full microprofile outputs changed')
        provenance=dict(input_tokens=length,batch=batch,context_tokens=context,frequency_mhz=2100,
        ...
        for role in ('mixed','decode'):
            point=ProfilePoint(role=role,tp=1,frequency_mhz=2100,input_tokens=length,context_tokens=context,batch=batch,
    ...
    frequency_costs=[]
    expected={(a,b) for a in (900,1500,2100) for b in (900,1500,2100) if a!=b}
    pairs={}
    ...
    result['status']='target-local development empirical 2100MHz profile; fresh serving qualification remains required'
    result['heldout_calibration_complete']=False;result['instant_heldout_calibration_complete']=False
    result['frequency_registration']=dict(schema='distributed14b-measured2100-profile-registration-v1',hostname=hostname,node=node,
        registration=source_reference,measurement=measurement_reference,original_profile_retained=True,
        original_profile_reference_only_below_or_equal_1500=True,original_points_at_or_above_2100_removed=len(removed),
        removed_frequency_domain=[2100,2520],new_frequency_mhz=2100,added_points=len(added),measured_domains=domains,
...
def _verify(registration_reference,profile_reference=None):
    ...
    need(declaration['schema']=='distributed14b-actual2100-registration-input-v3' and declaration['approved'] is True,
    ...
    need(declaration['node']==spec['node']=='B' and spec['registration_frequency_mhz']==2100 and declaration['hostname']==spec['hostname']
    ...
    sampler=load(SAMPLER_AUDIT,'actual2100_saved_measurement');audited=sampler.audit_samplers(status['isolated_samplers'],spec['host_manifest'],artifacts=measurement['files'])
    ...
    order=load(driver/'source_order.py','actual2100_saved_source_order')
    ...
    for declared,saved in zip(spec['points'],status['points']):
        ...
        rows.append(result)
    transition_rows=[];switcher=load(driver/'frequency_switch.py','actual2100_saved_switch')
...
def prepare(measurement_path,out):
    ...
    declaration=dict(schema='distributed14b-actual2100-registration-input-v3',approved=True,node=spec['node'],hostname=spec['hostname'],
