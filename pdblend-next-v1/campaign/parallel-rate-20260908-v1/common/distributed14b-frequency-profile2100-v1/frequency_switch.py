...
def pairs():return [(a,b) for a in (900,1500,2100) for b in (900,1500,2100) if a!=b]
def save(path,value):
...
async def observe(common,session,instance,probe,clocks,hardware,out):
    ...
    try:
        ...
        await clocks.set([gpu],2100,verify_rise=False)
...
def derive(raw,power,clocks):
    ...
    native=v.load(v.PARENT,'actual2100_switch_native')
    ...
    if any(not e.get('request_ids') for e in events):
        selected,_=v.load(v.HERE/'terminal.py','actual2100_switch_terminal').trailing_empty(compatible,events)
    ...
    context=v.load(v.HERE/'context_export.py','actual2100_switch_context')
