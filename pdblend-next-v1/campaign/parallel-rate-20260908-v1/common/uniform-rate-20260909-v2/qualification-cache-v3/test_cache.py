...
class CacheTests(unittest.TestCase):
    ...
    def test_stat_only_directory_replaced_with_file_rejected(self):
        with tempfile.TemporaryDirectory() as directory:
            root=Path(directory);entry=root/'empty';entry.mkdir()
            result=self.optional_fixture(root,' assert (root/"empty").is_dir()')
            entry.rmdir();entry.write_text('file')
            with self.assertRaisesRegex(ValueError,'directory identity changed'):
                cache.verify_cached(result['qualification'],result['cache'])

    def test_stat_only_directory_new_inode_rejected_but_child_addition_allowed(self):
        with tempfile.TemporaryDirectory() as directory:
            root=Path(directory);entry=root/'empty';entry.mkdir()
            result=self.optional_fixture(root,' assert (root/"empty").is_dir()')
            (entry/'new').write_text('allowed because only directory kind was queried')
            self.assertTrue(cache.verify_cached(result['qualification'],result['cache'])['passed'])
            entry.rename(root/'prior-directory');entry.mkdir()
            with self.assertRaisesRegex(ValueError,'directory identity changed'):
                cache.verify_cached(result['qualification'],result['cache'])

    def test_symbolic_link_same_target_new_inode_rejected(self):
        with tempfile.TemporaryDirectory() as directory:
            root=Path(directory);(root/'target').write_text('data');link=root/'link';link.symlink_to('target')
            result=self.optional_fixture(root,' assert (root/"link").is_symlink()')
            replacement=root/'replacement';replacement.symlink_to('target');replacement.replace(link)
            with self.assertRaisesRegex(ValueError,'symbolic link changed'):
                cache.verify_cached(result['qualification'],result['cache'])

    def test_symbolic_link_target_change_rejected(self):
        with tempfile.TemporaryDirectory() as directory:
            root=Path(directory);(root/'target').write_text('same');(root/'other').write_text('same')
            link=root/'link';link.symlink_to('target')
            result=self.optional_fixture(root,' assert os.readlink(root/"link")=="target"')
            link.unlink();link.symlink_to('other')
            with self.assertRaisesRegex(ValueError,'symbolic link changed'):
                cache.verify_cached(result['qualification'],result['cache'])


    def hostname_fixture(self, root, hostname):
        binding=root/'binding.json';binding.write_text(json.dumps(dict(hostname=hostname)))
        qualification=root/'qualified.json';qualification.write_text(json.dumps(dict(binding=cache.ref(binding))))
        validator=root/'original.py';validator.write_text('import json,socket\nfrom pathlib import Path\n'
            'def verify(reference):\n'
            ' actual=socket.gethostname()\n'
            ' q=json.loads(Path(reference["path"]).read_text())\n'
            ' return dict(passed=bool(actual),independently_recomputed=True,binding=q["binding"])\n')
        return cache.ref(qualification),cache.ref(validator)

    def test_local_hostname_is_allowed_and_bound_to_real_binding(self):
        with tempfile.TemporaryDirectory() as directory:
            root=Path(directory);qualification,validator=self.hostname_fixture(root,cache.socket.gethostname())
            result=cache.build(qualification,validator,root/'cache',workspace=root)
            saved=cache.checked(result['cache'])
            self.assertEqual(saved['captured_hostname'],cache.socket.gethostname())
            self.assertTrue(cache.verify_cached(qualification,result['cache'])['passed'])
            with patch.object(cache.socket,'gethostname',return_value='foreign-host'):
                with self.assertRaisesRegex(ValueError,'another host'):
                    cache.verify_cached(qualification,result['cache'])

    def test_local_hostname_cannot_authorize_foreign_binding(self):
        with tempfile.TemporaryDirectory() as directory:
            root=Path(directory);qualification,validator=self.hostname_fixture(root,'foreign-host')
            with self.assertRaisesRegex(ValueError,'hostname differs'):
                cache.build(qualification,validator,root/'cache',workspace=root)
            self.assertFalse((root/'cache').exists())

    def test_original_v2_blocker_is_real_not_assumed(self):
        import importlib.util
        source=Path(cache.__file__).resolve().parent.parent/'qualification-cache-v2/cache.py'
        spec=importlib.util.spec_from_file_location('frozen_cache_v2',source)
        old=importlib.util.module_from_spec(spec);spec.loader.exec_module(old)
        with tempfile.TemporaryDirectory() as directory:
            root=Path(directory);qualification,validator=self.hostname_fixture(root,cache.socket.gethostname())
            with self.assertRaisesRegex(ValueError,'unsupported qualification input channel: socket.gethostname'):
                old.build(qualification,validator,root/'cache',workspace=root)
