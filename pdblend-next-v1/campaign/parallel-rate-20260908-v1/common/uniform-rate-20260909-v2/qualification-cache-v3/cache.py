...
class ReadCapture:
    ...
    def __init__(self, workspace=WORKSPACE):
        ...
        self.symlinks = {}
        self.captured_hostname = None
        self.original_gethostname = socket.gethostname
        self.directories = {}
        self.absent = set()
        self.active = False
    ...
    def hook(self, event, args):
        ...
        if event == 'socket.gethostname':
            self.active = False
            try:
                actual = self.original_gethostname()
            finally:
                self.active = True
            if self.captured_hostname is not None and self.captured_hostname != actual:
                self.fail('hostname changed during full qualification')
            self.captured_hostname = actual
            return
...
def build_worker(qualification, validator, out, *, workspace=WORKSPACE):
    ...
    if capture.captured_hostname is not None:
        need(socket.gethostname() == capture.captured_hostname == binding.get('hostname'),
             'captured hostname differs from actual host or qualification binding')
    value = dict(schema='immutable-independently-recomputed-qualification-cache-v3',
        hostname=socket.gethostname(), captured_hostname=capture.captured_hostname,
        python_version=sys.version, qualification=qualification,
...
def verify_cached(reference, cache_reference):
    ...
    need(value['schema'] == 'immutable-independently-recomputed-qualification-cache-v3'
    ...
    if value['captured_hostname'] is not None:
        binding = checked(value['proof']['binding'])
        need(socket.gethostname() == value['hostname'] == value['captured_hostname'] == binding.get('hostname'),
             'captured hostname differs from actual host or qualification binding')
