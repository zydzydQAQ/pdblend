...
def native_diagnosed_zero_output(checkpoint_reference):
    """Reuse the existing independent audit of C's unique native refusal path."""
    ...
    if receipt['summary'].get('work_complete') is True:
        return set(), None
    auditor = ROOT / 'C/audit_eco_native_queue_negative_v1.py'
    need(sha(auditor) == 'c165006fdc260e48958d5be211320b8300abf431b7243510f8bab728dd386809',
         'native refusal auditor changed')
    reference_adapter = isinstance(cp.get('binding'), dict) or isinstance(cp.get('receipt'), dict)
    if reference_adapter:
        auditor = ROOT / 'common/token-evidence-v2/audit_native_queue_v3.py'
        need(sha(auditor) == 'a9b18ec971053cd8fa53a08f42d2c79c58a2f3a55d135a0396499556490717ab', 'native refusal reference adapter changed')
    spec = importlib.util.spec_from_file_location('_v2_native_refusal_auditor', auditor)
    module = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(module)
    proof = module.audit(Path(checkpoint_reference['path']))
    if reference_adapter:
        proof['checkpoint_reference_format_adapter'] = ref(auditor)
    need(proof['passed'] and proof['checkpoint'] == checkpoint_reference
         and proof['every_other_request_completed_exact_output'] and proof['no_accepted_request_stranded'],
         'native zero-output refusal diagnosis failed')
    return {str(item['request_id']) for item in proof['refusal_details']}, proof
...
def diagnosed_zero_output(checkpoint_reference):
    if checkpoint_reference:
        cp = checked(checkpoint_reference)
        row = cp['row']
        if (checkpoint_host(cp), row['model']) == ('B', '32b') and row['system'] in ('mixed', 'distserve', 'dynamollm'):
            ...
            if isinstance(receipt_ref, dict):
                ...
                unknown = [r for r in requests if received_output_count(r) is None
                    and not (r.get('token_count_source') == 'server_usage' and truth(r.get('token_ids_verified')))]
                if unknown and all(str(r.get('token_evidence_schema')) == '2' and truth(r.get('request_timeout'))
                                   and not truth(r.get('token_stream_opened')) for r in unknown):
                    auditor = ROOT / 'common/token-evidence-v2/audit_unforwarded_deadline_v1.py'
                    need(sha(auditor) == 'd2e87c6af4dc10f9cad68c4c5afe6f6dde3433e8c032723c1d9a3df07fa73eb0',
                         'unforwarded deadline auditor changed')
                    spec = importlib.util.spec_from_file_location('_uniform_unforwarded_deadline', auditor)
                    module = importlib.util.module_from_spec(spec)
                    spec.loader.exec_module(module)
                    proof = module.audit(checkpoint_reference)
                    need(proof['passed'] and proof['checkpoint'] == checkpoint_reference, 'unforwarded deadline diagnosis failed')
                    return {str(item['request_id']) for item in proof['zero_output_details']}, proof
    return native_diagnosed_zero_output(checkpoint_reference)
