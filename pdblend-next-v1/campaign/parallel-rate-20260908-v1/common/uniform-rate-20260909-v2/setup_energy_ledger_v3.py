    ...
    for node, directory in directories(root):
        ...
        for path in directory.glob('**/status.json'):
            try:
                ...
                continue
    # Recovery inputs can contain a byte-identical receipt without a second
    # physical power tree. Resolve only exact receipt aliases on the same node;
    # one independently audited original is sufficient, never a second meter.
    identities = {}
    for node, path in sorted(set(candidates)):
        try:
            digest = hashlib.sha256(path.read_bytes()).hexdigest()
            identities.setdefault((node, digest), []).append(path)
        except OSError as exc:
            errors.append(dict(node=node, path=str(path), error=repr(exc)))
    audited = []
    for (node, digest), paths in sorted(identities.items()):
        failures = []
        for path in paths:
            try:
                entry, samples = audit(path)
                m.need(entry['receipt']['sha256'] == digest, 'receipt changed during meter audit')
                entry['node'] = 'Anew20260909' if node == 'A' else node
                entry['identical_receipt_aliases'] = [dict(path=str(other), sha256=digest)
                    for other in paths if other != path]
                audited.append((node, entry, samples))
                break
            except (OSError, ValueError, KeyError, TypeError) as exc:
                failures.append(dict(node=node, path=str(path), error=repr(exc)))
        else:
