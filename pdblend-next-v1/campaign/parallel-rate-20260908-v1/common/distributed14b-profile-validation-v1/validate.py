...
def source_check(spec):
    require(spec['schema']=='distributed14b-target-profile-probes-v1' and spec['authorized'] is True,'explicit probe declaration required')
    ...
    for p in spec['points']:
        ...
        candidates=[x for x in profile['points'] if x['role']=='mixed' and x['tp']==1 and x['frequency_mhz']==p['frequency_mhz'] and x['input_tokens']==p['input_tokens'] and x['batch']==p['batch'] and x['context_tokens']>=p['input_tokens']+p['output_tokens']]
        require(candidates,'probe has no exact input/batch/frequency and sufficient-context A reference')
    return binding
...
def prediction_error(result,p,profile):
    candidates=[x for x in profile['points'] if x['role']=='mixed' and x['tp']==1 and x['frequency_mhz']==p['frequency_mhz'] and x['input_tokens']==p['input_tokens'] and x['batch']==p['batch'] and x['context_tokens']>=p['input_tokens']+p['output_tokens']]
    context=min(x['context_tokens'] for x in candidates); candidates=[x for x in candidates if x['context_tokens']==context]
    return dict(reference_points=candidates,comparison='target native full-batch iteration / original A same-input/batch/frequency nearest sufficient context; latency discrepancy is recorded, not a zero-error gate',iteration_relative_errors=[result['owner_full_batch_iteration_s']/x['iteration_s']-1 for x in candidates],client_ttft_to_reference_prefill_ratios=[result['client_ttft_s']/x['prefill_s'] for x in candidates],not_new_target_profile=True)
