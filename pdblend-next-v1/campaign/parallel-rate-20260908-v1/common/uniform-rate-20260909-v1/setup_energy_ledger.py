...
def audit(path):
    ...
    power = path.parent / ('power/power.csv' if operation or restoration else 'power.csv')
    ...
    energy_key = ('setup_and_correctness_energy_j' if restoration else
                  'full_operation_energy_j' if operation else 'energy_j')
...
def collect(root=ROOT):
    ...
    for node, directory in directories(root):
        ...
        for path in directory.glob('**/measured-restore/status.json'):
            try:
                if 'setup_and_correctness_energy_j' in m.read(path):
