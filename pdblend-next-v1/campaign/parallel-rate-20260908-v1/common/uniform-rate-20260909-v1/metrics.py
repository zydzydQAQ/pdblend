...
def request_metrics(rows, trace, row):
    ...
    for key, prescribed in expected.items():
        ...
        generated += emitted
        terminal_usage = request.get('token_count_source') == 'server_usage' and truth(request.get('token_ids_verified'))
        ...
        exact_tokens = exact_tokens and (terminal_usage or rejected_without_output)
...
def audit_receipt(receipt_path, row, *, receipt_reference=None):
    ...
    result = request_metrics(rows, trace, row)
    ...
    result.update(measurement_valid=True, arrival_qualification=dict(passed=True, **arrival_proof),
        ...
        token_throughput_tps=result['generated_tokens'] / duration,
...
def audit_checkpoint(checkpoint_path):
    ...
    row = checkpoint['row']
    result = audit_receipt(receipt['path'], row, receipt_reference=receipt)
    ...
    result['measurement_host'] = row.get('measurement_host', row['node'])
