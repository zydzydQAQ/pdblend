...
def fixture():
    ...
    transitions=[dict(gpu=g,source_mhz=a,target_mhz=b,actual_duration_s=.08,all8_energy_j=30.)
                 for g in (6,7) for low in (900,1500,2100) for a,b in ((low,2400),(2400,low))]
...
class Tests(unittest.TestCase):
    def test_complete_empirical_two_cards(self):
        ...
        self.assertEqual(sum(x['frequency_mhz']==2400 for x in p['points']),28)
        self.assertTrue(all(x['frequency_mhz']<=2400 for x in p['points']+p['interference_points']))
        self.assertEqual([x for x in p['points'] if x['frequency_mhz']<=2100],[x for x in original['points'] if x['frequency_mhz']<=2100])
        self.assertEqual({x['role'] for x in p['points'] if x['frequency_mhz']==2400},{'mixed','decode'})
        self.assertTrue(all(x['samples']==2 for x in p['points'] if x['frequency_mhz']==2400))
        self.assertTrue(p['frequency2400_registration']['empirical_estimates_not_hard_guarantees'])
    ...
    def test_physical_source_alias_only(self):
        paths=[Path(path).parent for path in r.DRIVERS]
        for name in ('run.py','validate.py','frequency_switch.py','source_order.py','context_export.py','stream.py'):
            self.assertEqual((paths[0]/name).read_bytes(),(paths[1]/name).read_bytes())
        a,b=[r.read(p/'source-order-contract.json') for p in paths]
        self.assertEqual(r.sha(a['engine_logger_source']),r.sha(b['engine_logger_source']))
        b['engine_logger_source']=a['engine_logger_source'];self.assertEqual(a,b)
