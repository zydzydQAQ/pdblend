...
import source_identity_v5 as source_identity
import final_baseline_overlay_v3 as overlay_rules
from baseline_row_metadata_v1 import metadata as row_metadata
import audit_cooperative_arrivals_v1 as arrivals
import audit_eco_observations_v2 as eco_observations
...
def declaration_chain(p,cp,row,logical_ref):
    ...
    if execution_ref is None:return logical_ref
    p.need(isinstance(execution_ref,dict),'unrecognized baseline execution declaration')
    instruction=p.checked(execution_ref)
    if instruction.get('schema')=='A-Eco-final-PDB-execution-scope-v1':
        allowed=[execution_ref,instruction['logical_declaration'],*instruction.get('append_declarations',[])]
        p.need(logical_ref in allowed and cp.get('logical_declaration')==instruction['logical_declaration']
               and cp['row']==row and [r for r in instruction['required_cells'] if r['cell_id']==row['cell_id']]==[row],
               'A scoped execution changed or excluded its preregistered scientific row')
        return execution_ref
    if execution_ref==logical_ref:return logical_ref
...
def inspect(audit,row,checkpoint,declaration_ref,continuation_refs=()):
    ...
    try:
        ...
        executed_metadata=row_metadata(p,executed)
        for key in (*p.PAIR_FIELDS,'system','repeat','slo_scale','arrival_window_s'):
            actual=executed_metadata[key]
        ...
        if continuation is not None:
            ...
            if instruction.get('schema')=='C-Eco37-remaining36-native128-negative-continuation-v1':
                p.need(continuation['sha256']=='b92d704ce668f232a00f6974ee8992f70e7251a798820c1a63a677bc22166db2'
                       and instruction['authorized'] is True and instruction['no_observed_cell_replayed'] is True,
                       'C continuation lacks exact whole-group authority')
                original_rows=p.checked(declaration_ref)['cells']
                retained=instruction['retained_observations']
                old_ids={r['cell_id'] for r in retained}
                p.need(len(retained)==len(old_ids)==1
                       and instruction['remaining_cells']==[r for r in original_rows if r['cell_id'] not in old_ids]
                       and instruction['remaining_cell_ids']==[r['cell_id'] for r in instruction['remaining_cells']]
                       and row['cell_id'] not in old_ids, 'C continuation selected or repeated an observation')
                status=p.checked(instruction['previous_terminal_status'])
                p.need(status.get('finished_s') and status['node_lease_held'] is False,
                       'C previous measurement owner did not finish cleaning')
                for previous in retained:
                    old=p.checked(previous['checkpoint']);diagnosis=p.checked(previous['diagnosis'])
                    p.checked(previous['independent_audit'])
                    p.need(old['row']['cell_id']==previous['cell_id'] and diagnosis['passed'] is True
                           and diagnosis['checkpoint']==previous['checkpoint'], 'C retained negative evidence differs')
            else:
        ...
        if row.get('engineering_replacement') and row['model']=='32b' and row['system']=='dynamollm':
            proof['independent_arrival_engineering_qualification']=arrivals.verify(p,cp,binding,directory,declaration_ref)
        if row['system']=='ecoserve' and ('eco-drain' in row['cell_id'] or row['model']=='14b' and 'baseline-eco-drain' in binding['host_release']):
            proof['independent_eco_observation']=eco_observations.verify(p,cp,binding,summary,directory,checkpoint)
        ...
        if 'independent_eco_observation' in proof:
            eco=proof['independent_eco_observation']
            point.update(failure_classification=eco['classification'],
                classified_native_queue_refusals=eco['classified_native_queue_refusals'])
...
def collect(audit,root,pdb_points,declaration_refs,continuation_refs=(),overlay=None,originals=()):
    p=audit.p
    p.need(overlay is not None,'explicit scientific baseline overlay required')
    ...
    for path in declarations:
        ...
        document=p.read(path)
        for row in document.get('cells',document.get('required_cells',[])):
            if row['system'] not in p.BASELINES:continue
            if row.get('existing_original_rate') and row['cell_id'] not in overlay['fresh']:continue
            cid=row['cell_id']
            if cid in seen:
                p.need(row==seen[cid],'conflicting baseline declaration '+cid);continue
            seen[cid]=row
            metadata=row_metadata(p,row)
            ...
            if not selected and cp is None and cid not in overlay['fresh'] and cid not in overlay['retired']:continue
            point=inspect(audit,row,cp,declaration,continuation_refs)
            overlay_rules.annotate(point,overlay)
            overlay_rules.verify_fresh_identity(p,point,overlay,originals)
            result.append(point)
