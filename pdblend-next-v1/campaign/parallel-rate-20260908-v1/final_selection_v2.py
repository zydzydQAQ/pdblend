...
import final_selection_v1 as prior
from final_selection_v1 import checked, need, sha, read
...
def declared_a_cells(selection):
    ...
    for reference in a_declarations(selection):
        ...
        for row in declaration['cells']:
            need(row['model']=='14b' and row['cell_id'].startswith('parallel-rate-p6-'),
...
def preserve_identity(point,binding,config,checkpoint,selection):
    ...
    if point['model']=='14b':
        ...
        release=checked(spec['formal_release'])
        expected=source_identity_v2.identity(checked(release['binding']),point['dataset'])
