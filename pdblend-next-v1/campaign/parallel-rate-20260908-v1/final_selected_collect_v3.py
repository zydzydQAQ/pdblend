...
import report_versions_v7 as report_versions
import final_selected_baseline_v2 as inspect_baseline
import source_identity_v3 as source_identity
import audit_dynamic
import raw_metrics_v3
import report_scope
import final_selection_v3 as selection_rules
import final_completion_v1 as completion_rules
import final_baseline_overlay_v1 as baseline_overlay
...
def collect(out,selection_path,selection_sha):
    ...
    series="selected-Cp4-Ap7-Bp4"
    if out.exists():raise FileExistsError(out)
    audit=load_audit();p=audit.p
    code_sources={str(path):sha(path) for path in (Path(__file__),ROOT/'boundary_first_loss.py',ROOT/'inspect_exploration.py',ROOT/'report_versions_v7.py',ROOT/'final_selected_baseline_v2.py',ROOT/'source_identity.py',ROOT/'source_identity_v3.py',ROOT/'audit_dynamic.py',ROOT/'raw_metrics_v3.py',ROOT/'raw_metrics_v2.py',ROOT/'report_scope.py',ROOT/'boundary_first_loss_v2.py',AUDIT/'report.py',AUDIT/'protocol.py',AUDIT/'raw_metrics.py')}
    code_sources.update({str(path):sha(path) for path in (ROOT/"collect_v13.py",ROOT/"final_selection_base_v3.py",ROOT/"final_selection_v3.py",ROOT/"final_completion_v1.py",ROOT/"capacity_calibration_compatibility_v1.py",ROOT/"final_baseline_overlay_v1.py")})
    ...
    result=dict(schema=2,created_s=time.time(),candidate_series=series,baseline_snapshot='snapshot-006',
        ...
        archived_baseline_observations=[x for x in baseline_points if not x['selected_for_final_comparison']],
    ...
    write_csv(out/'archived-replaced-baselines.csv',[x for x in baseline_points if not x['selected_for_final_comparison']])
    ...
    lines+=['','原450点保留；另列3个到达时序失真的baseline及整组替换历史，不参与科学胜负或有效曲线。旧版本与失败尝试保留。零GPU锁冲突失败单列于results.json的attempts，不计为性能观测。',
        '本矩阵采用预声明的整模型版本：C/B实际p4关闭容量扩缩，A实际p7开启经资格验证的扩缩，数值证书保留其实际P6来源和专用兼容复用证明。按版本保留实际源码、画像、策略及每次重复；未完成队列仍明确显示缺口。',
