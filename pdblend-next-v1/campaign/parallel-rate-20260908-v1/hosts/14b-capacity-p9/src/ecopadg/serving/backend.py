...
class ClockOwner:
    ...
    async def owned_physical_command(self,loop,fn,event):
        # A cancelled coroutine does not stop a running executor thread. Keep
        # the actual future and a sticky uncertainty record until owned close.
        ...
        try:
            ...
            self.physical_command_uncertainty.append(dict(sequence=sequence,gpu=event['gpu'],
                target_mhz=event['target_mhz'],at_s=time.time(),error=repr(exc),
    ...
    def require_covered_write(self,gpus,frequency,reason,bootstrap=None):
        ...
        if self.pending_physical_commands or self.physical_command_uncertainty:
            event=dict(kind='prior_physical_clock_uncertainty',at_s=time.time(),gpus=list(gpus),
                rejected_target_mhz=frequency,reason=reason,safely_replannable=False,
                pending_sequences=list(self.pending_physical_commands),
                prior_uncertainty=list(self.physical_command_uncertainty),
                retained_commands=dict(self.applied),measurement_confirmation=False)
    ...
    async def fresh_failure_confirmation(self, gpus, frequency, deadline, origin):
        ...
        for gpu in gpus:
            ...
            if not confirmed: pending.append(gpu)
            events.append(dict(kind='physical_clock_failure_confirmation',at_s=decision_s,
                gpu=gpu,target_mhz=frequency,observed_mhz=observed,
                observed_read_started_s=started_s,observed_read_finished_s=finished_s,
                decision_s=decision_s,within_tolerance=confirmed,origin=origin,
                original_settle_timeout_s=self.settle_timeout_s,
                ...
                deadline_extended=False,command_state=dict(self.applied),
                physical_write_attempts=list(self.transaction_writes)))
        if self.pending_physical_commands or self.physical_command_uncertainty:
            ...
            while rising:
                ...
                if getattr(self,'failure_fresh_confirmation',False):
                    if pending and time.monotonic()>=deadline:
                        pending=await self.fresh_failure_confirmation(
                            gpus,frequency,deadline,'active')
                if pending and time.monotonic()>=deadline:
    ...
    async def verify_deferred(self):
        ...
        async with self.lock, self.snapshot_guard():
            ...
            for gpu in tuple(self.deferred):
                ...
                if getattr(self,'failure_fresh_confirmation',False):
                    if not await self.fresh_failure_confirmation(
                            [gpu],item['target'],item['active_since']+self.settle_timeout_s,'deferred'):
                        self.deferred.pop(gpu,None)
                        continue
    ...
    async def close(self):
        async with self.lock, self.snapshot_guard():
            errors=[]
            # No scheduling method clears this barrier. Finish the actual
            # submitted operations before queuing reset commands in this owner.
            pending=list(self.pending_physical_commands.items())
            for sequence,future in pending:
                try:await asyncio.shield(future)
                except Exception:pass  # Physical failure is repaired only by resets below.
            for gpu in self.gpus:
                try:
