...
class Controller:
    def __init__(self,config):
        ...
        if self.capacity_enabled and not (self.strategy.startswith('pdblend')
                and config.get('allow_pd') is False and not config.get('transfers')
                and config.get('dynamic_pools') is False and not config.get('slow_topology')
                and set(config.get('node_gpus',()))==set(range(8))):
    ...
    async def initialize(self,app):
        ...
        if self.capacity_enabled:
            ...
            # Source/actual calibration validation precedes admission or any tick.
    ...
    async def capacity_control(self):
        try:
            while await self.control_tick(1.):
                if self.control_stop.is_set():break
                if self.slow_task is not None and not self.slow_task.done():continue
                self.slow_task=asyncio.create_task(self.capacity_service.tick())
                # A timer cancellation must not abandon a physical transaction.
    ...
    async def quiesce_controls(self):
        ...
        self.control_stop.set()
        if getattr(self,'capacity_service',None):await self.capacity_service.quiesce()
        if getattr(self,'capacity_task',None):await self.capacity_task
    ...
    async def finish_measurement(self, deadline_s):
        ...
        async def finish():
            ...
            if getattr(self,'capacity_service',None):
                # Called only after all workload requests have ended; sampler
                # remains active through removal and restoration of initial2.
    ...
    async def stop(self,app):
        ...
        tasks=[task for task in (self.dispatch_task,self.telemetry_task,self.role_task,self.slow_task,
               getattr(self,'pd_slow_control_task',None),getattr(self,'capacity_task',None)) if task is not None]
