...
# 改动点 (相对 DistServe 原码): 原文件顶部 `import pandas as pd` 移到 mmlu 分支内
# (惰性导入), 使 sharegpt/alpaca/longbench 预处理无需安装 pandas。
import numpy as np
...
def read_dataset(
    ...
    if name.lower() == "sharegpt":
        ...
        import pandas as pd  # 惰性导入 (原码为文件顶部导入)
        dataset = []
        choices = ["A", "B", "C", "D"]
