...
from .admission_diagnostics import reject
...
class JointPlanner:
    def __init__(self, profiles: ProfileStore, transfers=(), *, max_frequency=2520,
                 ...
                 reserved_batch_guard=False, protect_pending_decode=False, residency_horizon=False, park_grace_s=.5,
                 independent_idle_mixed_on_stale_tail=False):
        ...
        self.reserved_batch_guard = reserved_batch_guard
        self.protect_pending_decode = protect_pending_decode
        self.independent_idle_mixed_on_stale_tail = independent_idle_mixed_on_stale_tail is True
    ...
    def candidates(self, snapshot, request, now):
        ...
        plans=[]
        if not paths:reject(self,'state_unavailable',fresh_accepting=len(live))
        tail_dependencies=tuple(i for i in snapshot.instances if i.requests)
        if any(i.timestamp_s > now for i in tail_dependencies):
            reject(self,'state_expired',detail='future telemetry timestamp')
            return plans
        stale_tail=any(not 0 <= now-i.timestamp_s <= self.telemetry_ttl_s
                       for i in tail_dependencies)
        residency_model=None
        if stale_tail:
            if not self.independent_idle_mixed_on_stale_tail:
                reject(self,'state_expired',detail='stale resident tail')
                return plans
            # An unrelated long step cannot lend tail/parking energy credit.
            # Admit only on a fresh endpoint with no work or resource owners.
            paths=[(m,m) for m in live if m.role=='mixed'
                   and m.mode=='continuous' and m.admit_prefill and not m.requests
                   and not m.running and not m.waiting
                   and not m.reserved_kv_tokens and not m.reserved_transfer_bytes
                   and not m.kv_allocations and not m.transfer_allocations]
            tails=None;node_tail=0.
            contexts={m.instance_id:0 for m,_ in paths}
        else:
            tails=TailModel(self,snapshot,now)
            if any(t is None for t in tails.tails.values()):
                reject(self,'profile_missing',detail='unmeasured resident tail')
                return plans
            node_tail=max(tails.tails.values(),default=0.)
            contexts=tails.contexts
            try:
                residency_model = (ResidencyHorizon(self, snapshot, tails, self.park_grace_s)
                                   if self.residency_horizon else None)
            except ValueError:
                reject(self,'profile_missing',detail='unmeasured residency horizon')
        ...
        tail_expiry=min((i.timestamp_s+self.telemetry_ttl_s for i in tail_dependencies if not stale_tail),
                        default=now+self.telemetry_ttl_s)
        points={}
        def point(instance,frequency,batch):
            key=(instance.instance_id,frequency,batch)
            if key not in points:
                points[key]=self.point(instance,request,frequency,batch,context=contexts[instance.instance_id])
            return points[key]
        for p,d in paths:
            ...
            if d.free_kv_tokens - d.reserved_kv_tokens < reservation:
                reject(self,'capacity_insufficient',instance_id=d.instance_id,required_kv=reservation,free_kv=d.free_kv_tokens-d.reserved_kv_tokens)
                continue
            if p is not d and p.free_kv_tokens - p.reserved_kv_tokens < prefill_reservation:
                reject(self,'capacity_insufficient',instance_id=p.instance_id,detail='prefill KV')
                continue
            ...
            batch = max(1, d.running + d.waiting + 1,
                        len(d.requests)+1 if self.reserved_batch_guard else 1)
            source_point=None
            if p is not d:
            ...
            if self.dvfs and any(not r.emitted for r in d.requests):
                fs=(d.frequency_mhz,)
            restriction=getattr(self,'startup_frequency_restrictions',{})
            ...
            for freq in fs:
                ...
                if dp is None or pp is None:
                    reject(self,'profile_missing',instance_id=d.instance_id,frequency_mhz=freq,batch=batch,detail='decode or prefill point')
                    continue
                ...
                if p is d:
                    context=contexts[d.instance_id]
                ...
                if any(cost is None for cost in costs):
                    reject(self,'profile_missing',instance_id=d.instance_id,frequency_mhz=freq,detail='frequency transition')
                    continue
                ...
                if (self.protect_pending_decode and not self.pending_decode_safe(
                    ...
                    reject(self,'deadline_insufficient',instance_id=d.instance_id,frequency_mhz=freq,batch=batch,detail='first token outstanding commitment')
                    continue
                if (ttft > request.ttft_remaining(now) or iteration > request.tpot_s
                    ...
                    reject(self,'deadline_insufficient',instance_id=d.instance_id,frequency_mhz=freq,batch=batch,predicted_ttft_s=ttft,remaining_ttft_s=request.ttft_remaining(now),iteration_s=iteration,tpot_s=request.tpot_s)
                    continue
                ...
                # Standalone node work grants zero unknown-tail or parking
                # credit, even with residency_horizon enabled. This ranking
                # estimate is not an upper bound on unobserved in-flight work.
                after_tail=(ttft+remaining*iteration if stale_tail else
                    tails.after_admission(d,admitted,freq,switch_delay=costs[0][0],
                ...
                if after_tail is None:
                    reject(self,'profile_missing',instance_id=d.instance_id,frequency_mhz=freq,batch=batch,detail='prospective resident tail')
                    continue
                ...
                plans.append(ControlPlan(snapshot.version,now,expires,
                    ...
                    reason=("fresh idle mixed fallback; standalone work estimate without stale tail credit"
                            if stale_tail else "minimum measured incremental energy among feasible paths")))
    ...
    def plan(self, snapshot: RuntimeSnapshot, pending: tuple[RequestBudget,...], *,
        ...
        if not first:
            return ControlPlan(snapshot.version,now,now+self.telemetry_ttl_s,
        ...
        if first[0].reason.startswith('fresh idle mixed fallback;'):
            # Unknown shared work cannot support a multi-request energy beam.
            # Route and frequency are still enumerated against the input SLO.
            return first[0]
