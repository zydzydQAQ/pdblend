from .startup_frequency import admission_planner as startup_admission_planner, committed as startup_committed, failed as startup_failed
...
class Controller:
    ...
    async def recovery_plan(self, snapshot, request, now, *, pressure_pending=None, admission_estimator=None):
        ...
        if self.strategy == 'mixed':
            ...
            plan=await self.planning_executor.run(observed_plan,capacity_admission_plan, admission_estimator or self.planner, recovered, pending, now=now, joint=False, enabled=self.pending_capacity_enabled(), pressure_pending=pressure_pending)
            ...
            while True:
                ...
                while rid in self.active:
                    ...
                    if self.strategy=="mixed":
                        ...
                        plan=await self.planning_executor.run(observed_plan,capacity_admission_plan,admission_estimator,snapshot,
                    ...
                    if self.evaluation_v3 and (plan is None or not plan.feasible):
                        recovered=await self.recovery_plan(snapshot, request, time.time(),pressure_pending=pressure_pending,admission_estimator=admission_estimator)
