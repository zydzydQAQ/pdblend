...
class PhysicalCapacityExecutor:
    ...
    async def _execute(self, operation, gpus, remove_id, budget_s, *, proposal=None,
        ...
        async with self.lock:
            ...
            if proposal is not None and self.deadline_s is not None and time.time() + budget_s + self.cleanup_reserve_s >= self.deadline_s:
                raise ProposalDropped('deadline no longer covers proposal and cleanup')
            require(self.deadline_s is None or time.time() + budget_s + self.cleanup_reserve_s < self.deadline_s,
            ...
            limit = time.time() + budget_s
            if self.deadline_s is not None:
                limit = min(limit, self.deadline_s - self.cleanup_reserve_s)
    ...
    async def _rollback(self, operation, instance, before, transaction):
        limit = time.time() + self.cleanup_reserve_s
        if self.deadline_s is not None:
            limit = min(limit, self.deadline_s)
        if operation == 'restore':
        ...
        for instance in list(self.inventory.value['active_instances']):
            if instance['id'] not in self.inventory.value['initial_ids']:
                await self._execute('remove', tuple(instance['gpus']), instance['id'],
                    (60. if self.deadline_s is None else min(60., max(1., self.deadline_s-time.time()-self.cleanup_reserve_s-1))),
