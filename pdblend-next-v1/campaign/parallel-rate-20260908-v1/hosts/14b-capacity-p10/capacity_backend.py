...
def cancel_transfer_rows(rows, tp):
    ...
    for row in rows:
        require(all((type(row.get(k)) is int and row[k] == 0 for k in ('buffered_tensors', 'buffered_gpu_bytes', 'inflight_receives', 'inflight_sends', 'send_failed'))) and row.get('listener_alive') is True and (row.get('send_counters_observed') is True) and (row.get('send_healthy') is True) and isinstance(row.get('allocations'), dict) and (not row['allocations']) and (type(row.get('send_started')) is int) and (row['send_started'] >= 0) and (type(row.get('send_completed')) is int) and (row['send_started'] == row['send_completed']), 'actual native worker cancellation residue or missing sender evidence')
...
class TransitionMeter:
        ...
        def memory_observer():
            try:
                while not self.memory_stop.is_set():
                    ...
                    self.memory_stop.wait(0.1)
            except Exception as exc:
                self.memory_error = repr(exc)
        self.memory_thread = threading.Thread(target=memory_observer, daemon=True)
        self.memory_thread.start()
        self.sampler = PowerSampler(range(8), interval=0.02, backend=backend, sample_clocks=True)
        self.sampler.start()
    ...
    async def finish(self):
        ...
        if interrupted is not None:
            raise interrupted
        return dict(result, receipt=dict(path=str(self.out / 'measurement.json'), sha256=sha(self.out / 'measurement.json')))
    ...
    def _finish_snapshot(self, finished_s):
        ...
        try:
            ...
            energy = trapezoid_energy(clip_power_window(samples, self.started_s, finished_s, pad_s=0.0))
        except ValueError as exc:
            energy = None
            window_error = repr(exc)
        peaks = {str(g): max((v['used_bytes'] for v in memory if v['gpu'] == g), default=None) for g in range(8)}
        memory_error = getattr(self, 'memory_error', None)
        memory_complete = all((value is not None for value in peaks.values()))
        return dict(measurement_valid=not self.sampler.error and (not stop_error) and (not power_alive) and (not memory_alive) and (not window_error) and (not memory_error) and memory_complete and evidence['power_source_verified'], measurement_start_s=self.started_s, measurement_end_s=finished_s, energy_j=energy, duration_s=finished_s - self.started_s, gpu_indices=list(range(8)), power_evidence=evidence, sampling_error=self.sampler.error, sampling_stop_error=stop_error, power_observer_stopped=not power_alive, memory_observer_stopped=not memory_alive, window_error=window_error, memory_sampling_error=memory_error, memory_sampling_complete=memory_complete, memory_sampling_interval_s=0.1, clock_samples_observed=len(clocks), peak_memory_per_gpu_bytes=peaks, scope='whole-node transition; includes concurrent service; diagnostic overlap, not an extra serving-energy term', artifacts={str(p): sha(p) for p in self.out.iterdir() if p.is_file()})
...
"""
    ...
    def __init__(self, controller, binding, inventory):
        ...
        require(self.service_budget in (2048, 8192) and self.restore_budget in (2048, 8192), 'original per-model service/restore budgets must be explicit')
        self.oracles = fixed(binding['correctness_oracles'])
        require(self.oracles.get('schema') == 'capacity-correctness-oracles-v1' and self.oracles.get('model_source_identity') == self.expected_oracle_identity() and (self.oracles.get('measured') is True), 'measured same-source numerical oracle required')
        require({c['prompt_length'] for c in self.oracles['cases']} == {128, 7168} and all((len(c['prompt']) == c['prompt_length'] and len(c['token_ids']) == c['max_tokens'] == 64 for c in self.oracles['cases'])), 'short and long full64 numerical cases required')
        self.inputs = binding['files']
        require(self.inputs and all((sha(p) == h for (p, h) in self.inputs.items())), 'engine package changed')
        require(binding['engine_entry'] in self.inputs and binding['image'] == binding['identity']['engine_image'], 'current engine entry/image must be pinned')
        require(binding['engine_module'] == 'ecopadg.serving.engine' and str(Path(binding['engine_pythonpath']) / 'ecopadg/serving/engine.py') == binding['engine_entry'], 'original module invocation/source must be explicit; direct script shadows stdlib types')
        self.counter = 0
        self.endpoint_timeout_s = 35.0
    ...
    def allocate(self, gpus):
        require(len(gpus) == self.tp and tuple(sorted(gpus)) == tuple(gpus) and (len(set(gpus)) == len(gpus)) and (set(gpus) <= set(range(8))), 'physical TP allocation invalid')
        self.counter += 1
        ...
        config = dict(copy.deepcopy(self.template), id=iid, tp=self.tp, port=port, kv_port=kv_port, role='mixed', peers={iid: dict(host='127.0.0.1', tp=self.tp, kv_port=kv_port)}, retained_weights=None, scheduler_budget=dict(schema_version=1, max_num_batched_tokens=self.service_budget, max_num_seqs=self.template.get('max_num_seqs', 32)), runtime_dir=str(self.root / iid / 'owner'))
        require(config.get('max_model_len') == 8192, 'ordinary engine model limit differs')
        config_path = self.root / iid / 'engine.json'
        return dict(id=iid, tp=self.tp, gpus=list(gpus), port=port, kv_port=kv_port, url=f'http://127.0.0.1:{port}', role='mixed', mode='continuous', native_kind=self.binding['native_kind'], container_name='pdb-v2-' + iid, engine_config=str(config_path), planned_config=config, owner_id=self.owner, service_budget_tokens=self.service_budget, restore_budget_tokens=self.restore_budget, scheduler_cache_observed=True, scheduler_cache_count=self.binding.get('scheduler_cache_count', 1))
    ...
    async def command(self, args, limit, cap=30):
        ...
        try:
            ...
            if child.returncode is None:
                ...
                await child.wait()
            raise
        record = dict(argv=args, at_s=time.time(), exitcode=child.returncode, stdout=stdout.decode(errors='replace'), stderr=stderr.decode(errors='replace'))
    ...
    async def gpu_state(self, gpus):
        ...
        def inspect():
            ...
            for g in gpus:
                ...
                rows.append(dict(gpu=g, at_s=time.time(), free_bytes=int(mem.free), used_bytes=int(mem.used), process_pids=[int(p.pid) for p in procs]))
            return rows
    ...
    async def inspect(self, instance, limit):
        ...
        row = rows[0]
        require(row['Name'].lstrip('/') == instance['container_name'] and row['Image'] == self.binding['image'] and (row['Config'].get('Labels', {}).get('pdblend.capacity.owner') == self.owner), 'created container ownership/image differs')
    ...
    async def clock_ownership_proof(self, instance, operation):
        ...
        check_lease(authority=config.get('capacity_lease_authority'), expected_inventory=config['capacity_inventory_path'], expected_job_path=config.get('capacity_job_path'))
        inventory = self.inventory.value
        known = inventory['known_instances'].get(instance['id'], {})
        allowed = ('start_intent',) if operation == 'bootstrap' else ('stop_intent', 'start_intent', 'started_unpublished', 'ready_unpublished')
        require(inventory['transition_inflight'] is True and instance['id'] not in inventory['initial_ids'] and (known.get('owner_kind') == 'created_for_cell') and (known.get('state') in allowed) and known.get('transaction') and (known.get('gpus') == instance['gpus']), 'exact owned transition clock intent required')
        routed = self.controller.backend.instances.values()
        require(not set(instance['gpus']) & {g for item in routed for g in item['gpus']}, 'clock bootstrap/release GPU remains published')
        rows = await self.assert_spare(instance['gpus'])
        require({row['gpu'] for row in rows} == set(instance['gpus']) and len(rows) == len(instance['gpus']) and all((0 <= time.time() - row['at_s'] <= 1.0 for row in rows)), 'fresh complete empty GPU evidence required')
        self.inventory.event('clock_' + operation + '_proof', instance_id=instance['id'], transaction=known['transaction'], gpus=instance['gpus'], rows=rows)
        return dict(schema='capacity-clock-' + operation + '-v1', instance_id=instance['id'], transaction=known['transaction'], inventory_path=str(self.inventory.path), inventory_sha256=sha(self.inventory.path), gpus=instance['gpus'], rows=rows)
    ...
    async def start(self, instance, before, limit):
        ...
        require(engine_argv and all((type(v) is str for v in engine_argv)) and isinstance(env, dict), 'explicit actual engine command/environment required')
        args = ['docker', 'run', '-d', '--name', instance['container_name'], '--gpus', 'all', '--network', 'host', '--ipc', 'host', '--label', 'pdblend.capacity.owner=' + self.owner]
        for option in self.binding.get('security_options', ['label=disable']):
            args += ['--security-opt', option]
        for (key, value) in env.items():
            args += ['-e', key + '=' + value]
        args += ['-v', '/root/workspace:/root/workspace', '-v', '/root/workspace/models:/models:ro', self.binding['image']] + engine_argv
        if self.controller.backend.clocks:
            async with self.controller.action_lock:
                if self.controller.config.get('measured_frequency_write_guard_v1') is True:
                    proof = await self.clock_ownership_proof(instance, 'bootstrap')
                    await self.controller.backend.clocks.set(instance['gpus'], getattr(self.controller, 'max_service_frequency_mhz', 2520), verify_rise=False, bootstrap=proof)
                else:
                    await self.controller.backend.clocks.set(instance['gpus'], getattr(self.controller, 'max_service_frequency_mhz', 2520))
        cid = (await self.command(args, limit, 30)).strip()
        row = await self.inspect(instance, limit)
        require(row['Id'] == cid and row['State']['Running'] and (row['State']['Pid'] > 0), 'actual new physical process absent')
        require(row['Config']['Cmd'] == engine_argv and row['HostConfig']['NetworkMode'] == 'host' and (row['HostConfig']['IpcMode'] == 'host') and (row['HostConfig'].get('AutoRemove') is False), 'actual creation contract differs')
        ...
        return dict(container=dict(id=cid, name=instance['container_name'], image=row['Image'], StartedAt=row['State']['StartedAt'], host_pid=row['State']['Pid']), host_pid=row['State']['Pid'], config_sha256=sha(path))
    ...
    async def verify(self, instance, limit):
        ...
        expected = dict(self.binding['expected_provenance'], instance_id=instance['id'], cuda_visible_devices=','.join(map(str, instance['gpus'])), tp=instance['tp'])
        ...
        if observed_budget is not None:
            require(observed_budget.get('max_num_batched_tokens') == self.service_budget, 'actual native per-model service budget differs')
        else:
            require(self.binding['native_kind'] == 'legacy_sync_put' and self.binding.get('budget_evidence_mode') == 'static_engine_initialization' and (self.service_budget == self.restore_budget == self.template.get('max_num_batched_tokens') == 8192) and (self.template.get('max_num_seqs') == 32), 'legacy missing dynamic budget ACK only supports verified static8192/32 initialization')
        replies = []
        for case in self.oracles['cases']:
            request_id = 'capacity-verify-' + uuid.uuid4().hex
            body = dict(prompt=case['prompt'], max_tokens=case['max_tokens'], ignore_eos=True, temperature=0, stream=False, request_id=request_id)
            ...
            require(output.get('token_ids') == case['token_ids'] and output.get('usage', {}).get('prompt_tokens') == case['prompt_length'] and (output.get('usage', {}).get('completion_tokens') == case['max_tokens']), 'new replica numerical output/original work differs')
        ...
        return dict(provenance=provenance, numerical_correctness=True, ordinary_replies=replies, runtime=raw, cancellation=cancellation, independent_mixed_only=True, peer_reconfiguration_performed=False)
    ...
    async def verify_cancel(self, instance, limit):
        ...
        body = dict(prompt=case['prompt'], max_tokens=512, ignore_eos=True, temperature=0, stream=False)
        self.inventory.event('correctness_request_intent', instance_id=instance['id'], request_id=request_id, controlled_cancel=True)

        async def issue():
            async with self.controller.session.post(instance['url'] + '/v1/completions', json=body, headers={'X-Request-Id': request_id}, timeout=aiohttp.ClientTimeout(total=max(0.1, limit - time.time()))) as response:
        ...
        try:
            ...
            response = await asyncio.wait_for(child, max(0.1, limit - time.time()))
            require(response['status'] >= 400 and 'cancel' in response['body'].lower(), 'controlled request did not terminate as cancellation')
            while time.time() < limit:
                settled = await self.request(instance, '/runtime', limit=limit, cap=1)
                if not engine_residual(settled, time.time()):
                    return dict(request_id=request_id, before=raw, cancelled=reply, response=response, settled=settled, verified=True)
                await asyncio.sleep(0.05)
            ...
            if not cancelled:
                try:
                    ...
                    self.inventory.event('cancel_cleanup_error', instance_id=instance['id'], request_id=request_id, error=repr(exc))
            if not child.done():
                child.cancel()
    ...
    async def resume_idle(self, instance, limit):
        raw = await self.request(instance, '/runtime', limit=limit)
        await self.request(instance, '/control', dict(generation=raw['generation'] + 1, role='mixed', mode='continuous', admit_prefill=True, admit_decode=True), limit=limit)
    ...
    async def is_same_running(self, instance):
        try:
            ...
            return bool(row['State']['Running'] and row['State']['StartedAt'] == original.get('StartedAt') and (row['State']['Pid'] == original.get('host_pid')))
