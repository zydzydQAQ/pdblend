...
def online_profile_points(profiles, role, input_tokens, context_tokens, frequency_mhz=2520):
    ...
    """
    queries = sorted({(p.tp, p.batch) for p in profiles.points if p.role == role and p.frequency_mhz == frequency_mhz})
    resolved = (profiles.lookup(role, tp, frequency_mhz, input_tokens, context_tokens, batch) for (tp, batch) in queries)
...
class DistServeSearch:
    ...
    def __init__(self, profiles, transfers, gpu_count=8, topology=None, *, max_service_frequency_mhz=2520):
    ...
    def search(self, input_tokens, predicted_output, ttft_s, tpot_s, rate, kv_capacity):
        ppoints = online_profile_points(self.profiles, 'prefill', input_tokens, input_tokens + 1, frequency_mhz=getattr(self, 'max_service_frequency_mhz', 2520))
        dpoints = online_profile_points(self.profiles, 'decode', input_tokens, input_tokens + predicted_output, frequency_mhz=getattr(self, 'max_service_frequency_mhz', 2520))
        candidates = []
        for (p, d) in itertools.product(ppoints, dpoints):
            links = [t for t in self.transfers if (t.source_tp, t.target_tp) == (p.tp, d.tp) and t.max_input_tokens >= input_tokens and (t.profile_batch >= p.batch) and t.source_sha256 and t.validated]
            if not links:
                continue
            ...
            for np in range(1, self.gpu_count // p.tp + 1):
                for nd in range(1, self.gpu_count // d.tp + 1):
                    ...
                    if used > self.gpu_count:
                        continue
                    capacity = min(np * p.batch / p.phase_time_bound('prefill'), nd * d.batch / (d.iteration_s * d.bound * max(predicted_output, 1)))
                    if capacity < rate:
                        continue
                    for groups in self.placements(p.tp, d.tp, np, nd):
                        measured = []
                        for (source, target) in itertools.product(groups[:np], groups[np:]):
                            link = self.transfer_point(p.tp, d.tp, source, target, input_tokens, p.batch, decode_frequency_mhz=getattr(self, 'max_service_frequency_mhz', 2520))
                            if link is None:
                                break
                            measured.append(link.seconds_upper)
                        else:
                            bound = max(measured)
                            capacity = min(np * p.batch / (p.phase_time_bound('prefill') + bound), nd * d.batch / (d.iteration_s * d.bound * max(predicted_output, 1)))
                            if capacity >= rate and p.phase_time_bound('prefill') + bound + d.iteration_s * d.bound <= ttft_s:
                                candidates.append(Placement(p.tp, d.tp, np, nd, p.batch, d.batch, capacity, groups, bound))
...
class DynamoLLMPolicy:
    ...
    """
    PERIODS = {'ScaleInst': 1800.0, 'ScaleShard': 300.0, 'ScaleFreq': 5.0}
    ...
    def forecast(self, now, window_s=300):
        return {name: sum((now - window_s <= t <= now for t in samples)) / window_s for (name, samples) in self.arrivals.items()}

    def amortizes(self, operation, saving_w, horizon_s, lost_capacity_j=0):
        cost = self.costs.get(operation)
        return bool(cost and cost.source_sha256 and (saving_w * max(horizon_s - cost.duration_upper_s, 0) > cost.energy_upper_j + lost_capacity_j))
