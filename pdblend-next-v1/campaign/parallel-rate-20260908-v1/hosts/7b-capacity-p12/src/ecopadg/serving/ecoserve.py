...
class EcoServeScheduler:
    ...
    def __init__(self, profiles, instances, *, lower=2, upper=3, max_service_frequency_mhz=2520):
    ...
    def prefill(self, i, r):
        p = self.profiles.lookup('mixed', i.tp, getattr(self, 'max_service_frequency_mhz', 2520), r.input_tokens, r.input_tokens + 1, 1)
    ...
    def feasible(self, i, r, now, switch):
        ...
        credits = [q.emitted * q.tpot_s - (now - q.first_token_s) for q in i.requests if q.arrival_s < switch and q.first_token_s is not None]
        reserve = (r.input_tokens + (r.output_limit or r.predicted_output) + 15) // 16 * 16
        if total > r.ttft_remaining(now) or (credits and statistics.mean(credits) < total) or i.free_kv_tokens - i.reserved_kv_tokens < reserve:
            return None
    ...
    def plan(self, snapshot, pending, *, now):
        ...
        for group in groups:
            ...
            for offset in range(len(group)):
                ...
                if choice is None:
                    continue
                windows = tuple((WindowAction(x, states[x].generation, x == rid) for x in group if states[x].mode != 'temporal' or states[x].admit_prefill != (x == rid)))
                (total, reserve) = choice
                return ControlPlan(snapshot.version, now, now + 1, routes=(RouteAction(r.request_id, rid, rid, reserve, total, 0, 0),), frequencies=(FrequencyAction(rid, getattr(self, 'max_service_frequency_mhz', 2520)),), windows=windows, reason='EcoServe: current-then-cyclic window, accumulated prefill, mean saved TPOT and KV')
        return ControlPlan(snapshot.version, now, now + 1, feasible=False, reason='EcoServe: no macro instance satisfies constraints')
    ...
    def membership_plan(self, snapshot, now):
        ...
        windows = tuple((WindowAction(i.instance_id, i.generation, i.instance_id in chosen) for i in snapshot.instances if i.mode != 'temporal' or i.admit_prefill != (i.instance_id in chosen)))
        return ControlPlan(snapshot.version, now, now + 1, windows=windows, reason='EcoServe macro membership commit: one selected prefill window per group')
