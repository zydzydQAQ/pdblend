...
async def current_clock_first_plan(controller, plan, request, *, completion_recovery=False):
    ...
    async with clocks.lock, controller.state.lock:
        ...
        if controller.config.get('idle_domain_reacquire_v1') is True and not targets:
            from .idle_domain_reacquire import reacquire
            result = await reacquire(controller, instance, plan, request,
                                     event['observations'], frequencies)
            if result is not None:
                snapshot, instance, target, observations = result
                targets = [target]
                event['observations'] = observations
                event['idle_domain_reacquired'] = True
                event['reacquired_snapshot_version'] = snapshot.version
        legacy_idle=False
        if not targets:
