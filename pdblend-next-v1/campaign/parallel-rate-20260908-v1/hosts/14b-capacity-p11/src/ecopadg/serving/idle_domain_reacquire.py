...
async def reacquire(controller, instance, plan, request, observations, frequencies):
    ...
    try:
        ...
        deadline = time.monotonic()+clocks.settle_timeout_s-(time.time()-last_finished)
        ...
        if not confirmed:
            raise ClockWriteUncertain('idle reacquisition not confirmed within original settling bound')
        ownership()
        for gpu in instance.gpus:
        ...
        event.update(confirmed=True, physical_state_unknown=False,
            confirmed_before_original_settle_deadline=not current_only,
