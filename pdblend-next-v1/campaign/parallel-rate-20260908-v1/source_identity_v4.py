...
def identity(binding, dataset):
    ...
    if is_p8_reuse:
        for name, reference in reuse_proof['approved_patch_sources'].items():
    ...
    result.update(capacity_layout_measured_controller_manifest=semantics['candidate_manifest'],
        capacity_transition_measured_controller_manifest=actual_manifest if is_p8_reuse else semantics['candidate_manifest'],
        capacity_transition_source_selection=reuse_proof.get('transition_source_selection') if reuse_proof else None,
