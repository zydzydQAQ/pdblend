    ...
    async def case():
        ...
        try:
            async with c.action_lock:
                ...
                with pytest.raises(ClockWriteUncertain):await c.backend.execute(chosen)
            assert not await c.backend.confirm(chosen)
        finally:await close(c,x)
    asyncio.run(case())


def test_frozen_source_composition_and_default_feature():
    import importlib.util,hashlib
    root=Path(__file__).resolve().parent
    spec=importlib.util.spec_from_file_location('final_build',root/'build_p2_final.py')
    ...
    assert (HOST/'src/ecopadg/serving/runtime.py').read_text()==mod.runtime((parent/'src/ecopadg/serving/runtime.py').read_text())
    assert (HOST/'src/ecopadg/serving/planner.py').read_bytes()==(parent/'src/ecopadg/serving/planner.py').read_bytes()
    assert (HOST/'src/ecopadg/serving/backend.py').read_bytes()==(root/'hosts'/f"{manifest['model']}-fixed-p1/src/ecopadg/serving/backend.py").read_bytes()
