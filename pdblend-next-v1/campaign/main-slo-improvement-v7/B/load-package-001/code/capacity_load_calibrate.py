...
def validate_spec(spec):
    ...
    if 'legacy_idle_measurement' in spec:
        helper=spec['legacy_idle_measurement']
        require(binding.get('backend_kind')=='guarded_legacy_self_only_v1'
            and sha(helper['path'])==helper['sha256']
            and spec['files'].get(helper['path'])==helper['sha256'],
            'legacy idle helper requires exact source and guarded backend declaration')
...
async def measure_idle(controller, out, spec, duration_s):
    ...
    if 'legacy_idle_measurement' in spec:
        binding=fixed(spec['capacity_binding']);helper=spec['legacy_idle_measurement']
        require(binding.get('backend_kind')=='guarded_legacy_self_only_v1'
            and sha(helper['path'])==helper['sha256']
            and spec['files'].get(helper['path'])==helper['sha256'],
            'unfrozen legacy idle helper')
        for item in binding['source_composition']['sources'].values():
            require(sha(item['path'])==item['sha256'] and binding['files'].get(item['path'])==item['sha256'],
                    'legacy idle backend/guard source changed')
        guard=binding['source_composition']['sources']['guard']
        require('legacy_isolation' in sys.modules and sha(sys.modules['legacy_isolation'].__file__)==guard['sha256'],
                'legacy idle helper must share the actually loaded physical guard')
        module=load(helper['path'],'legacy_idle_'+helper['sha256'])
        return await module.measure_idle_legacy(controller,out,spec,duration_s)
    from capacity_backend import TransitionMeter
    require(duration_s >= 60, 'idle evidence requires at least sixty seconds')
    require(time.time()+duration_s+120 < spec['deadline_s'], 'idle and cleanup exceed deadline')
    from ecopadg.serving.completion_policy import engine_residual
...
async def execute(spec,out):
    ...
    try:
        ...
        execution_binding=dict(original,deadline_s=spec['deadline_s'])
