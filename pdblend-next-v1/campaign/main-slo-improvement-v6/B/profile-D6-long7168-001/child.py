...
import aiohttp
from capacity import validate_runtime_capacity, validate_declared_batch_observation
import late_context
...
class DispatchGuard:
 ...
 def record(self,method,url,kwargs):
  ...
  self.journal.flush();os.fsync(self.journal.fileno()) # Durable intent before first network await.
  ...
  async def measure(self,point,path):
   ...
   if not raw.get('error'):
    try:
     ...
     raw['declared_batch_evidence']=validate_declared_batch_observation(raw,events)
