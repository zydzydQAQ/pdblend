...
import aiohttp
from capacity import validate_runtime_capacity, validate_execution_release
...
OLD = REPO/'campaign/B32B-engine-v3-candidate-v2'
RELEASE = REPO/'releases/io-v3-runtime'
PORTS = (33500, 33501)
...
def predecessor():
    authority=json.loads((ROOT/'priority-authorization.json').read_text())
    require(authority['authorized'] is True and authority['automatic_retries'] is False,
            'new authorized priority profile declaration required')
    require(time.time()+720 < authority['deadline_s'], 'profile work/cleanup cannot fit original deadline')
    reference=authority['predecessor_release'];require(sha(reference['path'])==reference['sha256'],'v5 release changed')
    stage=Path(authority['predecessor_output']);status=json.loads((stage/'status.json').read_text())
    order=json.loads((stage/'declaration-order.json').read_text());expected=[r['cell_id'] for r in order]
    require(len(expected)==16 and all(r['model']=='32b' and r['stage']=='screen_fixed2' for r in order),
            'exact B v5 sixteen declared points required')
    require(status.get('complete') is True and status.get('phase')=='complete'
            and status.get('completed')==expected and not status.get('failed')
            and status.get('pid')==authority['predecessor_pid'] and not Path('/proc',str(status['pid'])).exists(),
            'running or incomplete B v5 stage cannot release this profile')
    checkpoints=[]
    for cid in expected:
        path=stage/'results/checkpoints'/(cid+'.json');cp=json.loads(path.read_text())
        require(cp['row']['cell_id']==cid and cp.get('measurement_valid') is True
                and sha(cp['receipt'])==cp['receipt_sha256']
                and all(sha(p)==h for p,h in cp['artifacts'].items()),'completed v5 checkpoint changed')
        checkpoints.append(dict(path=str(path),sha256=sha(path)))
    prior=authority['prior_micro'];prior_root=Path(prior['path'])
    require(sha(prior_root/'manifest.json')==prior['manifest_sha256'],'previous independent micro source changed')
    prior_launch=json.loads((prior_root/'launch-confirmation.json').read_text())
    require(prior_launch.get('source_manifest_sha256')==prior['manifest_sha256']
            and type(prior_launch.get('pid')) is int and prior_launch['pid']>1, 'actual prior micro process identity required')
    prior_state=json.loads((prior_root/'status.json').read_text())
    require(prior_state.get('complete') is True and prior_state.get('phase')=='finished'
            and prior_state.get('cleanup_complete') is True and prior_state.get('passed') is True
            and not Path('/proc',str(prior_launch['pid'])).exists(), 'previous micro must finish and release its clean owner')
    require(sha(prior_root/'late-context-evidence.json')==prior_state['late_context_evidence_sha256'],
            'previous completed context evidence changed')
    reference=authority['original_binding'];require(sha(reference['path'])==reference['sha256'],'actual PDB binding changed')
    return dict(authorization_sha256=sha(ROOT/'priority-authorization.json'),
        v5_status_sha256=sha(stage/'status.json'),checkpoints=checkpoints,
        old_baseline_or_ablation_called_complete=False,deadline_s=authority['deadline_s'])


def check_short_capacity(evidence):
    instances = evidence.get('instances', [])
    require(len(instances) == 2, 'both retained TP2 instances must be observed')
    for expected_id, item in zip(IDS, instances):
        require(item['runtime']['id'] == expected_id, 'live instance ordering/identity differs')
        validate_runtime_capacity(item['runtime'], require_accepting=True)
...
class Gate:
    def __init__(self):
        ...
        self.state = dict(complete=False, passed=False, phase='preflight', started_s=time.time(),
            scope='6 natural batch6 TP2 input7168 output512 observations; no profile publication or KV certification',
    ...
    async def identity(self,label):
        check()
        authority=json.loads((ROOT/'priority-authorization.json').read_text())
        reference=authority['original_binding']
        require(sha(reference['path'])==reference['sha256'],'fresh restored PDB binding changed')
        binding=json.loads(Path(reference['path']).read_text())
        binding['deadline_s']=authority['deadline_s']
        common=load('priority_b_original_identity',REPO/'campaign/five-system-execution-v3/run.py')
        common.validate_binding(binding)
        actual=await common.identity(self.session,binding)
        expected=json.loads((ROOT/'expected-identity.json').read_text())
        target=next(r for r in actual if r['runtime']['id']=='nextv3b0')
        require(target['container']['State']['Pid']==expected['container_pid']
                and target['container']['State']['StartedAt']==expected['container_started_at'],
                'same measured restored owner process required')
        evidence=dict(instances=actual,original_binding=reference,full_model_hashes_checked_by_child=True)
    ...
    async def work(self,hardware):
        ...
        # Full identity work can outlive an early runtime sample. Refresh both
        # already-verified endpoints before authorizing any mutation.
        fresh=await asyncio.gather(*(self.runtime(port,'capacity-before-controls') for port in PORTS))
        self.state['capacity_before_controls']=dict(instances=[dict(runtime=r) for r in fresh])
        check_short_capacity(self.state['capacity_before_controls'])
        ...
        self.child=await asyncio.create_subprocess_exec('python3','-u',str(ROOT/'child.py'),'--parent-pid',str(os.getpid()),cwd=str(ROOT),
        ...
        for point in report['points']:
            raw=json.loads((Path(point['path'])/'raw.json').read_text())
            references.append(raw['declared_batch_evidence']['output_reference_sha256'])
...
if __name__=='__main__':
    parser=argparse.ArgumentParser();parser.add_argument('--check',action='store_true');parser.add_argument('--execute',action='store_true');parser.add_argument('--release',type=Path);args=parser.parse_args()
    require(not(args.check and args.execute), 'choose check or explicit execute')
    EXECUTION_RELEASE=args.release
    check()
    if not args.execute:print(json.dumps(dict(package_valid=True,planned_points=6,gpu_executed=False,scheduled=False,required_kv_tokens=46080)))
    else:
        predecessor() # Reject an absent/stale release before waiting for the node lease.
