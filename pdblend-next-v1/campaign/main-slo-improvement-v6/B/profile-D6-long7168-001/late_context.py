...
def derive_all(root, *, original_evidence, tp2_validator):
    ...
    for index in range(6):
        ...
        if not (spec and spec.get('batch_size') == 6 and spec.get('input_lengths') == [7168]*6
                and spec.get('output_lengths') == [512]*6 and spec.get('clock_command_mhz') == (1500 if index < 3 else 2520)
                and spec.get('budget_tokens') == 8192 and spec.get('max_num_seqs') == 32
                and spec.get('target_gpus') == [0, 1] and spec.get('tp') == 2
                and spec.get('seed') == 0 and spec.get('temperature') == 0
                and spec.get('arrival_offsets_s') == [0.]*6):
            result['valid'] = False
            result['errors'].append(dict(stage='declaration', error='not the six independently declared current B work points'))
        if path.exists():
