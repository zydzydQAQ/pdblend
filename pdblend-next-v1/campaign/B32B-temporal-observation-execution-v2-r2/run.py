...
def preserve_container(before,after):
    for key in ('Id','Image','Config','HostConfig','Mounts','Path','Args'):
...
class Operation:
    ...
    async def run_child(self):
        ...
        job=dict(binding=self.diag_binding,binding_path=str(self.out/'diagnostic-binding.json'),binding_sha256=c.sha(self.out/'diagnostic-binding.json'),
            ...
            output_dir=str(self.out/'child'),work_deadline_s=self.limits['work_end_s'],cleanup_deadline_s=self.limits['cleanup_end_s'])
        ...
        while self.child.returncode is None:
            if self.stop or (c.ROOT/'STOP').exists() or (self.out/'STOP').exists() or self.sampler.error or not self.before_deadline():
                self.state['work_interrupt_reason']='STOP/deadline/sensor';self.deadline=self.limits['cleanup_end_s'];self.restoring=True
...
def main():
    ...
    c.require(Path(spec['results']).parent==a.spec.resolve().parent and Path(spec['results']).name=='results' and a.spec.resolve().parent.parent==c.ROOT.parent
        and a.spec.resolve().parent.name.startswith('B32B-temporal-observation-attempt-') and spec['attempt_claim']==str(c.ROOT/'execution-once.json'),'isolated attempt namespace differs')
    ...
    for revision in ('v1','v2'):
        c.require(not (c.ROOT.parent/('B32B-temporal-observation-execution-'+revision)/'execution-once.json').exists(),'prior attempt claim exists; no automatic retry')
