...
def test_new_solo_pair_observation_preserves_all_six_original_requests(tmp_path):
    _,j,s=job(tmp_path)
    assert child.validate_job(j)==s
    assert s['request_ids']==['2f1aba32e37b50a0bc5046049a36677d','098e756232a3552f8a0e2cd88f93959e']
    old=child.read(child.ROOT.parent/'B32B-temporal-observation-candidate-v1/specs/original-vs-continuous.json')
    current=copy.deepcopy(s);current['output_dir']=old['output_dir'];current['request_ids']=old['request_ids']
    assert current==old

def test_old_pair_observation_uuids_rejected_before_any_http(tmp_path):
    jp,j,s=job(tmp_path)
    s['request_ids']=[s['requests'][3]['request_uuid'],s['requests'][5]['request_uuid']]
    child.write(j['observation_spec'],s);j['observation_spec_sha256']=child.sha(j['observation_spec']);child.write(jp,j)
    with pytest.raises(RuntimeError,match='only output_dir may differ'):
        child.validate_job(j)
    def forbidden():raise AssertionError('old UUID scope must not create HTTP session')
    result=asyncio.run(child.execute(jp,session_factory=forbidden))
    assert result['completed_requests']==0 and not result['complete'] and not result['cleanup_complete']
    assert result['phase']=='terminal' and result['finished_s'] is not None and result['errors']
