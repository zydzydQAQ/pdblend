...
BASE=ROOT.parent/'B32B-capacity-probes-fixed-window-v2'
...
def audit(cid):
 ...
 for r in records:
  ...
  full=yes(r['success']) and int(r['generated_tokens'] or 0)==req['output_len']
  ttft,tpot=number(r['ttft_s']),number(r['tpot_s'])
  ok=full and ttft is not None and tpot is not None and ttft<=row['slo_ttft_s'] and tpot<=row['slo_tpot_s']
  require(yes(r['slo_ok'])==ok,'joint SLO score differs')
  if full:require(int(r['input_tokens'] or 0)==req['prompt_len'] and yes(r['token_ids_verified']),'completed work/token identity differs')
  else:failures.append(dict(idx=idx,error=r['error'],generated=int(r['generated_tokens'] or 0),expected=req['output_len']))
  if not ok:misses.append(dict(idx=idx,ttft_s=ttft,tpot_s=tpot,complete=full))
  complete+=full;good+=ok;generated+=int(r['generated_tokens'] or 0);inputs+=int(r['input_tokens'] or 0);dispatch.append(float(r['actual_dispatch_s']))
...
if __name__=='__main__':
 p=argparse.ArgumentParser();p.add_argument('--cell',default='32b-alpaca-r2-s701-w300-scale1');a=p.parse_args();r=audit(a.cell)
