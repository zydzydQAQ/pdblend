...
FIXED_WINDOW_PROTOCOL='a14b-sharegpt-slo90-v1'
COMPARISON_SYSTEMS=('pdblend','mixed','distserve','dynamollm','ecoserve')
WINDOW_STRATEGIES=('pdblend-greedy','pdblend-joint','pdblend-dynamic','mixed',
                   'distserve','ecoserve','dynamollm','dynamollm-resident')
...
def configure_fixed_window(args, config, trace):
    ...
    if (protocol != FIXED_WINDOW_PROTOCOL or config.get('evaluation_protocol') != EVALUATION_V3
            or args.split != 'development' or config['strategy'] not in WINDOW_STRATEGIES
            or args.dataset not in DATASET_SLOS):
        raise ValueError('fixed-window protocol requires one declared five-system evaluation-v3 development strategy')
    if (config.get('arrival_window_s') != 100 or trace.get('arrival_window_s') != 100
            or trace.get('protocol_id') != protocol or trace.get('measurement_schema') != 3
            or trace.get('duration_s') != 100 or trace.get('split') != 'development'
            or trace.get('dataset') != args.dataset or trace.get('seed') != args.seed
            or args.seed != 701 or trace.get('comparison_systems') != list(COMPARISON_SYSTEMS)
            or trace.get('request_hard_timeout_s') != 120
            or trace.get('post_window_drain_allowance_s') != 120):
        raise ValueError('fixed-window trace/config identity or 100-second shared window differs')
    ...
    if (any(type(x) not in (int,float) or not math.isfinite(x) or not 0 <= x < 100 for x in offsets)
            or offsets[0] != 0 or offsets != sorted(offsets)):
        raise ValueError('fixed-window arrivals must start at zero and lie in [0,100)')
    scale=getattr(args,'slo_scale',None)
    if scale is None: scale=config.get('slo_scale',1.)
    if type(scale) not in (int,float) or scale not in (.5,2.):
        raise ValueError('SLO scale must be 0.5 or 2')
    ...
    comparison_system=('pdblend' if strategy.startswith('pdblend') else
                       'dynamollm' if strategy=='dynamollm-resident' else strategy)
    if config.get('comparison_system',comparison_system) != comparison_system:
        raise ValueError('comparison system differs from the actual strategy')
    config['comparison_system']=comparison_system
    return dict(protocol_id=protocol,arrival_window_s=100.,slo_scale=scale,
    ...
    try:
        ...
        if window_result:
            ...
            summary.update(measurement_window_protocol=FIXED_WINDOW_PROTOCOL,
                ...
                comparison_system=config['comparison_system'],
                implementation_variant=config['strategy'],
                implementation_scope=('resident_only; not full DynamoLLM' if config['strategy']=='dynamollm-resident'
                                      else 'actual configured strategy; mechanism evidence remains separate'),
