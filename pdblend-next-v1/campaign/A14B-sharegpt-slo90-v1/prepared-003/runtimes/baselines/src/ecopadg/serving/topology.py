...
class DockerLifecycle:
    ...
    async def start(self,spec,peers,retained_weights=None):
        ...
        return await self.command('run','-d','--name',self.container_name(spec),
            ...
            '-e','PYTHONPATH='+self.template.get('observation_engine_pythonpath','/root/workspace/pdblend/src'),'-e','VLLM_HOST_IP=127.0.0.1',
...
class TopologyManager:
        ...
        async with self.lock:
            ...
            try:
                if self.backend.clocks and add_specs:
                    await self.backend.clocks.set(sorted({g for s in add_specs for g in s.gpus}),2520)
                for spec in remove:
    ...
    async def rollback(self,before,remove,stopped,created,retained_weights,transaction):
        ...
        if self.backend.clocks:
            await self.backend.clocks.set(sorted({g for s in restored for g in s.gpus}),2520)
