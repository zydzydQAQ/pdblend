...
def point(row,binding,evidence):
    ...
    p.update(status='not_bound' if binding is None else 'pending',checkpoint_verified=False,metrics_verified=False,
        error=None,receipt_path=None,checkpoint_path=None,implementation_variant=None)
...
def snapshot(out,*,bindings=(),retained_attempt_bindings=(),counts_only=False,plots=False):
    ...
    counts=workload_counts(manifests,evidence);by_system={}
    for path in bindings:
        b=evidence.read(path);key=b['model'],b['system']
        require(key not in by_system and key[0] in MODELS and key[1] in SYSTEMS and b['protocol_id']==PROTOCOL,
            'wrong or duplicate model/system binding')
        manifest_path,manifest_sha=DEFAULT_SOURCES[b['model']]
        require(b['files'].get(str(manifest_path))==manifest_sha,'binding does not freeze the exact shared source')
        b['_sha256']=evidence.files[str(Path(path).resolve())];by_system[key]=b
    points=[] if counts_only else [point(r,by_system.get((r['model'],r['system'])),evidence) for r in declarations]
    attempts=[]
    for path in retained_attempt_bindings:
        b=evidence.read(path);b['_sha256']=evidence.files[str(Path(path).resolve())]
        require(b.get('protocol_id')==PROTOCOL and b.get('model') in MODELS and b.get('system') in SYSTEMS,
            'retained attempt is from a different protocol/system')
        source_path,source_sha=DEFAULT_SOURCES[b['model']]
        require(b['files'].get(str(source_path))==source_sha,'retained attempt does not bind the shared source')
    ...
    for model in MODELS:
        ...
        bound=sorted(system for m,system in by_system if m==model)
    ...
    document=dict(schema=1,protocol_id=PROTOCOL,created_s=time.time(),counts_only=counts_only,
        models=by_model,points=points,request_counts=counts,retained_nonselected_attempts=attempts,
        validation_scope='declared source, complete CP artifact set/SHA, receipt/disk summary, bound configuration and offered-work/SLO arithmetic; native validity inherited from actual execution wrapper, not re-executed',
...
def main():
    ...
    p.add_argument('--retained-attempt-binding',type=Path,action='append',default=[])
    ...
    a=p.parse_args();result=snapshot(a.out,bindings=a.binding,retained_attempt_bindings=a.retained_attempt_binding,
