...
def test_eighteen_byte_exact_traces_and_original_pilot_order():
    ...
    assert len(spec['cells'])==len(original)==2
    assert [(c['dataset'],c['rate_rps']) for c in spec['cells'][:3]]==[('sharegpt',.25),('longbench',.125)]
    for c in spec['cells']:
...
def test_original_selection_uses_user_slos_and_uniform_configuration():
    ...
    assert len(contract.validate(spec,source,b.read(ROOT/'source-contract.json'),b.read,b.sha))==2
    assert [(r['slo_ttft_s'],r['slo_tpot_s']) for r in spec['cells'][:3]]==[(5.,.15),(15.,.2)]
...
def test_independently_frozen_anchor_contract_can_select_exact_original_three():
    ...
    contract['source_selection']['cell_ids']=[r['cell_id'] for r in spec['cells']];contract['expected_cells']=2
    assert len(mod.validate(spec,source,contract,b.read,b.sha))==2
...
def test_original_run_cell_injects_user_slos_before_real_controller_creation(tmp_path,monkeypatch,dataset):
    ...
    rows=b.read(ROOT/'runspec.json')['cells']+b.read(ROOT.parent/'B32B-load-screen-user-slo-v1/runspec.json')['cells']
...
def expanded_cpu_fixture():
    ...
    contract.update(expected_cells=6,requests_min=1000,requests_exact=None,duration_min_s=300.)
...
def test_new_frozen_contract_accepts_thousand_requests_three_seeds_and_300_seconds():
    ...
    assert len(mod.validate(spec,source,contract,traces.__getitem__,lambda key:key))==6
...
def test_new_contract_rejects_invalid_extended_work_and_protocol(fault):
    ...
    elif fault=='foreign_slo':spec['cells'][0]['slo_ttft_s']=15.
...
def test_post_quiesce_paused_idle_is_valid_but_setup_still_requires_accepting(monkeypatch):
    async def check():
        raw=state();raw.update(accepting=False,admit_prefill=False)
        async def http(*args,**kwargs):return copy.deepcopy(raw)
        monkeypatch.setattr(b,'http',http)
        result=await x.wait_idle(b,None,0,tokens=8192,accepting=None)
        assert result['accepting'] is False
        with pytest.raises(RuntimeError):await x.wait_idle(b,None,0,tokens=8192,seconds=0)
    asyncio.run(check())
