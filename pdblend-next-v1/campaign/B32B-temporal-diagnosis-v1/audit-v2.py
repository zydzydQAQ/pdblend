...
def main():
    ...
    identities = []
    for before, after in zip(status.get('identity_before', []), status.get('identity_after', [])):
        same = all(before['container'][k] == after['container'][k] for k in ('Id', 'Image'))
        same &= before['container']['State']['StartedAt'] == after['container']['State']['StartedAt']
        assert same
        identities.append(dict(id=before['runtime']['id'], same_container_image_start=same,
            nccl_environment_unchanged=before['nccl_environment'] == after['nccl_environment'],
            source_at_import_unchanged=before['provenance']['source_files_at_import'] == after['provenance']['source_files_at_import'],
            final_generation=after['runtime']['generation'], final_ack=after['runtime']['acknowledged_generation'],
            final_accepting=after['runtime']['accepting'],
            transfer_counter_deltas={k: after['runtime'].get(k,0)-before['runtime'].get(k,0)
                for k in ('transfer_send_started','transfer_send_completed','transfer_send_failed')}))
    report = dict(complete=status.get('complete'), diagnostic_cases_complete=status.get('diagnostic_cases_complete'),
        request_count=len(comparisons), total_output_tokens=sum(len(x['token_ids']) for x in comparisons), identities=identities,
