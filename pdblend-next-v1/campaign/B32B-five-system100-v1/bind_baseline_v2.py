...
import argparse,asyncio,copy,hashlib,importlib.util,json,pathlib,socket,sys,os
...
def main():
    ...
    p.add_argument('--strategy',choices=SYSTEMS);p.add_argument('--gate',type=pathlib.Path);a=p.parse_args();s=read(a.spec);h=pathlib.Path(s['host_release']);sys.path[:0]=[str(h/'src'),str(h),'/root/workspace/pdblend/.runtime-deps']
    require('PDBLEND_NODE_LOCK_FD' not in os.environ,'binding requires a fresh independent lease')
