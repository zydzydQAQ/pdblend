...
BASE=ROOT.parent/'B32B-batch-coverage-observation-v2-r2'
...
def main():
 ...
 output=ROOT/'audit.json';require(not output.exists(),'prior audit must not be overwritten');output.write_text(json.dumps(result,indent=2)+'\n');print(json.dumps({k:v for k,v in result.items() if k not in ('points','outputs','source_sha256','operation_per_gpu_energy_j')}))
