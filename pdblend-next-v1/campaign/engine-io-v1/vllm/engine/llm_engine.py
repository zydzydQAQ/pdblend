...
class LLMEngine:
    ...
    def __del__(self):
        ...
        for scheduler in getattr(self, "scheduler", ()):
            try:
                close_io = getattr(scheduler, "shutdown_pdblend_io", None)
                if close_io is not None and not close_io():
                    logger.warning("PDBlend scheduler I/O did not shut down cleanly")
            except Exception:
                logger.exception("PDBlend scheduler I/O shutdown failed")
