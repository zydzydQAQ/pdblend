...
def apply_scheduler_control(scheduler: Any) -> None:
    ...
    if os.environ.get("PDBLEND_ASYNC_IO") == "1":
        ...
        cache = io.caches.get("legacy")
        if cache is None or cache.path != path:
            state["control_error"] = "control path changed after startup"
            return
        payload, error, _ = cache.read()
        if error or payload is None:
            state["control_error"] = error
            return
        _accept_control(scheduler, state, payload["generation"], payload["mode"])
...
def emit_scheduler_snapshot(scheduler: Any, scheduler_outputs: Any) -> None:
    ...
    if os.environ.get("PDBLEND_ASYNC_IO") == "1":
        from .pdblend_io import initialize_engine_io
        io = initialize_engine_io(scheduler)
        if io.writer is None or io.writer.path != path:
            raise RuntimeError("telemetry path changed after startup")
        payload.update(scheduler_observed_s=payload["ts"],
            runtime_generation=(getattr(scheduler, "_pdblend_runtime", None) or {}).get("generation"),
            runtime_control_error=getattr(scheduler, "_pdblend_runtime_error", None),
            runtime_control_checked_s=getattr(scheduler, "_pdblend_control_checked_s", None))
        io.writer.publish(payload)
        return
