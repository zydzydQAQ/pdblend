"""PDB-only three-TP2 capacity screening, fixed host v1.2 and original traces."""
import asyncio
...
PREVIOUS = ROOT.parent / 'B32B-idle-tail-v2'
...
async def main():
    ...
    status = dict(phase='prepare_three_tp2', complete=False, started_s=time.time(),
        scope='PDB-only three-TP2 capacity screening, original per-dataset priors and traces retained',
        active_gpus=list(range(6)), measured_gpus=list(range(8)), controller_release=str(HOST))
    write('status.json', status)
    try:
        manifest = verify_inputs()
        previous = json.loads((PREVIOUS / 'status.json').read_text())
        if not previous.get('complete') or previous.get('phase') != 'finished':
            raise RuntimeError('B4 host correction must finish first')
        ...
        for row in before:
            ...
            if (not row['State']['Running'] or row['Id'] != frozen['Id']
                ...
                raise RuntimeError('frozen B4 resident identity differs')
        ...
        async with aiohttp.ClientSession(trust_env=False, timeout=aiohttp.ClientTimeout(total=10)) as session:
            ...
            for i in range(4):
                ...
                if (state.get('error') or state.get('runtime_error')
                    ...
                    raise RuntimeError('B4 engine is not completely drained')
                states[i] = state
            write('runtime-before.json', states)
        template = json.loads((PREVIOUS / 'longbench.config.json').read_text())
        template['instances'] = template['instances'][:3]
        if (template.get('output_prior') != 211 or template.get('park_idle') is not False
            ...
            or [i['gpus'] for i in template['instances']] != [[0, 1], [2, 3], [4, 5]]
            or any(i['role'] != 'mixed' or i['tp'] != 2 for i in template['instances'])):
            raise RuntimeError('three-TP2 fixed policy differs')
        ...
        stopped = await command('docker', 'stop', '-t', '10', 'pdb-next-b32q3')
        write('removed-residency.json', dict(stopped_preserved=['pdb-next-b32q3'],
