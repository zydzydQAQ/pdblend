...
class Controller:
    ...
    async def record_pdb_infeasible(self, snapshot, request, plan, active,
                                    event_kind='pdb_infeasible_snapshot'):
        # Optional development evidence only. A request records its first real
        # refusal snapshot before fallback clocks or later telemetry mutate it.
        if (not self.strategy.startswith('pdblend')
                or self.config.get('diagnostic_infeasible_snapshot') is not True
                or active.get('diagnostic_'+event_kind+'_recorded')):
            return
        active['diagnostic_'+event_kind+'_recorded'] = True
        await self.journal.emit(dict(kind=event_kind,
            ...
            scope='first observed planning snapshot of this kind; execution requires a later admission event'))
        ...
        try:
            while True:
                ...
                while rid in self.active:
                    now=time.time()
                    if now>request.arrival_s+request.ttft_s:
                    ...
                    if plan is None:
                        ...
                        break
                    if plan.reason.startswith('fresh idle mixed fallback;'):
                        await self.record_pdb_infeasible(snapshot,request,plan,active,
                            event_kind='pdb_stale_tail_fallback_snapshot')
    ...
    async def completions(self,request):
        ...
        completed=False
        try:
            route=await asyncio.wait_for(future,budget.ttft_s+1)
            ...
            future.cancel()
            self.pending.done(rid)
            if not completed and not self.active[rid]['budget'].emitted:
            ...
            if not completed and wire_id.startswith("pdb:"):
