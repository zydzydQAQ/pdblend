...
class JointPlanner:
    def __init__(self, profiles: ProfileStore, transfers=(), *, max_frequency=2520,
                 ...
                 protect_pending_decode=False,independent_idle_mixed_on_stale_tail=False):
    ...
    def point(self, instance, request, freq, batch, *, context=None):
        ...
        return self.profiles.lookup(instance.role, instance.tp, freq,
    ...
    def candidates(self, snapshot, request, now):
        ...
        if stale_tail:
            if not self.independent_idle_mixed_on_stale_tail:
                return plans
            # A long step cannot publish new telemetry until it returns. It
            # must not lend energy-sharing credit to a different admission.
            # Only a genuinely idle, fresh mixed endpoint can use this path;
            # no stale KV, pending transfer, or existing token budget is used.
            paths=[(m,m) for m in live if m.role=='mixed'
            ...
            contexts={i.instance_id:resident_context(i) for i in snapshot.instances}
        else:
        ...
        for p,d in paths:
            ...
            for freq in fs:
                ...
                # When an unrelated tail is unknown, score this new request
                # as standalone node work. This deliberately grants no old
                # tail credit; it is a ranking estimate, not an upper bound
                # on total energy of the unobserved in-flight requests.
                ...
                joules=(max(0.,pp.phase_power("prefill")-pp.residency_w)*pp.prefill_s/prefill_share
                        ...
                        + new_residency*after_tail-current_residency*node_tail + transfer_j
    ...
    def plan(self, snapshot: RuntimeSnapshot, pending: tuple[RequestBudget,...], *,
        ...
        if first[0].reason.startswith('fresh idle mixed fallback;'):
            # Route and frequency are still jointly enumerated. Do not build
            # a multi-request energy beam from an uncertain shared horizon.
            return first[0]
