...
from .policy import MixedLeastLoadPolicy, MixedReplica
from pdblend.bench.client import Request, nearest_rank
from pdblend.results.journal import CompactJournal, payload_receipt, file_sha256
    ...
    async with session.post(url+'/baseline/generate', json=payload) as response:
        ...
        async for line in response.content:
            ...
            if observe is not None:
                observe(event)
                event = {key:event[key] for key in ('token_ids', 'received_s', 'finished', 'token_index')
                         if key in event}
            events.append(event)
...
async def execute(specs, trace: list[Request], out: Path, *, duration_s: float,
    ...
    events = CompactJournal(out/'events.jsonl.gz')
    outcomes = CompactJournal(out/'outcomes.jsonl.gz')
    def emit(handle, row):
        return handle.write(row)
    rows = []
    try:
            ...
            async def submit(r):
                ...
                try:
                    ...
                    result = await generate(session, urls[route.instance_id], dict(
                        ...
                        observe=lambda event: emit(events, dict(kind='mixed_client_sse',
                            request_id=rid, at_s=event['received_s'], payload=event)))
                    ...
                    row.update(payload_receipt(result['events'], journal_path='events.jsonl.gz', request_id=rid))
    ...
    return dict(system='mixed', native_runner=True, started_s=started_s, finished_s=time.time(),
                ...
                journal_path='events.jsonl.gz', outcomes_path='outcomes.jsonl.gz',
                events_sha256=file_sha256(out/'events.jsonl.gz'),
                outcomes_sha256=file_sha256(out/'outcomes.jsonl.gz'),
                raw_schema='pdblend-journal-v1',
