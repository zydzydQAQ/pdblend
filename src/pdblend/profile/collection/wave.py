...
from pdblend.profile.collection.parallel import common_window_overlap, evaluate_interference
...
class ProfileWave:
    ...
    def __init__(self, root: Path, member: str, *, timeout_s: float = 3600):
        ...
        self.qualification_measure_s = self.spec.get('qualification_measure_s', 5.)
        if (type(self.qualification_measure_s) not in (int, float)
                or not math.isfinite(self.qualification_measure_s)
                or self.qualification_measure_s < 5.):
            raise ValueError('qualification measurement must be finite and at least 5 seconds')
    ...
    async def probe(self, profiler, phase: str) -> dict:
        # A heterogeneous peer can finish an in-flight warmup burst after the
        # ready barrier opens. The frozen cohort can reserve a longer probe
        # window, but the actual common-window check still decides validity.
        # Keep the caller's ordinary training/holdout window unchanged.
        profiler = copy.copy(profiler)
        profiler.decode_measure_s = self.qualification_measure_s
        local_ready = {}
    ...
    async def qualify(self, profiler, fleet=None) -> None:
        ...
        evidence = dict(complete=True, passed=self.parallel, cross_job=cross_job,
                        ...
                        qualification_measure_s=self.qualification_measure_s,
