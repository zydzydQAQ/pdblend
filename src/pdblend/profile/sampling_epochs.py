...
class SamplingEpochs:
    ...
    def _check(self, state):
        if state.get('errors'):
            raise RuntimeError('sampling cohort failed: '+json.dumps(state['errors']))
    ...
    def fail(self, exc):
        with self._state() as state:
            failure = dict(error=repr(exc), at_s=time.time())
            state.setdefault('errors', {})[self.member] = failure
