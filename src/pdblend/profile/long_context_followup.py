...
from . import long_context_collect as lc
from .identity import sha256_value
from .long_context_plan import FREQUENCIES
...
def source_hashes():
    ...
    return {name:lc.digest(here/name) for name in ('long_context_followup.py','long_context_collect.py',
        'profiler.py','window_sampling.py','wave.py','parallel.py','identity.py')}
...
def prepare_training_only(*,training_manifest,out):
    ...
    from .calibration import _checkpoint_points
