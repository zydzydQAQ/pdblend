...
from .calibration import _checkpoint_points, evaluate_holdout
from .identity import sha256_value
from .model import PerfModel
from .power_table import validate as validate_table
from .wave import ProfileWave, atomic_json
...
def source_hashes():
    return dict(pc.implementation_hashes(), **{'profile/local_power.py':pc.digest(__file__),
        'profile/local_mixed_repair.py':pc.digest(Path(__file__).with_name('local_mixed_repair.py'))})
...
def prepare(*, candidate, plan, base_candidate, training_raw, prior_holdout, out, original_holdout=None):
    ...
    if original_holdout is not None:
        from .local_mixed_repair import plan_repair
...
class Adapter:
    ...
    def needs_followup(self,package,out):
        from .local_mixed_repair import needs_collection
    ...
    async def after_samples(self,*,profiler,client,gpus,package,out):
        from .local_mixed_repair import collect_existing
        manifest,_,_=load_package(package)
    ...
    def audit(self,package,raw,out,manifest,plan,model,binding):
        ...
        if manifest.get('mixed_repair') is not None:
            from .local_mixed_repair import audit_repair
