...
class ShortClockMismatch(ValueError):
    """A rejected measurement, never permission to relabel its frequency."""
    def __init__(self, target, frequency):
        values=[v for _, row in frequency for v in row]
        self.diagnostics=dict(target_mhz=target,mean_mhz=statistics.fmean(values),
            minimum_mhz=min(values),maximum_mhz=max(values),samples=len(frequency),
            per_gpu_mean_mhz=[statistics.fmean(row[i] for _,row in frequency)
                for i in range(len(frequency[0][1]))])
        super().__init__('short window actual clock mismatch: '+str(self.diagnostics))
...
def summarize(evidence):
    ...
    if round(freq)!=p['freq_mhz']:raise ShortClockMismatch(p['freq_mhz'],frequency)
