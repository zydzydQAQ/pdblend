...
from pdblend.online.policies import Policy
from pdblend.planner.topology import ResidentPool, TopologyPlanner, candidate_topologies
from pdblend.online.tp_modes import TPMode
from pdblend.engine.launcher import InstanceSpec, make_specs
...
def prepare_tp_runtime(*, model_name: str, gpus: Sequence[int], fixed_tp: int, mode: str,
    ...
    for topology, path in profiles.items():
        ...
        try:
            loaded = load_profile(Path(path), system='pdblend', model_id=spec.model_id,
                                  tp=topology[0], pp=topology[1])
        except VersionError as exc:
            raise ValueError(f'independent profile selection failed: {exc}') from exc
        model = loaded.model
        _identity(model, spec.model_id, topology)
        models[topology], paths[topology] = model, Path(path)
        selections[f'tp{topology[0]}-pp{topology[1]}'] = loaded.manifest_fields()
    if not models:
