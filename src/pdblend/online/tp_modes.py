...
from pdblend.online.reshard import ReshardCoordinator, TopologyTarget
...
class ResidentTPRouter:
    ...
    def route(self, request_id: str, input_tokens: int, *, prefer_tp: int | None = None,
              model_id: str | None = None, max_tokens: int = 1, prefer_pd: bool = False,
              allowed_routes: set[tuple[str, ...]] | None = None,
              choice_scores: dict[tuple[str, ...], float] | None = None) -> dict | None:
        ...
        with self._lock:
            ...
            def allowed(choice):
                ids = tuple(m.instance_id for m in choice)
                return ((allowed_routes is None or ids in allowed_routes)
                        and (choice_scores is None or ids in choice_scores))
            choices = [choice for choice in choices if allowed(choice)]
            if prefer_pd:
                ...
                pairs = [pair for pair in pairs if allowed(pair)]
                if pairs:
                    choices = pairs
            if allowed_routes is not None:
                choices = [c for c in choices if tuple(m.instance_id for m in c) in allowed_routes]
            if choice_scores is not None:
                choices = [c for c in choices if tuple(m.instance_id for m in c) in choice_scores]
            if not choices:
                return None

            def rank(choice):
                if choice_scores is not None:
                    value = choice_scores[tuple(m.instance_id for m in choice)]
                    if not math.isfinite(value) or value < 0:
                        raise ValueError("resident choice score must be finite and nonnegative")
