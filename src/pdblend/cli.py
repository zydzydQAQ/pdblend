...
def main(argv=None) -> None:
    ...
    be.add_argument("--tp-mode", choices=["fixed_tp", "offline_tp", "resident_hetero_tp", "slow_reshard_tp"])
    be.add_argument("--topology-profiles", type=Path, help="JSON mapping tpN-pp1 to explicit profile paths")
    be.add_argument("--resident-pools", type=Path, help="JSON list of explicit pool_id/topology/replicas layouts")
    be.add_argument("--joint-resident", action="store_true")
    be.add_argument("--incremental-energy-path", type=Path)
    be.add_argument("--transition-catalog-path", type=Path)
    be.add_argument("--capacity-floor-path", type=Path)
    be.add_argument("--transition-qualified-only", action="store_true")
