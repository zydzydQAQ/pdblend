...
async def _await_initial_transition(controller, task, timeout_s):
    """Wait for the controller's real completed setup; never invent a plan ACK."""
    started = time.time()
    async def ready():
        while True:
            if task.done():
                await task  # Preserve native/control failures, including cancellation.
                raise RuntimeError('controller exited before service readiness')
            completed = [r for r in controller._log if r.get('kind') == 'transition_complete']
            if controller.plan_now is not None and completed:
                return dict(started_s=started, ready_s=time.time(),
                    transition_id=completed[0]['transition_id'],
                    transition_finished_s=completed[0]['finished_s'],
                    evidence='controller_completed_physical_transition')
            await asyncio.sleep(.05)
    return await asyncio.wait_for(ready(), timeout_s)
...
async def _point(fleet: Fleet, gpus: Gpus, model: PerfModel, policy: Policy, slo: SLO, trace: list[Request],
    ...
    initial_ready = None
    try:
        if comparison_wait_initial_plan:
            initial_ready = await _await_initial_transition(ctl, ctl_task, tail_timeout_s)
        await asyncio.sleep(2.0)
        load = LoadClient(f"http://127.0.0.1:{proxy_port}", sampling_seed=sampling_seed,
                          **({'token_diagnostics': True} if comparison_record_tokens else {}))
        if warmup:
        ...
        if getattr(load, 'replay_started_s', None) is not None:
            t_start = load.replay_started_s
