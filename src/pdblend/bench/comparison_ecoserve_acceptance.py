"""Raw evidence audit for an EcoServe author-lookup single observation.

A quiet 150-second window need not trigger scaling again: qualified unchanged
mechanisms can be inherited. Every observed action must nevertheless have its
own native receipts. Neither old full-profile flags nor SLO success is promoted.
"""
...
from .comparison_ecoserve_inputs import validate_ecoserve_inputs, eco_topology, _source
...
RAW_REFS=('trace','native_result','events','power','canonical_requests','metering',
          'startup_qualification','reset','drain')
...
def _startup(point, identity, startup, instances, raw_refs, preflight):
    ...
    for row in rows:
        ...
        for flag,value in (('--tensor-parallel-size','1'),('--pipeline-parallel-size','1'),
    ...
    for iid,spec in instances.items():
        cap=caps[iid]
        _need(cap.get('supported') is True and cap.get('tp')==cap.get('pp')==1
        ...
        state=cap['state'];_state(state,1,cap.get('received_s',state.get('response_at_s')))
...
def _reset(reset, instances, origin):
    generations,times=_drained(reset.get('drain'),instances)
    _need(reset.get('initial_clock_mhz')==2520 and _finite(reset.get('reset_s')) and reset['reset_s']>=0,
          'window reset clock/duration missing')
    acks,states=reset.get('reopen_ack',{}),reset.get('reopen_state',{})
    _need(set(acks)==set(states)==set(instances),'window reset fleet incomplete')
    for iid,spec in instances.items():
        state=states[iid];received=state.get('response_at_s');generation=_state(state,spec['tp'],received)
        _need(max(times)<=received<=origin and acks[iid].get('acknowledged') is True
              and acks[iid].get('generation')==generation and generation>generations[iid]
              and generation==reset.get('generation',{}).get(iid),'fresh reset generation ACK differs')
        _need(state.get('role')=='mixed' and state.get('mode')=='temporal'
              and all(state.get(k) is True for k in ('accepting','admit_prefill','admit_decode')),
              'role/admission reset differs')


def _tokens(events, *, require_terminal=True):
    result=[];times=[];terminal=False
    for envelope in events:
        payload=envelope.get('payload',{});ids=payload.get('token_ids')
        _need(isinstance(ids,list) and all(type(t) is int and t>=0 for t in ids),'exact token IDs missing')
        stamp=payload.get('received_s',envelope.get('at_s'))
        _need(_finite(stamp) and (not times or stamp>=times[-1]),'token receipt times regress')
        _need(not terminal,'client/native events follow terminal event')
        result.extend(ids);times.extend([stamp]*len(ids))
        _need(payload.get('token_index')==len(result),'token index discontinuity')
        terminal=payload.get('finished') is True
    if require_terminal:
        _need(terminal,'client/native terminal stream missing')
    return result,times



def _observation_end(native, events):
    """Use recorded cohort/close times when global gather timed out.

    This is an audit boundary, never an invented runner completion field.
    Native close proves observation reached the full 150s service boundary;
    every request must have a finite terminal-outcome timestamp before close.
    """
    origin=native.get('service_started_s')
    _need(_finite(origin),'native service origin missing')
    closed=[r for r in events if r.get('kind')=='eco_closed']
    _need(len(closed)==1 and _finite(closed[0].get('at_s')) and closed[0]['at_s']>=origin+150,
          'raw controller close does not prove the complete service window')
    close_s=closed[0]['at_s'];ends=[]
    outcomes=native.get('outcomes',[])
    _need(outcomes,'terminal outcome cohort missing')
    for row in outcomes:
        finished=row.get('finished_s');arrival=row.get('arrival_s')
        _need(_finite(arrival) and _finite(finished) and origin+arrival<=finished<=close_s,
              'request terminal-outcome time missing or outside observation')
        ends.append(finished)
    reported=native.get('service_finished_s')
    if reported is not None:
        _need(_finite(reported) and max(origin+150,*ends)<=reported<=close_s,
              'reported service completion contradicts raw cohort/close times')
        return dict(observation_end_s=reported,source='native_result.service_finished_s',
                    controller_closed_s=close_s,cohort_terminal_end_s=max(ends))
    _need(native.get('error'), 'missing service completion requires an explicit runner failure')
    return dict(observation_end_s=close_s,source='complete_cohort_and_eco_closed',
                controller_closed_s=close_s,cohort_terminal_end_s=max(ends))


def _layout_add(groups,iid):
    groups=[list(g) for g in groups]
    if not groups:return [[iid]]
    index=next((i for i,g in enumerate(groups) if len(g)<3),0)
    groups[index].append(iid)
    if len(groups[index])>3:
        old=groups[index];groups[index:index+1]=[old[:-2],old[-2:]]
    return groups


def _shutdown_read_cancel(row, native, closed_s):
    """Closing the controller cancels an outstanding read-only polling task.

    The complete cohort and subsequent all-rank drains are still mandatory.
    This exception never applies to a failed mutation or an in-service read.
    """
    ends=[r.get('finished_s') for r in native.get('outcomes',[])]
    stamp=row.get('at_s');started=row.get('started_s')
    return bool(ends and all(_finite(t) for t in ends)
        and row.get('error')=='CancelledError()' and row.get('method')=='GET' and row.get('body') is None
        and row.get('path','').split('?',1)[0] in ('/baseline/state','/baseline/events')
        and _finite(stamp) and _finite(started) and started<=stamp
        and max(native['service_started_s']+150,*ends)<=stamp<=closed_s)


def _isolated_meter_method(point, identity, startup, native, metering, raw_refs):
...
def _observed_active_frequency(events, native, identity, instances, power, observation_end_s):
    """Independent physical-frequency gate; control provenance stays in _protocol."""
    origin=native['service_started_s'];end=origin+150
    clocks={iid:[] for iid in instances}
    for row in events:
        if row.get('kind')!='eco_http_receipt' or row.get('error'):continue
        if row.get('path') in ('/baseline/clock','/baseline/park'):
            _need(row.get('instance_id') in clocks,'frequency transition belongs to unknown instance')
            clocks[row['instance_id']].append((row['at_s'],row['path']=='/baseline/clock'))
    # Preserve exactly the original settle, coverage, gap and +/-30 MHz checks.
    samples=power.get('frequency_samples',[])
    for iid,changes in clocks.items():
        indices=[identity['fleet_gpu_uuids'].index(u) for u in instances[iid]['gpu_uuids']]
        for index,(at,active) in enumerate(changes):
            finish=changes[index+1][0] if index+1<len(changes) else observation_end_s
            left,right=max(origin,at+1.),min(end,finish)
            if not active or right<=left:continue
            selected=[(stamp,values) for stamp,values in samples if left<=stamp<right]
            _need(selected and selected[0][0]<=left+1. and selected[-1][0]>=right-1.
                  and all(0 < b[0]-a[0] <= 1. for a,b in zip(selected,selected[1:]))
                  and all(len(values)==8 and all(_finite(values[g]) and abs(values[g]-2520)<=30 for g in indices)
                          for stamp,values in selected),'actual active GPU frequency unproven or different')


def _protocol(events, native, config, identity, instances, reset, power, observation_end_s, lifecycle=None):
    origin=native['service_started_s'];end=origin+150
    ...
    for iid,state in states.items():
        generation=_state(state,1,state.get('response_at_s'))
    ...
    for row in events:
        ...
        if row.get('error'):
            from .comparison_ecoserve_lifecycle import bound_read_cancel
            allowed = (bound_read_cancel(row, lifecycle) if config.get('eco_comparison_lifecycle')
                       else _shutdown_read_cancel(row,native,closed[0]['at_s']))
            _need(row.get('instance_id') in instances and allowed,
    ...
    for index,row in enumerate(events):
        ...
        if kind=='eco_membership_commit':
            ...
            if row.get('operation')=='add':
                ...
                state=drained[-1]['observed_engine_state'];_state(state,1,state['response_at_s'])
...
def audit_ecoserve_window(point, engine_identity, startup_qualification, reset, native_result,
    ...
    lifecycle=None
    if point.get('eco_comparison_lifecycle') or (preflight.get('config') or {}).get('eco_comparison_lifecycle'):
        from .comparison_ecoserve_lifecycle import audit_lifecycle_events
        lifecycle=gate('eco.comparison_lifecycle',lambda:audit_lifecycle_events(
            data['events'],native_result,preflight['config']))
    ...
    def metrics():
        ...
        protocol=_protocol(data['events'],native_result,config,engine_identity,instances,reset,data['power'],boundary['observation_end_s'],lifecycle)
    ...
    valid=not failures
    return dict(schema='ecoserve-single-observation-acceptance-v1',scope='author_lookup_protocol+single_observation',
