...
from .forecast import percentile
from .planner import ACTIVE, PARKED, Plan, SLO
...
class Shield:
    ...
    def observe(self, records, now: float) -> Pressure:
        ...
        p.decode = p.tpot_p90 > self.threshold * self.slo.tpot_s
        return p
    ...
    def apply(self, plan: Plan, pressure: Pressure, max_freq: int) -> Plan:
        ...
        if self.level:
            ...
            return Plan(counts, max_freq, max_freq, max_freq, plan.tau, plan.power_w, plan.ttft_s,
                        plan.tpot_s, detail)
        if counts == plan.counts:
            return plan
        detail = dict(plan.detail, shield_floor=self.floor_active)
        return Plan(counts, plan.f_P, plan.f_D, plan.f_M, plan.tau, plan.power_w, plan.ttft_s,
                    plan.tpot_s, detail)
