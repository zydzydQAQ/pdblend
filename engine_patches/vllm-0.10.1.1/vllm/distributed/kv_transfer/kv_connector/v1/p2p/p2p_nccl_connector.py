...
class P2pNcclConnector(KVConnectorBase_V1):
    ...
    def __init__(self, vllm_config: "VllmConfig", role: KVConnectorRole):
        ...
        self.is_consumer = self.config.is_kv_consumer
        # Rank-0 ZMQ address of this engine; a request whose decode_addr is
        # this address is a decode leg here, any other decode_addr makes it
        # a prefill leg, and an id without decode_addr is a plain request.
        self._zmq_address = f"{get_ip()}:{int(self.config.kv_port)}"
        self.chunked_prefill: dict[str, Any] = {}
        # decode legs whose remote KV was already scheduled for loading; the
        # engine releases a slice on consumption, so a preempted request must
        # recompute its prefill locally instead of waiting for it again
        self._kv_loaded: set[str] = set()
    ...
    def get_num_new_matched_tokens(
        ...
        """
        if self.leg(request.request_id) != "D" \
                or request.request_id in self._kv_loaded:
            return 0, False
    ...
    def update_state_after_alloc(self, request: "Request",
        ...
        """
        if num_external_tokens > 0:
            self._kv_loaded.add(request.request_id)
    ...
    def build_connector_meta(
        ...
        for new_req in scheduler_output.scheduled_new_reqs:
            ...
            if new_req.req_id in self._requests_need_load:
                meta.add_load(request_id=new_req.req_id,
                              token_ids=new_req.prompt_token_ids,
                              block_ids=new_req.block_ids[0],
                              block_size=self._block_size)
        ...
        for i, req_id in enumerate(cached_reqs.req_ids):
            ...
            if req_id in self.chunked_prefill:
                num_scheduled_tokens = (
            ...
            if req_id in self._requests_need_load:
                ...
                meta.add_load(request_id=req_id,
                              token_ids=token_ids,
                              block_ids=block_ids,
                              block_size=self._block_size)
    ...
    def request_finished(
        ...
        self._kv_loaded.discard(request.request_id)

        return False, None

    def leg(self, request_id: str) -> str:
        """'P' (send KV after prefill), 'D' (load KV, skip prefill) or ''."""
        match = re.search(r"___decode_addr_(.*):(\d+)", request_id)
        if match is None:
            return ""
        if f"{match.group(1)}:{match.group(2)}" == self._zmq_address:
            return "D" if self.is_consumer else ""
        return "P" if self.is_producer else ""
