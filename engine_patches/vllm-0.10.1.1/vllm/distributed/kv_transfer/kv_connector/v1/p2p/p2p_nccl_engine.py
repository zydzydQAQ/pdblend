...
# one PUTS batch is received into a single flat buffer; keep it small so the
# receiver never has to find a multi-GB contiguous block next to the KV cache
...
class P2pNcclEngine:
    ...
    def send_tensor(
        ...
        # Gather on the compute stream: the KV write for this layer is queued
        # there, so an idle send_stream would otherwise read stale slots.
        kv = self.extract_kv_from_layer(is_mla, tensor, slot_mapping)
    ...
    def recv_tensor(
        ...
        if self.send_type == "PUT" or self.send_type == "PUT_ASYNC":
            ...
            with self.recv_store_cv:
                ...
                # the slice is injected into the KV cache exactly once; keeping
                # it here until the request finished doubled every D request's
                # prefill KV and OOMed the receiver under sustained load
            ...
            if tensor is not None:
                if isinstance(tensor, tuple):
                    ...
                    tensor = self.pool.load_tensor(addr, dtype, shape,
                                                   self.device)
                    # load_tensor's H2D copy is asynchronous; the block must not
                    # be reused by the receive thread before it lands
    ...
    def listen_for_requests(self):
        while True:
            ...
            if data["cmd"] == "NEW":
                ...
                try:
                    with torch.cuda.stream(self.recv_stream):
                        flat = torch.empty(data["total"],
                                           dtype=getattr(torch, data["dtype"]),
                                           device=self.device)
                    self.router_socket.send_multipart([remote_address, b"0"])
                    comm, rank = self.comms[remote_address.decode()]
                    self.recv(comm, flat, rank ^ 1, self.recv_stream)
                    offset = 0
                    with torch.cuda.stream(self.recv_stream):
                        for meta in metas:
                            numel = math.prod(meta["shape"])
                            # a view would pin the whole flat buffer until every
                            # slice of the batch has been consumed; copy out
                            tensor = flat[offset:offset + numel].view(
                                meta["shape"]).clone()
                            offset += numel
                            tensor_size = tensor.element_size() * numel
                            if (self.buffer_size + tensor_size
                                    > self.buffer_size_threshold):
                                addr = self.pool.store_tensor(tensor)
                                received[meta["tensor_id"]] = (addr,
                                                               tensor.dtype,
                                                               tensor.shape)
                            else:
                                self.buffer_size += tensor_size
                                received[meta["tensor_id"]] = tensor
                        self.recv_stream.synchronize()
                    del flat
                except torch.cuda.OutOfMemoryError:
                    self.router_socket.send_multipart([remote_address, b"1"])
                    received = {meta["tensor_id"]: None for meta in metas}
                    logger.warning(
                        "🔴[PUTS]Recv Batch, Out Of Memory, %s👈%s, "
                        "tensors:%d", self.zmq_address, remote_address.decode(),
                        len(metas))

                with self.recv_store_cv:
                    for tensor_id, tensor in received.items():
                        self.recv_store[tensor_id] = tensor
                        self.have_received_tensor_id(tensor_id)
    ...
    def send_async(self):
        while True:
            with self.send_queue_cv:
                ...
                items = list(self.send_queue)
            groups: dict[tuple[str, torch.dtype], list[SendQueueItem]] = {}
            for item in items:
                groups.setdefault((item.remote_address, item.tensor.dtype),
                                  []).append(item)
            for group in groups.values():
                chunk: list[SendQueueItem] = []
                chunk_bytes = 0
                for item in group:
                    nbytes = item.tensor.element_size() * item.tensor.numel()
                    if chunk and chunk_bytes + nbytes > MAX_BATCH_BYTES:
    ...
    def send_sync(self, item: SendQueueItem) -> bool:
        ...
        if item.ready is not None:
            self.send_stream.wait_event(item.ready)
        tensor = item.tensor
    ...
    def send_batch(self, items: list[SendQueueItem]) -> bool:
        """One handshake and one NCCL transfer for many (request, layer) KV slices.

        Per-slice PUTs cost a ZMQ round trip each, so a 28-layer model paid ~30 ms
        of handshakes per request and the single send thread serialised all
        prefill requests behind it."""
        ...
        with torch.cuda.stream(self.send_stream):
            flat = torch.cat([item.tensor.reshape(-1) for item in items])
        data = {
            "cmd": "PUTS",
        ...
        if response != b"0":
            logger.error(
                "🔴Send Batch, Peer Out Of Memory/Threshold, %s 👉 %s, "
                "MyRank:%s, tensors:%d, size:%fGB, response:%s",
                self.zmq_address, remote_address, rank, len(items),
                flat.element_size() * flat.numel() / 1024**3,
                response.decode())
            return False

        self.send(comm, flat, rank ^ 1, self.send_stream)
        for item in items:
            self.have_sent_tensor_id(item.tensor_id)
        return True
