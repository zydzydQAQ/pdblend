...
使用 [`load_version`](src/pdblend/profile/versions.py) 显式选择版本，返回模型、覆盖域和可写入
...
截至 2026-09-23 18:44，7B Mixed 的独立 calibration/tuning 倍率标定已完成，
x0.5 对应 Alpaca/ShareGPT/LongBench 为 12/4/1.5 请求每秒。三条 seed701、300 秒评价
轨迹已冻结到 `results/2026-09-23/first-five-system-batch-v2/`；该目录仍无正式通过点。
EcoServe 7B/14B 自动 macro 任务已结束，32B 窗口正在执行，动作凭据与 CSV 校准分别验收。
六成员 PDBlend/Dynamo 增量 profile 波次已入队，使用 2+1+1+2+1+1 卡分组和采样阶段隔离。
这些是带时间的现场快照，后续进度以实时状态页及原始 receipt 为准。
